...
| **那個 handler 未認證可達,而公告只說「已認證的攻擊者」** | 閘門在 `0x0040bb1c`,只有 URI 含 `.htm`/`.asp` 才跑;`0x0040beb8` 的 `beq` 直接跳過 `send_r_unauthorized`。**反編譯器出了三個警告,所以每個分支都在指令層級讀過** | 我知道反編譯器什麼時候不可信,也知道那時候該去哪裡確認。而且我讀出來的東西**比公告本身更嚴重**,不是複述它 |
...
- **一個 request 都沒送出去。** 全部是靜態閱讀。那條 RCE 鏈的每一句話都是「程式碼讀起來是 X」,`/tmp/syscmd.log` 沒有人看過,port 沒有人掃過。
...
- **這台的版本號 `V2.1.6` 是不是公開下載得到的那個,我不知道。** 我讀到的是搜尋索引,**頁面本身 403,我沒讀到**。這個區別我第一次寫的時候寫太滿,被問了才改。
