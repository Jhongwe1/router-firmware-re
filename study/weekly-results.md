...
## W05 — 動態分析上半(無 gate,DoD,2026-08-17)

### 一句話

> 「大家說 Realtek SDK 模擬不起來,理由通常是 Lexra 指令集 —— **那個理由是錯的**。
> 真正卡住的是 `libapmib` 要讀 `/dev/mtdblock0`,而它用的是 `lseek`+`read`,不是 `ioctl`,
> **所以那個死結的解法是提供一個檔案**。我手上有那顆 flash 的逐 byte 副本,
> 因為那是硬體那一週的副產品。結果不只跑起來了 —— **廠商自己的 binary 回過頭來
> 驗證了我自己寫的兩個解析器**。」
...
| 主張 | 證據 | 它證明我會什麼 |
|---|---|---|
| **把「模擬不起來」拆成三個可檢驗的子問題,而且三個都給了答案** —— 指令集(不是問題,`strace` 的 opcode 手算驗過)、裝置節點(是問題,解法是一個檔案)、對齊陷阱(是問題,而且**是模擬器的範圍問題不是韌體的缺陷**) | [`notes/emulation-2018.md`](../notes/emulation-2018.md);`libapmib.so+0x27dc` 的 `sh s7,0(s8)`,編碼 `0xa7d70000` 手算對過原始 bytes | **不接受「跑不起來」這種結論**。一個死結被拆成三個具名的、可以各自處理的問題,而其中兩個的答案是「這不是我以為的那個問題」 |
| **用被測目標自己的 binary 去驗證自己寫的工具** —— `flash extr /web` 寫出 143 個檔,**每一個的 SHA-256 都跟 `fwrecon web` 事先宣告的相同**;`flash all` 的 2,317 行跟 `fwrecon compcs` 在 316 個共同名稱上 **249 個逐字相同**,66 個由四條渲染規則解釋(**用一支「有殘餘就 exit 1」的腳本,不是用眼睛**),1 個剩下 | [`notes/emulation-2018.md` §6](../notes/emulation-2018.md) | **知道什麼叫獨立的第二個來源**。`w6cg` 這個格式裡沒有 checksum、沒有項目數,所以在今天之前那個解析器的唯一檢查是結構性的 —— 現在它被一支不共用任何程式碼的程式逐 byte 確認了 |
| **一個 HTTP 參數的形狀,可以指到 SPI flash 上被改掉的第幾個 byte** —— `flash set HW_WLAN0_WSC_PIN 1` 在 4 MiB 裡只改 3 個 byte,而第三個是 `H601` 區的 8-bit 校驗和,**兩個相距 0x3EE 的欄位都精確吻合負值** | `sudo tools/qemu-env.sh diff`;[`notes/oracle-design.md`](../notes/oracle-design.md) | **在碰真機之前就把觀測手段驗過**。W06 對硬體的預測因此是具體的(「這幾個位址會變」),而不是「應該會有東西變」—— 而具體的預測才可能被推翻 |

### 這週沒證明什麼

- **這台裝置一個網路封包都沒有收到。** 31 個登記項目裡 **25 個還是 `⬜`**,
  而且原因很平凡:這台筆電沒有內建網路孔。**不是「大致完成」,是這一半沒開始。**
- **`boa` 沒有服務過任何請求,連在模擬環境裡都沒有。** 它死在一個未對齊的半字存取上。
  所以整個 Phase 2(那道授權閘門的實際涵蓋範圍)仍然是**讀出來的**,不是跑出來的 ——
  跟 W04-2 收工時完全一樣。今天做的是「打中的時候看得到嗎」,不是「打得中嗎」。
- **`flash set` 在模擬環境裡是寫一個普通檔案。** 真機走 MTD,有抹除區塊、有讀-改-寫。
  所以那三個 byte 是**對真機的預測**,不是對真機的量測 —— 而它可能是錯的,
  那正是它有價值的原因。
- **`FLW` 的磁區語意還是未決的**,而且**證據不夠的原因是我自己的作業單設計錯了**:
  讀回用了上一步已經填過同樣內容的 RAM 位址,`RUNBOOK.md` §8.7.8 早就用名字警告過那個坑。
- **有一筆解碼差異我沒有解決,而且解決不了** —— `L2TP_SERVER_IP_ADDR`
  我的 MIB 表說 64 bytes、廠商的 binary 印成 IPv4,**而那個欄位每一個 byte 都是零,
  所以資料沒有能力仲裁**。寫成「記錄,未解決」,不寫成「小差異」。
