...
> 2. **每一點都要說出「它證明我會什麼」** —— 面試官買的是能力,不是路由器。
...
- **面試前一晚只讀這份。** 每週的「一句話版本」是 30 秒自我介紹的素材,「三個點」是被追問時的深度。
...
> 面試講法:「我從這個專案學到最重要的一件事,不是逆向技巧,是**一個不會失敗的檢查等於沒有檢查**。我有十個工具 bug 的紀錄,沒有一個是工具自己發現的 —— 全部是靠交叉比對。所以我現在寫工具的時候,第一件事是想『它錯的時候會長什麼樣』。」
