...
`docs/disclosure.md` 的第 2 步就是 prior-art，寫在那裡至少三天了，三天前才剛因為
`CVE-2023-34435` 吃過同一個虧。昨天量到 `pc = ra = s0..s6 = 0x41414141` 之後，寫了
`PROGRESS.md` 一整段、`notes/` 一節、`BENCH-LOG.md` 一節，**然後才搜**。搜出來是
`CVE-2025-4462`：同一個端點、同一個參數、同一類缺陷、2025-05-09 公開、附 PoC。
花了不到五分鐘。

要辯護的只有一件事：**搜完之後，主張確實被改小了，而且是主動改小的。** 現在寫的
是「在兩個沒有被任何 CVE 點名的 build 上重現，並且量到偏移」，不是「發現」。
`docs/disclosure.md` 裡有一整段記著這件事發生的順序，而不是把它抹掉。

**但辯護不等於做對。**正確的順序是搜完再決定要不要寫，因為「值不值得寫」本身就取決
於搜出來的東西。這一條已經進 `docs/disclosure.md`。
</details>
