...
**kernel 三個版本的長度剛好各差 1,024 bytes。**
而下載到的那個檔案,在**兩個 section 上都精準落在 2015 和 2018 之間的曲線上**。

這正是週計畫要的那個論證:「你怎麼知道第三方鏡像上那個檔沒被動過?」
**正確答案不是「應該不會啦」,是「我不知道,但它的 section 長度跟前後版本連續,
一個被改過的檔不太可能剛好落在那條線上」** —— 而這種話只有真的做了橫向比對才敢講。

**但這仍然只是前綴。** 沒有 rootfs、沒有 `/etc/version`、沒有 `boa`。
要做真正的比對得重抓,而且**先驗 zip 自己的 CRC-32 再說**。
