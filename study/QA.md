...
## 2026-08-18（W07 Day 4）—— 一個敵意的讀者今天會問的六題

<details>
<summary><strong>Q. 你說「五個 handler 死在同一個位址」，但那是 qemu。裝置上呢？</strong></summary>

沒量過，而且我不會用「應該一樣」帶過。**可以說的是機制的差別**：這五發的故障是
**寫入一個唯讀頁**，不是未對齊存取。Linux/MIPS 的核心會在 trap handler 裡替使用者
空間補未對齊存取（那正是 `tools/alignfix/` 要重現的東西），**但它不會替你補權限
錯誤**。所以這一類是少數幾個「從模擬外推到矽」有結構理由的，而**那是一個論證，
不是一次量測**。要驗它的那一節是 `runsheet.md` `A3.23`，它排在今晚。

而 `D-11` 在實機上已經量到「一個未認證的合法請求讓 web server 不再回來」，
當時說不出是哪一個。**現在模擬側有五個名字，兩邊的形狀對得上** —— 那仍然不是同一
件事被證明。
</details>

<details>
<summary><strong>Q. `P4-1` 你們自己判 refuted，現在又說它成立。到底哪一個算？</strong></summary>

**兩個都算，而且它們不衝突。** `P4-1` 在實機上對 `formNtp`、`formWlanSetup`、
`formSelLang` 送出「缺 `submit-url`」的 POST，三個都活著 —— **那個量測沒有被推翻**。

被推翻的是從它推出去的那句話（「這個類別在這台不成立」）。47 個 handler 帶這個
慣用語，只有 5 個在無參數 POST 下真的走到那個 tail；三個手挑的 handler 各有大約
四分之一的機會命中。而第五個 `formSchedule` **用那個方法永遠找不到**，
因為它讀的參數不叫 `submit-url`。

**可以帶走的那一句是**：一個反證繼承了產生它的那個東西的覆蓋率，而「三個手挑的
handler」是一個沒有人寫下來的覆蓋率。
</details>

<details>
<summary><strong>Q. `$pc = 0x41414141` 很漂亮，但你有 exploit 嗎？</strong></summary>

**沒有，而且我不會把它講成有。** 沒有跳到任何地方、沒有 payload、
`qemu-user` 的位址空間不是裝置的位址空間。有的是三件事：偏移 509、
`s0`–`s6` 也可控、以及這支 binary 沒有 canary、沒有 `PT_GNU_RELRO`、沒有 PIE、
`GNU_STACK` 是 `RWX`（三個 build 都一樣，`tools/mipsref.py --segments`）。

**還有兩件我不知道的**：它在公開的 V2.1.2 映像上成不成立（今天那一發沒跑成），
以及 `localPin` 的溢位是不是已經有人發表過（**一次 prior-art 都還沒搜**）。
`localPin` 是 CVE-2019-19824 點名的參數，但那是**命令注入**不是溢位 ——
而三天前 `CVE-2023-34435` 就是「以為是自己的發現」的那個形狀。
</details>

<details>
<summary><strong>Q. 你新寫的 `formtable-scan.py` 說六個 build 的分派表是這樣。憑什麼相信它？</strong></summary>

**在一個 build 上憑對照，在另外五個上憑不上。** `--compare` 在 `unit-2018` 上與
Ghidra 逐項相同：57 對 57，兩張表位址一致，兩個方向零差異。**另外五個沒有 Ghidra
的報告可以比**，所以那五個的數字是「這支工具說的」。

唯一的間接證據是子集關係本身：一個會截斷表的掃描器會產生散落的缺項，
不會產生一個乾淨的子集。**那不是證明**，而這句話跟結論寫在一起
（`PROGRESS.md` W07 Day 4、`runsheet.md` `A1.5`）。

它也有一個會硬失敗的控制：`--expect formLogin` 找不到就 exit 2 ——
因為一個因為 stride 猜錯而回報「乾淨的空答案」的掃描器，
跟一支真的沒有分派表的 binary 長得一模一樣，而這個 repo 出過兩次那種東西。
</details>

<details>
<summary><strong>Q. 你改了一條凍結過的反證條件。那個凍結還有意義嗎？</strong></summary>

**有，而且改它正是凍結在運作。** `P4-9` 的反證條件點名「`P4-3` 的已知崩潰」當正
對照，而 `P4-3` 在這台早就判 `refuted`（`formNtp` 把 `submit-url` 回顯進
`Location`，800 bytes 回 799，任何長度都不崩潰）。**那個對照不存在**，
所以條件的第二個子句恆為真，整條退化成「零崩潰就代表 harness 壞了」——
一個會把正確的否定結果讀成儀器故障的條件。

改動在**送出任何請求之前**做，`[freeze].sha256` 在**同一個 commit** 裡改，
`git diff` 會把它顯示成兩行刻意的修改，理由寫在 `PROGRESS.md § Corrections`。
**凍結從來不是「不能改」，是「不能悄悄改」。**
</details>

<details>
<summary><strong>Q. 作業單這件事你們搞了三週才發現？那前兩週的紀錄可信嗎？</strong></summary>

**前兩週的紀錄可信，前兩週的作業單不是在場次之前寫的。** 這兩句話都要說。

W05 與 W06 的每一列都有一節走得到 —— 那是舊規則做到的事，而它只在**結果出現之後**
才要求程序。所以那些步驟是**回填**的：它們忠實地描述了發生過什麼
（`BENCH-LOG.md` 是逐字的，這一點沒有動搖），但它們**沒有在動手之前存在過**。

差別在哪裡今天有一個很具體的例子：`P2-11`（601 秒視窗）的計畫上寫著
「這一項連程序都還不存在，而且不能在進站之後補寫」—— **而沒有任何工具說得出這句
話**。現在說得出了。**回填一份作業單和照著一份作業單做，是兩份不同的文件。**
</details>

## 2026-08-18（W07 Day 5）—— 一個敵意的讀者今天會問的七題

<details>
<summary><strong>Q. 你昨天寫了一整節在講「$pc 完全可控」，今天才去搜有沒有 CVE，結果第一頁就有。這種順序算研究嗎？</strong></summary>

**不算，而且真正的答案比這個問題預設的更難看：那個 CVE 早就在這個 repo 裡。**

`notes/prior-art.md` 從 W04 就有這一列：

```
| CVE-2025-4462 | /boafrm/formWsc → localPin | buffer overflow | §1 — the same line of code as 3987 |
```

`notes/cve-status.md` 同一條標 🟥，而且寫著 **"The same line of source as 3987,
and identical in the 2015 image."**

所以事情不是「搜晚了」，是**寫發現的時候沒有打開自己的登記簿**，然後隔天跑去網路上
問一個自己檔案裡已經寫著答案的問題。**網路同意了我的答案——那是最沒有價值的一種
「說對了」。**

而且同一天犯了兩次：開放題 #67（`formSysCmd` 是被加進去的嗎）的答案，
`notes/formSysCmd-analysis.md` 從 W04 就寫著 Pierre Kim 的公告與「這是那個修補
本身」的推論，連反證方式都寫好了。

**唯一能辯護、而且是真的辯護的一點**：`cve-status.md` 那句 "identical in the
2015 image" 是一個**靜態預測**，而今天在那個映像上量到了 `ra = 513`、`s0`–`s6`
依序、`s7` 未動。**一個被證實的靜態預測比一次「重新發現 CVE」值錢**，那才是昨天
應該寫下來的東西。

規則已經改了，而且是改在流程上不是改在提醒上：`docs/disclosure.md` 的程序多了一個
**第 0 步——先開 `notes/prior-art.md`，再開 `notes/cve-status.md`，最後才是外部
來源**，而且一個只是跟登記簿一致的網路結果不算發現，算檢查。
</details>

<details>
<summary><strong>Q. 那你今天量的東西還剩下什麼？</strong></summary>

三件，而且要分開講（第 1 件在上一題修正之後意義變了：它不是「還剩下」，
它是**證實了 repo 自己的靜態預測**）：

1. **`localPin` 的偏移**：`unit-2018` 是 509，`v2.1.2` 是 513，`s0`–`s6` 依序往外，
   `s7` 兩邊都沒被動到。**四條搜尋路徑都沒有找到任何地方公開過這兩個數字。**
   這是驗證工作，不是發現，而它有用的地方在於：它證明這個缺陷跨越 2015、2017 兩個
   沒有被點名的 build。
2. **參數缺席那一半（`(A)`）**：所有公開的條目——`CVE-2021-35395`、Talos 那一系列
   ——講的都是「值太長」。**「值不存在」讓 accessor 回傳一個 `.rodata` 字面量的位址、
   然後被寫進去**，四條路徑都沒搜到。這是一個負面搜尋結果，不是新穎性的主張，
   而它就是照這樣寫的。
3. **廠商原始碼那一行**。見下一題。
</details>

<details>
<summary><strong>Q. 你說「廠商原始碼證實了」。那不就是承認你前面全部是猜的嗎？</strong></summary>

**不是，而且方向相反：那是第三個獨立來源，而前兩個各自都成立過。**

前兩個是 Ghidra 的反編譯（`0x445950` 那七行指令，逐指令確認過）與模擬環境裡的
實際故障（`pc = 0x2b32721c`，`strcpy` 內圈，store 目標 `0x004725d0` 落在 `R-X` 段）。
第三個是 `rtl819x/users/boa/src/apform.h` 裡的

```c
#define OK_MSG(url) { needReboot = 1; if(strlen(url) == 0) strcpy(url,"/wizard.htm"); ...
```

三個來源互相不共用任何一段程式碼或任何一個工具。**一致才有意義，而且它們一致。**

原始碼多給的是兩件反編譯給不了的東西：**這是一個巨集**，所以缺陷在展開它的每一個
handler 上，不是逐個 handler 的失誤；而它的兄弟 `ERR_MSG(msg)` 根本不碰 `url`，
**所以「哪 42 個活著」等於「哪 42 個走錯誤路徑」**——那是開放題 #65 的答案，
而且不用再量一次。
</details>

<details>
<summary><strong>Q. 你的模擬環境把你自己的電腦關機了三次，還好意思說量測可信？</strong></summary>

**那三次關機正是量測可信的理由之一，只是代價付得很難看。**

先講事實：`chroot` 不是隔離。guest 走到 `system("reboot -f")`，`busybox` 的
`reboot -f` 是裸的 `reboot(2)`，`qemu-user` 直接交給宿主核心，而整套工具用 root 跑，
中間沒有任何 namespace。**這不是模擬器的 bug，這是我的環境設計錯誤。**

要辯護的部分：找出來的方式沒有靠猜。記憶體全程 7 GB 以上、行程數一秒內從 68 掉到
36——那是有序關機不是 OOM；接著把同一發關進 `unshare --pid --fork` 並開
`qemu-mips-static -strace`，讓 guest 自己把那行 `execve` 印出來；再跑六格矩陣
（兩個 build × 三種請求）確認是那一發造成的、不是計時器。**每一步都有對照，而且
宿主的開機時間在六格全程沒有變過。**

不能辯護的部分：這個環境從 W06 就在用 root 跑 chroot，這個風險一直在那裡，
而發現它的方式是踩到，不是想到。它現在是 `RUNBOOK` §8.12.25 的一整段。
</details>

<details>
<summary><strong>Q. 「reap 壞掉」聽起來像小事。你為什麼把它寫成一整節？</strong></summary>

**因為它擋掉的是昨天唯一一個沒跑成的量測，而且它跟另一個缺陷組成一個閉環。**

`reset` 因為別的 profile 有殘留行程而拒絕——正確。它印出的修復指令是
`--profile 2018`，而 profile 叫 `unit-2018`；那個字串是從**目錄名**推出來的。
打成正確的 `--profile unit-2018` 之後，`reap` exit 1、不印任何東西、什麼都沒殺。
於是 `reset` 再拒絕一次。**兩邊都沒有說謊，兩邊也都沒有幫上忙。**

值得寫下來的是它為什麼藏得住：`env_pids` 的最後一句是 `[ ... ] && printf`，
在最後一個 `/proc` 項目上為假，回傳 1，`set -e` 就在賦值那一行把整支腳本結束掉。
而 `cmd_reset` 呼叫它的寫法是 `cmd_reap … || true`——**`||` 串列會關掉 `set -e`**，
所以它在唯一一個丟掉結果的地方能動。更狠的是：非 root 時 `readlink` 讀不到
`/proc/*/root`，迴圈最後一圈走 `continue`（status 0），**缺陷根本不出現**。
而 `reap` 本來就只能 root 跑，CI 不是 root。

**我第一版的守衛測試也沒抓到它**，因為我把它寫成 `if ( set -e; … ); then`，
而 `if` 的條件裡 `set -e` 是關掉的——**測試重現了藏住這個 bug 的那個機制**。
現在四個突變體，每一個都被為它寫的那一案殺掉。
</details>

<details>
<summary><strong>Q. `A1.9` 是你們昨天才加的檢查器逼出來的步驟。今天一跑就錯兩次，那個檢查器有用嗎？</strong></summary>

**有用，而且它的範圍今天才被量清楚。**

昨天那條規則抓的是「活著的列沒有步驟」，它抓到了 47 列，全部補上。今天第一次真的去
**執行**其中一節，發現它錯兩次：沒有帶 `--alignfix`（`flash set` 印出 `Bus error`
之後不會結束，看起來像慢不像壞），以及叫人拿壓縮後的 flash 位移去對解壓後的欄位
位移——**第一次跑出來差 2 bytes，看起來「差不多對」**。

所以正確的說法是：**寫在場次之前是必要的，不是充分的。**一個沒被執行過的步驟是一個
關於指令的預測，不是指令。這一條沒有變成新的檢查器（我不知道怎麼自動判斷一節「跑過
沒有」而不作假），它變成 `tools/config-diff.py`——**把那一節整個換成一支會拒絕的工具**，
而它在兩個方向都會拒絕：解出來零個欄位變動，或者變動不只一個。
</details>

<details>
<summary><strong>Q. 你在 v2.1.2 上換掉了對照組。這不就是「量不出來就換條件」嗎？</strong></summary>

**差別在於換的理由是被量出來的，而且換之前那兩個對照組怎麼壞的都寫下來了。**

`formNtp:` 在 `unit-2018` 上是對照組，因為 W06 在**實機**上量過它活著。搬到 v2.1.2
之後它 SIGSEGV——它在那個 build 上就是「參數缺席就死」的七個之一。
`formWsc:localPin=1234` 更麻煩：它**沒有崩潰，而是讓 guest 重開機**，而一個重開機的
案例在報表上長得像 `no signal`，也就是長得像一個合格的對照組。

換上去的是 `formSelLang:`，它在兩個 build 上都活著，而且是被量出來的、不是選出來的。
**這是這一節本來就在講的那句話往上一層**：一個結論繼承產生它的那份涵蓋率，
而「build」也是涵蓋率的一部分。第一輪的報表 `control_problems` 非空，
`tools/check-reports.py` 會拒收它——那一輪的資料留著，但它不算結果。
</details>

## 2026-08-19（W07 收尾）—— 一個敵意的讀者今天會問的七題

<details>
<summary><strong>Q. 你在按下不可逆的按鈕之前改掉了自己的預測。那不就是先看答案再下注嗎？</strong></summary>

**這是今天最該被問的一題，而它的答案取決於一件可以查證的事：改的時間點。**

改預測的 commit 是 `b88b932`，時間 `2026-08-18 23:09:10 +0800`。按鈕是作者在
`2026-08-19 00:0x` 按的。**改在前，按在後，而且中間隔著一次斷電重開。**
`test-cases.toml` 裡有 `amended = "2026-08-18"` 和 `amend_reason` 兩個欄位，
freeze 雜湊從 `ef7ab66d…` 變成 `ea8cf733…`，兩行改動在同一個 commit 的 diff 裡。

但「時間對」不足以回答這一題，因為真正的問題是**為什麼要改**。理由是舊預測
**今天量不到東西**：舊預測說「reset 會把 `COMPCS` 覆寫回 `COMPDS`」，而桌面上
解出來的結果是那兩個區今天已經差 0 / 343。按下去之後量到「兩個區一樣」，
分不出「reset 有作用」和「reset 什麼都沒做」。**一個不能分辨兩種世界的測試不是測試。**

新預測不是把舊預測往安全的方向調，它比舊的更容易被打死：它說三段而且三段都要對，
包含一段舊預測沒有講的（`eth1` 會回到 `MTU:1500` 並送得出 DISCOVER）。
而它的依據寫在改預測的那一刻就落地了——`/bin/reload` 輪詢 `/proc/load_default`
執行 `flash default-sw`，`/bin/flash` 的 usage 把「from hard code」和
「current setting to default」分成兩個命令。**那是一個可以錯的機制敘述，不是一個
方向。**

**如果要說這件事哪裡真的不好看**：舊預測是 W05 寫的，而它在 W05 就已經是錯的——
`COMPDS` 那時就被寫壞了，只是沒有人去解它。所以能辯護的是「發現得晚，但發現在
按鈕之前」，不是「一直都對」。
</details>

<details>
<summary><strong>Q. 你自己的未認證 POST 把這台的出廠預設區寫壞了，然後你說「還好按鈕不讀那個區」。那不是運氣嗎？</strong></summary>

**是運氣，而且我不打算把它講成別的。**

如果 `flash default-sw` 讀的是 flash 上的 `DEFAULT_SETTING` 區，這台今天晚上
就是一塊救不回來的磚——這個 repo 沒有燒錄器（`docs/lab-inventory.md` 第 2 項，
`P9-5`/`P9-6`/`P9-7`/`P9-11` 四項全部卡在那上面），而 `A2.5` 的 `FLW` 寫回路徑
需要 bootloader console，那正是當時沒有的東西。

**能講成方法的只有一件事**：那個「如果」在按下去之前就被寫成反證條件（a），
連後果都寫完了——「`P8-19` 要從跨越所有重開機升級成跨越原廠重置，而唯一的回頭路
是寫 flash」。所以兩個答案都是結果，而不是一個結果加一場災難。

**而真正該記住的教訓不是運氣**：一輪「參數缺席」的未認證 POST 掃描，寫壞了 25 個
出廠預設欄位，而 W05 那一格的結論是「改掉的欄位，沒有一個往危險的方向走」。
那句話當時看起來是對的，因為**沒有人去看出廠預設區**。
</details>

<details>
<summary><strong>Q. 「reset 之後 H601 沒變」——你比的是 `flash allhw` 的輸出，不是 bytes。那你憑什麼說沒變？</strong></summary>

**你說得對，而 `PROGRESS.md` 和 `weekly-results.md` 的「沒有證明什麼」那一節都寫著這一條。**

拿到的是解碼後的視圖：MAC、四組 WLAN 位址、整組 TX power 校準表，全部有值而且
`HW_NIC0_ADDR` 跟線上 ARP 回應、跟 flash `0x006000` 起的原始 bytes 三者一致。
**那是第二個來源，不是權威的那一個。** 權威的那一個是逐 byte 的 `cmp`，它要第 2 站
的 dump，而序列埠接上去板子就不開機（見下一題）。

**這件事有一個具體的損失**：如果 reset 動了 `H601` 裡某個 `flash allhw` 不列出的
byte，我現在看不見。而「前」那一份有七份、橫跨三天、byte 完全相同，所以那個比對
不會過期——下一場一份 `A2.3` 就補得回來。
</details>

<details>
<summary><strong>Q. 你在裝置上跑了一串命令、它就不開機了，然後你說「最可能是那一發 dd」。你連是哪一個命令都不知道，憑什麼寫進紀錄？</strong></summary>

**紀錄裡寫的正是「不知道是哪一個」，而那是刻意的。**

`BENCH-LOG.md` `T-67` 逐字列了八個命令，然後寫「**最可能是我造成的，而我沒有辦法
指認是八個命令裡的哪一個**」，並且把最強的嫌疑（`dd if=/dev/mtdblock0 bs=1 …
count=7510`，57,000 次單 byte 讀）標成**假設不是量測**。

要把它變成量測，需要在一台可以犧牲的裝置上一個一個重放——這個專案只有一台。
所以它留在開放題裡，而不是留在結論裡。

**而真正的結果不是那個假設，是恢復的過程**：三個實體測試的第一個就中了——
把 UART 轉接器從排針上拔掉，它立刻開機。**USB 轉序列埠會讓這塊板子起不來**，
而那件事沒有任何文件寫過，作業單只寫了一半（「pin 1 的 `VCC` 不要接」）。
它讓三次斷電重開量到零、花掉四十分鐘，而且**跟一塊真的磚頭長得一模一樣**。
</details>

<details>
<summary><strong>Q. 你說 `32.49.0.49` 是「字串被當成 IP」。那會不會只是你的假 DHCP server 送了畸形封包？</strong></summary>

**這一題有對照組，而對照組是在不知道會用到它的時候做的。**

上一場（2026-08-18）在同一個網段、同一台裝置、同一支 `udhcpc`，**沒有送任何路由
選項**，ACK 之後的三發免費 ARP 宣告的是 `192.168.77.100`——裝置自己的租約位址，
間隔一秒。今晚同一段程式、同樣三發、距 ACK 同樣的偏移（+13.4 / +14.4 / +15.4 秒
對上 +14.1 / +15.0 / +16.0），宣告的變成 `32.49.0.49`。**唯一的變數是那三個路由
選項。**

而 `32.49.0.49` 的四個 byte 是 `0x20 0x31 0x00 0x31`，正好是路由選項字串化之後
`…/16` 和 `192.168…` 中間那個「空白接 1」——**不是隨機的垃圾，是有位置的垃圾。**

**沒有排除的**：我的封包確實不是一個正常 DHCP server 會送的東西（三個路由選項
一起送）。要把「畸形觸發」和「解析錯誤」分開，要分三次單獨送 33、121、249——
那是開放題 #77，而且工具已經在了，缺的是一個「只送其中一個」的旗標。
</details>

<details>
<summary><strong>Q. `eth1.bound` 沒加引號——你為什麼不直接說這是命令注入？那樣不是更有衝擊力嗎？</strong></summary>

**因為它不是，而寫成命令注入會在第一個懂 shell 的讀者面前垮掉。**

那一行是 `sysconf conn dhcp $interface $ip $subnet $router $dns`。POSIX `sh`
**不會**把變數展開的結果重新解析成命令——`;`、`|`、`` ` `` 在展開之後都是普通字元。
會發生的是**分詞**：如果 `$dns` 含空白，`sysconf` 收到的 argv 就多一格、位置整排位移。

所以正確的說法是**參數注入**。而它並非因此就不重要：option 6 本來就可以帶多個
DNS 位址，`udhcpc` 會把它們串成空白分隔的字串，所以**攻擊者不需要任何技巧就能
改變 `sysconf` 的參數個數**。今晚那三發垃圾 ARP 就是那個形狀的一個可見後果。

**`P8-19` 的反證條件寫的是「`eth1.bound` 對所有 DHCP 提供的值都加了引號 → 這條
收掉」。** 它沒有觸發，而該收的是「命令注入」這個用詞，不是這條發現。
</details>

<details>
<summary><strong>Q. 你花了一整個 session 修工具，關掉的登記簿列只有兩列。這叫進度嗎？</strong></summary>

**如果只看列數，這是這一週最差的一天。如果看那兩列讓什麼變成可說的，不是。**

`P9-9` 那一列的價值不在「reset 會還原設定」——那是規格書。價值在
**它是在一台被本專案自己弄壞、而且連出廠預設區都被弄壞的機器上量的**，
所以它回答的是「廠商自己的復原路徑，在這種狀態下還有沒有用」。
那個問題只有在那個狀態下才問得出來，而那個狀態是昨天造成的、今天才被看懂的。

而修工具那一半修掉的是**兩個會讓既有結論失效的東西**：

- 開放題 #72：一份已提交的報告把 GOT 槽當成變數、把六次取址報成六次讀取，
  而 `P2-11` 的整段推理站在那上面。修好之後那個寫入在 `0x0044f140`，
  離註記自己寫過的那一行只有 8 個 byte。
- 開放題 #73：四場進站沒有一場問過「這台還能不能路由」，而它已經斷了兩天。
  現在 `make doctor` 會問，而且今晚第一次跑就抓到了。

**兩者都不會出現在登記簿的列數上，而兩者都會出現在「你憑什麼相信前面那些結果」
這個問題的答案裡。**
</details>
...
## 2026-08-19（W07 最終場）—— 一個敵意的讀者今天會問的六題

<details>
<summary>展開</summary>

### Q23.1 你昨天說 `P5-2` 是「刻意不做的」，今天說它做完了。到底哪一句是真的？

**兩句都是真的，而中間那個差別正是這一題值得問的地方。**

昨天的判斷是：`ret2libc` 要知道 libc 載到哪，而這台沒有 shell、沒有 gdbserver、
`epc` 只在崩潰時才印一次 —— **沒有觀測通道。** 那句話到今天仍然成立。

錯的是我從它推出來的結論。「沒有觀測通道」講的是**未來還能不能再量一次**；
它不蘊含**過去沒有量到過**。`BENCH-LOG.md` 裡有兩行昨天為了 `P6-2` 與 `P5-6`
抄下來的 `do_page_fault`，兩行都印著 `epc ==`。

**所以正確的說法是：昨天的推理是對的，而它回答的不是這一列在問的問題。**

### Q23.2 你憑什麼說那個 `epc` 落在 `strcpy` 裡？你不就是拿已經知道的答案去挑符號嗎？

**這正是我把它做成漏斗、而且把每一層的存活數印進報告的原因。**

663 個動態符號 → 22 個能給出頁對齊的 base → 5 個在 `epc` 或它的延遲槽上放了一條
**store**（kernel 說是 write 錯誤）→ **1 個**跟 qemu 的**指令對**相符。

前三層全部是這個檔案自己的性質。**只有最後一層不是** —— 它是另一台主機上、
另一個執行器對**同一個輸入**印出來的東西。而那一層之所以能收斂到 1，
是因為它比對的不是「有沒有 store」，是 `bnez R` 接 `sb R,0(reg)` 這個**組合**，
連來源暫存器都要同一個。

**如果我省略前三層只寫「答案是 strcpy」，你這一題我就答不了。**

### Q23.3 那個「兩個行程的 base 差剛好等於 libapmib 的跨距」，會不會只是湊巧？

**會，而且機率是 36 分之 1，這個數字在報告裡。**

我掃了預測值周圍那 1 MB 的全部 256 個頁對齊 base，數有幾個能**同時**把 `epc`
和 `ra` 放進**同一個**函式：**7 個**。所以這條預測撐過的是一個大約 1/36 的過濾器，
不是 1/256，更不是萬中選一。

**剩下的重量由錯誤的「種類」承擔**：kernel 說那是 `invalid read access from
4187c8bc` —— 一個野指標讀取。那 7 個裡面，預測值指到的是 `free()`，
而它跟隔壁一頁的 `malloc()` 正是堆積損毀被讀出來的地方。

**我沒有辦法把它推到 1，所以我把它寫成 7 分之 1 加一個型態論證，而不是寫成確定。**

### Q23.4 兩行 console 都來自同一次開機。那你憑什麼講 ASLR？

**憑 ASLR 是 per-`execve` 的，不是 per-boot 的** —— `randomize_va_space` 是在
`load_elf_binary` 裡被查的。同一次開機起來的兩個行程，如果它是開的，版面就會各自
隨機。我有兩個獨立的 `execve`，而它們的版面**完全由 ELF 檔案決定**。

**但登記簿的反證條件寫的是「兩次重開機」，而我沒有跑那個實驗，所以這一列記
`partial` 不記 `confirmed`。** 把一個不可能觸發的反證條件記成成立，正是
`A3.24` 前天被抓到的事（拿抹除區比抹除區）。同一週學到的東西，不會因為這次
輪到我自己就換一套標準。

### Q23.5 你為什麼不順手把那個寫壞的反證條件改掉？你們不是有 `amended` 這個欄位嗎？

**因為它要改的證據，昨天就已經存在了。**

`P9-9` 用過 `amended`，而它合法的原因是：改在按鈕**之前**，freeze 雜湊在同一個
commit 一起動，commit 時間戳早於量測。那個順序是整件事的全部。

這一列不一樣。那兩行 `epc` 是 2026-08-18 抄下來的。今天去改反證條件，
就算改得更正確，也是**在證據存在之後改**，而 freeze 存在的理由就是擋這個。

**所以我把它照原樣記分，然後在結果的註記裡說它問錯了軸。** 一個被回報為
不充分的條件，比一個被偷偷改好的條件誠實。

### Q23.6 六份檔案寫錯了一個埠的狀態，這種東西不是校對問題嗎？值得寫進週報？

**如果它是校對問題，那它就不會發生六次。**

那六句話沒有一句是抄錯的。`P1-2` 2026-08-16 量到 52869 開著，這是真的；
`P6-1` / `P8-7` 2026-08-18 量到它關著，這也是真的。**兩次量測之間，這個專案
自己的一發未認證 POST 把 `UPNP_ENABLED` 寫成 0**，而這個 build 沒有任何一頁
UI 可以把它調回來。

問題出在**沒有一句話帶時間**。而這個 repo 的規矩「一份狀態一個擁有者」看不見
這一種 —— 六份檔案沒有一份是在重複別人的狀態，它們只是忘了說那是什麼時候的事。

**而第二層比第一層嚴重**：昨晚的 reset 把旗標還原了，所以那六句話很可能又變成
真的了 —— **而沒有人量過**。一個碰巧又成立的宣稱，跟一個被重新驗證過的宣稱，
在文件上完全一樣。所以 `docs/disclosure.md` 的 `D-16` 現在多一句：
**在下一次進站重量之前，它的網路狀態不可通報。**

被通報的開放埠，必須是有人真的去看過的埠。
...
## 2026-08-19（W07 最終場 · 進站）—— 一個敵意的讀者今天會問的五題

<details>
<summary>展開</summary>

### Q24.1 你早上在桌面算出一個位址，晚上量到一樣，然後說這證明了什麼？循環論證吧？

**如果只有 `boa` 那一個數字，你這題我答不了。**

`boa` 的 `0x2aae3000` 是從一行 kernel 錯誤訊息**反推**出來的 —— 它跟它自己的輸入
相符，那不是證據，那是算術沒錯而已。

**能答你的是第二個數字。** `wscd` 的 `0x2aabe000` **沒有任何觀測支撐**：
它是從 `libapmib.so` 的 program header 算出來的跨距 `0x25000`，加上一個假設
（載入器由低往高配置、沒有隨機化）。那個數字在被量到之前就寫在
`notes/mips-ret2libc.md` 和 `reports/libbase-unit-2018.json` 裡。

**一個計算跟它自己的輸入相符，跟一個計算預測到一個獨立的量測，是兩件事。**
而且我把它可能矇到的機率也量了：掃過周圍 1 MB 的 256 個頁對齊 base，
有 7 個能同時把 `epc` 和 `ra` 放進同一個函式 —— 所以是 1/36，不是萬中選一。
那個數字也在報告裡。

### Q24.2 `/proc/sys/kernel/randomize_va_space` 是 2。你憑什麼說沒有 ASLR？你在跟核心吵架

**是，而且吵贏的是量測。**

那個 sysctl 在 `kernel/sysctl.c` 裡，是**架構無關**的程式碼；它會不會被實作，
是各架構自己的事。Linux 2.6.30.9 的 MIPS 沒有 `arch_pick_mmap_layout` 的隨機化版本。

而不管理由是什麼，事實在 `/proc/*/maps` 裡：**兩個行程、四次開機，版面完全由
ELF 檔案決定**，`libapmib.so` 佔的正好是 `2aabe000 → 2aae3000`。

**這一題我想反問**：如果我只讀了那個旗標，我會把 `P5-2` 判成 refuted，
writeup 上會寫「這台有 ASLR」，**還附一個 `/proc` 的引用**。
那句話會是錯的，而且比一句沒有引用的錯話更難被抓到。

**一個來源不是一個量測。** 旗標是有人講的話。

### Q24.3 你打掉了 UPnP daemon 三次，然後說那是一個發現。那不就是你亂送東西嗎？

**「亂送」和「發現」的差別是對照組，而這一題的對照組有三個點。**

- `NewInternalClient=10.1.1.1` → HTTP 200，**daemon 活著**，後面還能再讀一次。
- `NewInternalClient=` 反引號 payload（22 bytes）→ 連線關閉，**daemon 死**。
- `NewInternalClient=` 22 個 `A`，**一個 shell 元字元都沒有** → 連線關閉，**daemon 死**。

第三個點是整件事的重點。**沒有它，我會把這件事寫成「CVE-2014-8361 在這個版本上
崩潰而不是執行」** —— 那是一個很好聽、很像回事、而且錯的結論。
有了它，答案是「任何 `inet_addr()` 不接受的值」，跟注入無關。

而它的落點在裝置自己的防火牆裡看得到：`DNAT … to:255.255.255.255`，
那是 `INADDR_NONE` 被當成位址用。

### Q24.4 那你為什麼不通報？你不是說它是一發未認證的請求就打掉服務嗎？

**因為我還沒有做先驗檢索，而「沒有人發表過」是一個需要檢索撐著的宣稱。**

`docs/disclosure.md` 的 `D-19` 現在的狀態就寫著 **NOT SEARCHED**，
而且寫了檢索要怎麼做才算數：要**按行為**檢索而不是按 binary 名字 ——
因為這台的 `miniigd` 自報 `Server: miniupnpd/1.4`，那是另一個 codebase 的名字，
按名字搜會搜到完全不同的東西。`D-17` 就是踩過這一格才學會的。

**還有兩件沒量的，而它們都會改變嚴重度**：
它為什麼會死（路徑上那個無界 `strcpy` 是候選，但 22 bytes 對它來說太短了），
以及 WAN 側打不打得到（`P8-7` 的後半沒有答案）。

**一個沒有嚴重度的通報，比不通報傷害更大。**

### Q24.5 `MINIUPNPD` 那條 chain 是 `(0 references)`，那不就代表映射根本不通嗎？你怎麼不寫

**因為那一場 WAN 線沒有接，而 `ip_forward` 是 `0`。**

「一台沒有 WAN 的路由器不轉送」不是關於「一台有 WAN 的路由器」的證據。
那個讀法是可用的、而且比較好聽 —— 它會讓 `P8-7` 變成一個乾淨的「不可利用」，
一列就收掉。

**我沒有拿。** `P8-7` 記成 `partial`，而登記簿自己的第二條反證支
（「映射建起來了但 WAN 側打不通 → iptables 沒有跟著開，映射與轉發是兩件事」）
現在是開放題 88，要線在 WAN 埠才判得了。

**一個在錯的前提下成立的結論，跟一個沒有結論，前者比較危險。**

</details>

## 2026-08-20 – 21（W08 Day 0–1）—— 一個敵意的讀者今天會問的七題

<details>
<summary>展開</summary>

> **先記一件事：2026-08-20 那一場沒有寫這一節。** 那天關掉了 `chipName: UNKNOWN`、
> 改寫了 `P9-6` 的預測、砍了八列又收回兩列，全部沒有進 QA。這個檔案的規矩是
> 每一場都寫，而漏掉的方式一如往常 —— 那天忙、而且結果都很順。**結果順的那一天
> 正是最需要被人問的那一天。** 這一節補回來，兩天一起。

### Q25.1 你說「半份設定從來沒被解過」。那你之前那份 `compcs-decode.md` 上面那五個檢查，是幹嘛用的？

**它們全部在檢查同一件事，而那件事不是我以為的那件。**

那五個檢查是：廠商自己的 8-bit 校驗和、環形填充交叉比對、344 個 TLV 對 344 筆 MIB
記錄、零個不認識的 id、尾巴剛好 1 byte。**它們全部是關於「串流」的**：這一串 byte
是不是一串格式正確的 TLV。

**沒有任何一個在問「某一筆的『值』有沒有被看懂」** —— 因為一筆沒解開的值**仍然是
一個格式完整的 TLV**。它有 id、有長度、長度也對得上，五個檢查一個都不會叫。

所以正確的講法不是「檢查沒做好」，是**檢查的對象選錯了**：一份半讀的區域能全過的
檢查清單，量的是別的東西。而寫下「bit 15 代表 table-valued」那句話的人，就是沒有
去跟著那句話走的人。

現在多了一個會叫的：`table_entries_decoded` 必須等於 `table_entries`，
而 `check-reports.py` 會擋下不相等的報告。**它跟前五個的差別是它問的是覆蓋率，
不是格式。**

### Q25.2 `"runner_up": 133` 從 W04 就印在報告上。你要說「工具早就找到了」，那不就等於承認你四個月沒看自己的報告？

**是，而且我不想把它講得比這更漂亮。**

不過有一個區別值得畫出來，因為它決定下一次怎麼避免：**那不是一個沒被讀到的數字，
那是一個被讀到、而且被正確理解的數字。** `runner_up` 存在的理由是防歧義 ——
「最長那段有 344 筆，第二長的有 133 筆，133 × 2 < 344，所以第一名不含糊」。
那個判斷是**對的**，那個檢查也**不該**開火。

**缺陷不在檢查，在問題。** 我問的是「哪一段是那張表」，而不是「有哪些段」。
一個沒有人提出的問題，沒有任何檢查抓得到。

這跟這個 repo 前面 46 個儀器 bug 都不同種：沒有東西壞掉、沒有兩個來源不一致、
沒有一個荒謬到不可能的數字（sink 普查那個 589 對 1）。**它是一個對太窄的問題所
給的正確答案**，而唯一能揭發它的動作是回頭去看一個從來沒被讀過的欄位。

### Q25.3 你憑什麼說 `WLAN_ROOT` 是「六塊」？六是你從資料裡數出來的，然後你再拿它去驗資料，這不是循環嗎？

**六是從 binary 讀的，不是從資料數的，而這個區別就是這一節存在的全部理由。**

`libapmib.so` 的 60-byte 記錄裡，第 44 個 byte 起是 `total_size = 15156`，
第 50 個 byte 起是 `element_size = 2526`。15156 / 2526 = **6**。
這兩個數字在**檔案的另一個地方**，跟 flash 上那 22,044 個 byte 沒有任何關係。

然後兩邊要對得上，而且對法是可以失敗的：

```
一塊 = 2526（結構）+ 133×4（自己的標頭）+ 616（巢狀表的標頭）= 3674
6 × 3674 = 22044      餘數 0
```

左邊每一個數字來自 `libapmib.so`，右邊那個 22044 來自 flash。
**如果我把六猜錯，餘數不會是零。**

而且它真的差點沒對上：第一版只算頂層標頭，`WLAN_ROOT` 少了 **3,696** —— 那正好是
6 × 154 × 4。**差額自己說出了原因**，而那是因為我拿一個外部的數字去比，
不是因為「走訪剛好用完了緩衝區」。走訪用完自己的輸入，是走訪的性質，不是格式的。

### Q25.4 「出廠是開放網路」——你只有一台，而且你只讀了 flash。你憑什麼講「出廠」？

**這一句要拆成三段，而只有前兩段站得住。**

**站得住的**：`COMPDS` 是出廠預設區，而 2026-08-19 已經在這台上量過
`flash default-sw` 會把它從**硬編碼**重寫。所以那一區的內容等於這個 build 的
編譯期預設值，不是這台被用過之後的狀態。這個 build 的出廠預設是
`ENCRYPT=0`、PSK 全零。

**也站得住的**：這台**現在**跑的 `COMPCS` 逐欄等於 `COMPDS`，而且裝置自己的
`/bin/flash` 在 W07 印過一行 `ENCRYPT=0`。兩個來源，一靜一動。

**站不住的、而我沒有寫的**：「所有 N150RT 出廠都是開放網路」。
我有一台。這個 build 的映像這樣寫，但別的 build、別的地區版本、
以及 ODM 出貨時會不會另外燒一份 `COMPDS`，我一件都不知道。
`notes/wlan-root.md` 第 5 節的範圍段就是這樣寫的。

**而還有一段更弱的，我也標出來了**：這是**靜態**讀。到今天為止這個專案沒有收過
任何一個 802.11 frame。「出廠映像裡寫著開放」跟「那根天線現在在廣播一個開放網路」
是兩句話，第二句要 `P7-3` / `P7-4` 才有資格講。

### Q25.5 你把「兩張表都符合就拒絕」改成「兩張表不一致才拒絕」。這不就是為了讓自己過關而放寬規則嗎？

**這是這一整天最該被問的一題，因為放寬一條拒絕規則是最容易自肥的改動。**

事實：`libapmib.so` 裡真的有兩段 `PROFILE_SSID..PROFILE_PSK_FORMAT`，
在 `0xb130` 和 `0xb43c`，**十二個 id 一樣、名字一樣、大小一樣** ——
因為這個 build 真的有兩張 profile 表（`PROFILE_TBL1`、`PROFILE_TBL2`）。
兩個候選給出**完全相同**的答案。

原本那條規則會因此拒絕解碼。它拒絕的不是「我分不出來」，是「同一個字的兩種拼法」。

**但你的懷疑是對的，所以我做了兩件事讓它可以被檢查：**
一、新規則不是「隨便挑一個」，是**簽章必須唯一** —— `(id→名字對應, element_bytes)`
這個組合只能有一種。名字不同或大小不同，照樣拒絕。
二、加了一個測試 `test_two_disagreeing_sub_tables_are_refused`，
它造兩張 id 相同但名字不同的表，**驗那個拒絕真的會開火**。
放寬之後還能不能拒絕，不是我說了算。

### Q25.6 你昨天叫我買一片 RP2040，今天說不用買。那你昨天在幹嘛？

**我把一個「有就更好」的東西講成了「這很棒」，而且你連續兩次說感覺不需要，我才收回。**
順序是反的：我的工作是先講那個工具可能在哪裡騙人，不是先把它講漂亮。

**那個提案本身沒有錯**，它會直接答開放題 89 與 91 —— 被動側錄 SPI 匯流排，
看得到開機途中有沒有 `0x02`(page program) 或 `0x20`/`0xD8`(erase) 飛過去。

**但它在這一週的位置是錯的**，理由三個而且沒有一個是價錢：要在**通電**的板子上夾
（本專案對 CH341A 明令禁止的動作）、SOIC-8 夾只有一個所以兩支儀器不能同時掛、
而**同一個問題有一個成本為零的版本**——循環 3 再夾上去時先讀一份，跟寫進去的
影像比。同一支儀器兩個時間點，儀器差異這個混淆項不存在。

側錄仍然比較強（它看得到「寫了什麼指令」，`seat-c` 只看得到「寫完的結果」），
所以它退回開放題 89 而不是被刪掉。

### Q25.7 `H601` 你到現在都沒解過，而 `HW_WLAN0_WSC_PIN` 你早就在報告裡引用了。那個值是哪來的？

**是問裝置問來的，不是解出來的 —— 而這一題點到的正是今天新開的開放題 94。**

`13572468` 那個值有兩個來源，但**兩個都是裝置自己講的**：
`flash get HW_WLAN0_WSC_PIN`（模擬環境裡跑廠商自己的 binary），
以及 `config-attrib.sh` 對 `H601` 前後 64 KiB 的 byte 差異。
**沒有任何一支這個 repo 的工具，能把 `0x006000` 那 8 KiB 解成有名字的欄位。**
`fwrecon compcs` 對那個位移是直接拒絕的 —— 它不是 `COMPHS`，沒有壓縮，magic 是 `H601`。

今天之後這件事只差一步：`HW_WLAN_ROOT` 是 id `0x80c9`，
它的子表是 `0xccd8` 那段 18 筆 `WLAN_ADDR..LED_TYPE`，
而 `WSC_PIN`(`0x0111`) 在 `0xd458` 那段 6 筆裡。表都在手上了，缺的是 `H601` 自己的
標頭解析。

**而它是整份映像裡揭露上最敏感的一區**（每一台不同的 MAC、射頻校準、WPS PIN），
所以那份報告要先過 `docs/disclosure.md` 才能 commit，不是 commit 完再想。

> **順帶一個今天算出來、而且只有一個來源的東西**：出廠那個 PIN `99956042` 的
> 第八位**是**一個合法的 WPS 檢查碼；W06 的 PoC 寫進去的 `13572468` 算出來應該是
> `13572464`，**檢查碼是錯的，而裝置照樣存了下來**、自己重算了 `H601` 的區塊校驗和、
> 回 302。所以 `formWsc` 在入口不驗那一位。`wscd` 事後驗不驗，沒量過。
> 那個檢查碼是我今天用 Python 算的，**一個來源**，開放題 95。

</details>

## 2026-08-21（W08 Day 1 桌面第四場）—— 一個敵意的讀者今天會問的六題

<details>
<summary>展開</summary>

> **今天沒有量到任何東西，而這一節照樣要寫。** 今天做的是把一份自己寫的作業單拆開，
> 發現它會正確地跑完然後印出錯的答案。**這種日子最容易寫成「我很嚴謹」**，
> 而那正是下面六題要擋掉的東西。

### Q26.1 你說 `A2.7` 的實驗會得到錯的結論。但你從來沒跑過它。你憑什麼？

**憑 loader 自己的指令，而且這一題問到了真正的界線。**

我沒有跑過那個實驗，我也沒有主張「跑了會怎樣」——我主張的是**那張判讀表的兩格
沒有窮盡**。理由是 `0x80401ED4` 那段程式取的位址是 `[0x8040D3A8]`，而那兩次 `FLR`
的目的位址都是 `0x81000000`。`0x8040D3A8` 的初值 `0x80500000` 是直接讀 `.data`
第 `0xD3A8` 個 byte 讀出來的，不是推的。

**能被合理挑戰的地方有兩個，兩個我都沒有量過：**（a）`objdump` 有沒有把那段解錯；
（b）那個全域在 `IPCONFIG` 之後有沒有被別的路徑改掉 —— 我只找了寫入它的三個地方，
而那是同一支工具說的。**所以四格的預期雜湊全部寫在進站之前**：落不上任何一格，
被推翻的是我今天的讀法，不是那台機器。這一題正確的回答是「我寫了一個會讓自己出錯的
實驗，還沒跑」，不是「我證明了它會錯」。

### Q26.2 你自己寫的作業單，自己找出四個錯，然後自己修好。這不是自導自演嗎？

**部分是，而且值得說清楚哪一部分是。**

不是自導自演的那一部分：`FLR` 的參數順序有一個**獨立的第三來源**——loader 的
handler `0x804099AC` 逐個 `strtoul(argv[0..2])`，然後印
`Flash read from <argv[1]> to <argv[0]>`。RUNBOOK §8.7.8 的四份實錄早就這樣寫。
所以「作業單寫反了」不是我的判斷，是**兩份既有證據對同一句話的判決**。

是自導自演的那一部分：**這四個錯是同一個人在四小時前寫下的**。這不是嚴謹，
這是**一個沒有第二個人看的流程**在同一天內自己發現了自己。真正的問題是為什麼
`tools/check-runsheet.py` 抓不到 —— 而它的說明自己就寫著它不檢查語意。
**修法不是幫它加一條「檢查 `FLR` 的參數順序」的規則**（那只會擋住這一個實例），
是讓作業單不要有第二份。現在 `A2.7` 一個手打的 `FLR` 都沒有。

### Q26.3 「模擬器抓到分支位移算錯」——你自己寫編碼器又自己寫模擬器，兩邊都你寫的，這能算兩個來源嗎？

**不能，而這正是那支工具第一版真正的 bug。**

第一版的模擬器**跟編碼器共用常數**。守衛套件把 `UART_THR` 改掉，payload 跟模型一起
搬家，於是一個寫到錯位址的 `sb` 被模型高高興興收下——**一個跟著受測物走的模型不是
模型**。現在模擬器帶裝置的位址（從 loader 的 putchar 讀來的），編碼器帶自己的。

至於分支位移那個 bug：抓到它的是模擬器，**釘死正確答案的不是模擬器**，是 loader 自己
的兩個 `beqz`（`0x80406B7C` 的 `+7` 落在 `0x80406B9C`，`0x80406B90` 的 `-7` 落在
`0x80406B78`）。而 `objdump` 對同一份 bytes 解出來的分支目標與我的 label 逐條相符 ——
**那才是第二個來源，而且它不是我寫的。**

### Q26.4 `nfjrom` 那件事你寫了一大段，結果最後說「不是新發現」。那你寫它幹嘛？

**因為它改的是工具的行為，不是專案的成績。**

如果只看「這是不是一個發現」，答案是不是，而且我在 note 裡就是這樣寫的：`nfjrom`
是 Realtek 自己的 MP image 名稱，別的廠商的 GPL 釋出裡有 `nfjrom.script`，
連 `192.168.1.6` 這個預設值都在 OpenWrt 的 wiki 上。**register 是空的，所以搜尋走出去，
而它回來是滿的。**

它留下來的理由只有一個：`loader-tftp.py put` 的 `--filename` 預設值是 `image`，
而**如果哪天有人為了「像一點」把它改成 `boot.img`，`J` 由人來按這個設計就靜靜地消失了**。
現在工具會拒絕。**一個不是發現的東西，可以是一個必要的守衛**，而把兩者混在一起講
才是問題。

### Q26.5 你給 `put` 加了一個「rescue 紀錄不能太舊」的檢查。一小時是哪來的？你不是說不要編數字嗎？

**一小時是編的，而工具自己講了它是編的。**

真正要檢查的性質是「`AUTOBURN 0` 是**這一次開機**送的」，而我沒有辦法檢查那個 ——
loader 沒有給我任何一個每次開機會變的值可以拿來當 nonce（我找過：`0x8040DD20` 是
常數 2098，IP 是人給的，未初始化的 DRAM 不保證每次不同）。

所以工具做的是**把「任何時候寫的」換成「一小時內寫的」**，而它的 docstring 明講
這不等於同一次開機。**這是一個弱化的守衛，不是一個假裝很強的守衛**，而兩者的差別
就在於有沒有把話說出來。第一版的問題不是它弱，是它**在唯一要緊的那個面向上不可能
失敗**，同時看起來像在守著那件事。

（順帶：實務上 `IPCONFIG` 也是 RAM 狀態，所以斷電之後 TFTP 根本不會回應 ——
真正會咬人的情境是「這一次有跑 `A2.4`，但寫到另一個檔名，而 `put` 讀到舊的那份」。）

### Q26.6 你花一個晚上做一個印字串的 148 byte 程式。這不是為了讓一列變綠而做的表演嗎？

**如果 `P9-12` 的反證條件是今天寫的，這一題就成立。它是 2026-08-18 寫的，而且凍結了。**

那一列自己說：`J` 之後主控台沒有輸出，就分不出「跳過去了但沒東西講話」與「根本沒跳」，
那時只能記 `partial`，**而且要指名下一個能分辨兩者的量測**——它指名的就是
「讓上傳的映像第一件事往 UART 寫一個字元」。所以這 148 個 byte 不是為了讓一列變綠，
它是**那一列三天前就寫下的、唯一能讓它變綠的東西**。

而它今天縮水了一點，理由要說出來：**`J` 自己會先印 `---Jump to address=%X`**
（`0x8040B35C`），我今天才讀到。那一行本來就把「沒跳」跟「跳了但沒講話」分開了一半。
payload 仍然有用（那一行只證明 loader 打算跳，不證明目標執行了），但**如果我先讀到
那一行，我會先問「還需要 payload 嗎」再開始寫**，而不是反過來。

</details>

## 2026-08-21（W08 Day 1 桌面第五場）—— 一個敵意的讀者今天會問的七題

<details>
<summary>展開</summary>

> **今天的產出是一個「什麼都沒有」的結論** —— 第四個參數沒有被讀、沒有指令讀
> 那個欄位。**缺席類的主張最好寫也最難守**，因為讀者無法區分「它不在」與
> 「你沒找到」。下面七題有五題在問這件事。

### Q27.1 「沒有任何指令讀 offset 4」—— 你怎麼知道你找的方法找得到？

**這是今天唯一真正需要回答的問題，其他六題都是它的變形。**

方法是：MIPS 不會把一個 32-bit 常數放進一個指令，它用 `lui` + `addiu`／`ori` 組出來。
所以我掃全 image 找**組出 `0x8040DBC0` 的指令對**，找到兩個（`0x80409170`、
`0x80409AC4`），再從各自的下一個指令開始走 CFG，收集透過那個暫存器讀到的位移 ——
`{0, 8}` 與 `{12}`。

**能被合理挑戰的地方，我列三個，而其中兩個沒有被排除：**

（a）**指標可能是資料。** 我搜了整個 stage2 有沒有任何 word 等於表裡任一列的位址，
零個。這一個排除掉了。

（b）**指標可能是別的方式組出來的** —— 例如 `lui` 之後隔很遠才 `addiu`，
或者用另一個基底加位移。我的窗口是 16 個指令，超過就漏。**這一個沒有被排除**，
只被縮小。

（c）**這是靜態的。** 執行期沒有 MMU 重映射、沒有自我修改，是我讀出來的假設，
不是量出來的。

**所以正確的說法是「這份 image 裡沒有指令用我找得到的方式讀 offset 4」**，
而它旁邊有一個獨立的第二來源：廠商原始碼裡那段 `if (n_arg != argc-1)` 在 `#if 0` 裡。
**兩個來源同時錯的方式，我想不出來 —— 但「我想不出來」不是證明，它是我目前的界線。**

### Q27.2 你拿廠商原始碼當第二來源。那份原始碼不是這台的韌體，你憑什麼？

**憑它解釋了我在 binary 裡看到、但沒有辦法從 binary 解釋的東西 —— 而這是它唯一的用途。**

具體地說：binary 裡有兩個常數，`li a2,1` 與 `move a0,zero`。反組譯只能告訴我
「它們是常數」，不能告訴我**為什麼差一**。原始碼給的答案是 `printf(… cnt2+1 …)`
在一個硬寫的 `cnt2 = 0` 上面。

**這條推論的方向很重要，而它很容易被反過來用。** 我沒有用原始碼去主張這台的行為；
我用這台的 binary 主張行為，用原始碼解釋成因。如果兩者矛盾，binary 贏 ——
而今天真的矛盾了一次：那份 SDK 的 `#ifndef CONFIG_SPI_FLASH` 把 `FLR` 跟 `FLW`
包在一起，而這台**同時有 SPI 版的 `FLW` 與非 SPI 版的 `FLR`**。所以這台的來源樹
不是這一份，而我沒有因此去改任何一條關於這台的主張。

**能被合理挑戰的地方**：`cnt2 = 0` 這個解釋在較早的樹裡可能不是註解掉，而是真的讀
`argv[3]`。**那不影響結論**，因為結論是從 binary 來的 —— 但它會讓「為什麼」那一段
變成猜測。所以那一段在註記裡標的是「解釋」，不是「證據」。

### Q27.3 你自己寫的工具，自己種的合成映像，自己寫的斷言。這比手抄好在哪裡？

**好在它會失敗，而手抄不會。**

手抄那張表的失敗方式是：**抄對了值、寫錯了描述**，而沒有任何機制會發現 ——
`{name, help, argc, handler}` 與 `{name, argc, handler, help}` 在一張只列值的表格
底下讀起來一模一樣。

工具的失敗方式我逼它示範了七次：把 prologue 測試改成永遠成立、把計數欄位寫死成
`false`、把 argc 的消耗關掉、把算出來的索引報成常數、拿掉 caller-saved 清除、
不執行延遲槽、不追分支。**第一輪只死了四個。**

**而活下來的三個才是這一題真正的答案**：它們活下來不是因為工具對，是因為
**我種的合成 handler 裡沒有一個走那條路**。也就是說，我的守衛套件在那三個維度上
等於不存在，而我沒有察覺 —— 直到我去殺它。補了三種形狀之後七個全死。

**還能被合理挑戰的地方**：突變體是我挑的。我挑的是我想得到的錯誤，
**一個我想不到的錯誤仍然會活著**，而套件不會告訴我。

### Q27.4 「六個 handler 不檢查 argc」聽起來像一個發現。它是嗎？

**不是，而且我沒有把它寫成發現。**

它要主控台存取。有主控台存取的人可以直接 `FLW` 亂寫 flash，不需要繞過 argc 檢查。
**所以它的攻擊面價值是零。** 它在這份 repo 裡的價值只有一個：**`A2.5` 是我自己要跑
的一節，而我打字會錯。** 那個方框是給操作者的，不是給讀者的。

**這一題如果反過來問「那你為什麼寫這麼多字」** —— 因為六個裡有一個是唯一不可逆的
那一個，而我原本以為問題出在「少送一個參數」。**問題出在「少送兩個參數」，
而且結果不是拒絕，是解參考 NULL。** 方向完全相反。

### Q27.5 開放題 99 你把實驗換掉了。是不是因為舊的那個會給你不喜歡的答案？

**相反 —— 舊的那個會給我一個我會喜歡、但不該收下的答案。**

舊設計是「跳完之後 `get` 一次」。它幾乎一定會逾時，而我幾乎一定會寫
「`0x804092F4` 把 port 關掉了，證實」。**那是四個充分原因裡挑我最想要的那一個。**

新設計不跳轉，只清那五個 bit，中斷全程不動，然後把值寫回去看它復不復活。
**它可能給我一個不喜歡的答案**：清掉之後 `get` 照樣活。那時要改的是註記裡
「the five switch ports down」那一行。

**能被合理挑戰的地方**：新設計證明的是「那五個 bit 足以殺掉網路」，**不是**
「`J` 之後網路死掉是因為那五個 bit」。後者要靠步驟 4，而步驟 4 依賴 `P9-16` 成立。
`P9-16` 若被推翻，99 就只剩下前半，而我會照這樣寫。

### Q27.6 `P9-16` 你已經從指令讀出答案了。再花一次進站去驗，是不是浪費？

**如果它只是為了確認我讀對了，是浪費。它不是。**

`jalr` 這個字我讀得沒有疑義。有疑義的是**回程存不存在**：`J` 在跳之前遮了中斷、
清了 `IE`、做了 cache 維護，回來之後 `GetLine` 還收不收得到字元，
不是 `jalr` 那個 opcode 決定的。**「指令會回來」與「這台會回到提示字元」
是兩件事，而我只讀出了第一件。**

**而它的成本是負的**：八個 byte、一次 `J`。成立的話 `P9-10` 的每一次 payload
省一次電源循環；不成立的話我知道 `P9-10` 要當單程規劃 —— 而那個資訊更值錢。

### Q27.7 你今天沒有碰裝置，卻改了三份會被別人照著做的文件。憑什麼？

**憑那三份文件裡有一份原本會把人送進一個問不出答案的實驗。**

`runsheet.md` 是給操作者的，而它上一版在 `A2.5` 裡送三個參數、沒有解釋為什麼；
Part B 裡寫著「`J` 之後沒有軟體的路回去」，而那是一次觀察被寫成一條規則。
**兩件都不是量測錯誤，是文字的範圍超過了證據的範圍。**

**能被合理挑戰的地方，而且我承認它**：我今天寫進去的東西同樣還沒有被裝置檢驗過。
所以 `A2.8` 的表頭寫的是「2026-08-21 桌面上寫成，**尚未在機器上跑過**」，
三個新登記項的 `exit_evidence` 全部是 `unverified`，而 PROGRESS 那一則的最後一句是
「Nothing was sent to the device」。**這一段的正確讀法是：我把一個問題換成了一個
更好的問題，而不是我答了它。**

</details>

## 2026-08-22（W08 第 2 站進站之二）—— 一個敵意的讀者今天會問的六題

<details>
<summary>展開</summary>

> **這一場三項全中，而下面六題有四題在問那個沒中的第四格。**
> 一個全中的場次最危險的地方，是它讓人不想去看那個沒中的。

### Q28.1 你說「四格逐字相同」證明第四個參數沒被讀。那如果它被讀了、但這台只有一顆 flash，所以看不出差別呢？

**這一題問到了正確的位置，而答案是：那個差別看得出來，只是不在我測的地方。**

如果 `argv[3]` 被讀進晶片編號，四格的**訊息**會不同 —— 因為訊息裡那個
`SPI flash#%d` 就是從同一個變數印的（廠商原始碼 `printf(… cnt2+1 …)`）。
我測的正是那一行，而它一個字元都沒變。所以「被讀了但看不出來」要求
**印出來的那個值與傳給寫入函式的值來自不同的地方** —— 反組譯說它們是
`li a2,1` 與 `move a0,zero` 兩個常數，原始碼說它們是 `cnt2+1` 與 `cnt2`。

**能被合理挑戰的地方，我承認一個**：我沒有測「第四個參數是否影響寫入落點」，
因為這台只有一顆 flash，chip 1 上沒有東西可以讀回來。所以嚴格說我證明的是
**「印出來的晶片編號不隨第四個參數改變」**，而「寫入落點不隨它改變」
是從反組譯推的（`move a0,zero`），不是量的。

### Q28.2 `P9-15` 你自己清掉再自己寫回去，然後說「它活過來了」。這不是循環論證嗎？

**不是循環，但它證明的東西比聽起來窄，而那個窄的版本才是我寫下的。**

它證明的是：**在中斷未動、沒有重開機的前提下，那五個 bit 單獨足以殺掉這條網路。**
三個不共用程式碼的訊號在三個時間點一致（DATA 回應、`ip neigh`、`rx_packets`）。

**它沒有證明的**：那五個 bit 是 `J` 之後網路死掉的原因 —— 而這正是第四格
（`T-94`）測出來**不成立**的東西。所以這一對實驗的價值不在「開關會動」，
在於**它把「開關會動」和「`J` 的死因」分開了**，而分開之前這兩件事看起來是同一件。

**還能挑戰的**：`carrier` 全程是 1，我分不出那是網卡說謊還是 `EnablePHYIf`
只關內部介面。**我沒有選邊，兩個都寫在卡上。**

### Q28.3 第四格沒有結論，你卻花最多篇幅寫它。這不是把失敗包裝成深刻嗎？

**這一題應該被問，而我的回答有一個可檢查的形式。**

如果我在包裝，我會做的事是：把 4c 的兩種結果之一事後宣布為「預期」，
或者把它從 `🔶` 改成 `✅`。**兩件都可以在 git diff 裡查。**
`test-cases.toml` 的 `refute` 欄位是插電之前凍結、雜湊進 `[freeze].sha256` 的，
而 `A2.8` 步驟 4c 的表格事前就寫著「兩種結果都要記，不准事後改判」。

篇幅多的理由是它**改變了下一場要做什麼**：三項 `confirmed` 之後沒有新動作，
而這一格生出了一個新實驗。**一個不改變後續行動的結論，不管幾分，都該寫得短。**

**但這一題有一半是對的**：我確實傾向把「沒有押注」講成一種美德。
它同時也是一個成本 —— 我因此少掉一個可以被推翻的預測，而**可被推翻的預測
才是這個 repo 的產品**。所以正確的說法是：那一格我事前不夠了解，
不足以寫出一個具體的預測，**而那是知識的缺口，不是方法的勝利**。

### Q28.4 「方向鍵」這種東西，你確定是原因而不是你剛好第二次打對了？

**確定，而且證據不是「第二次成功」。**

證據是 `cat -A` 印出來的那一行本身：`<RealTek>^[[A^[[BFLR 80520000 0 100`。
那四個位元組**在裝置回傳的資料流裡**（picocom 的 logfile 記的是收到的東西，
而 loader 會回顯它收到的字元）。所以裝置確實收到了 `1b 5b 41 1b 5b 42`，
`argv[0]` 不可能是 `FLR`。

**而同一份 log 也是我承認另一次失敗沒有解釋的理由**：`A2.8` 那次 `EB`，
同樣用 `cat -A` 看，那一行是乾淨的。**同一個工具，一次給了答案一次沒給** ——
如果我拿「第二次成功」當證據，兩次我都會宣布破案。

### Q28.5 `A2.5` 你說對照組壞了兩次。那 2026-08-17 那一輪的結論還算數嗎？

**算數，但理由要重寫，而我沒有去改那一天的紀錄。**

8/17 的結論（`FLW` 保留磁區）**這一夜獨立重現了**，而且多了 8/17 沒有的兩件事：
每一次讀回前的已知第三值，以及反方向的鄰居檢查（寫 `0x3F0000` 保住 `0x3F0100`）。
所以現在支撐那個結論的是**這一夜的證據**，不是 8/17 的。

**8/17 那一輪的問題是什麼**：它的 Step 5 用「`0x3F0100` 有 `ca fe ba be`」
當第二次寫入落地的證明。在 8/17 那一天那是成立的（那之前 `0x3F0100` 是 `ff`），
**壞掉的是我今晚照抄同一格** —— 同一個位址同一個樣式，預期值已經在那裡。
**同一個檢查，第一次是對照組，第二次不是。** 那個時間相依性是我沒有想到的。

`BENCH-LOG.md` 只追加，所以 8/17 的卡片原樣留著；修正寫在 `T-95` 與
`RUNBOOK §8.9.5`，而作業單多了 Step 6c。

### Q28.6 你今晚寫了四千字檢討自己講錯兩個解釋。那兩個解釋有造成任何損失嗎？

**沒有，而這正是要寫下來的原因。**

第一個（doctor 重置板子）如果沒有被檢查，會讓 `A2.5` 之前那一次
`make doctor TIER=2` 被當成禁忌 —— 而它是那一節的安全網閘門。
**一個錯誤的因果會讓人拿掉一個正確的檢查。**

第二個（TFTP 插隊）如果沒有被 `cat -A` 推翻，`Unknown command !` 就會被歸檔成
「已解釋」，而**後來那一次真正的成因（方向鍵）會被同一個標籤蓋掉**。

**兩個都沒有造成損失，是因為它們在同一場之內被檢查了。**
代價很低的時候記下來，是為了知道代價高的時候長什麼樣子 ——
而這個 repo 已經有一次代價高的：W08 第二場那個「loader 是 TFTP client」，
一個來源、讀一次、蓋一個設計、推上去，四天後才被自己的舊紀錄推翻。

</details>

## 2026-08-22（W08 桌面第六場）—— 一個敵意的讀者今天會問的七題

<details>
<summary>展開</summary>

> **這一場沒有量測，全部是靜態讀法，而且它推翻了自己的第一版。**
> 下面七題有五題在問「你憑什麼相信第二版」。

### Q29.1 你說第一版讀法把結論答反了。那我憑什麼相信第二版不是同一種錯？

**因為兩版壞掉的方式不一樣，而第二版的壞法是會被守衛案例抓到的那一種。**

第一版是**形狀比對**：找一個特定的指令序列。它的失敗模式是「另一個形狀做同一件事」，
而這種失敗**沒有任何測試抓得到** —— 你只能想到你想得到的形狀。

第二版是**算術**：對每一個 `mtc0 $12` 用四值逐位元格推導寫進去的值，然後讀 bit 0。
它的失敗模式是「某個指令的語意實作錯了」，而那**是測得到的**：
守衛案例裡有一對只差一個立即數的一個 bit（`xori 0x1e` 對 `xori 0x1f`），
一個必須讀成設 IE、一個必須讀成清 IE。任何一個指令語意寫錯，那一對就會塌成同一個答案。

**還能挑戰的地方，我先講**：那個推導是**直線的**，視窗二十四個指令，
會跨過函式邊界。所以「哪一個 `mtc0` 設了 IE」是可靠的（它只需要往前看四個指令），
而五個「未定」的判定裡有一個把來源追到了上一個函式。那五個我全部印出來源根，
沒有一個是「一個 bit 0 為 1 的字面值」—— 也就是說，
**沒有第三條把中斷從關打到開的路**，而這句話是我從印出來的東西讀的，不是省略的。

### Q29.2 「TFTP 由中斷驅動」這件事，你有第二個來源嗎？還是全靠同一份反組譯？

**有三個，而且第三個是這台自己在 W02 印出來的。**

1. `objdump` 反組譯 `stage2.bin`；
2. `tools/loader-unpack.py --irq`，它有自己的 MIPS 解碼器、不呼叫 objdump，
   而且是從 `GIMR0` 的設位元函式**反推**出 `request_IRQ` 的（不是靠名字找的）；
3. **`dumps/uart-bootloader.log`**，2026-08-16 W02 的捕獲：
   `---Ethernet init Okay!` 這一行就在第一個 `<RealTek>` 上面一行，
   而印它的那個 `printf` 的下一個指令就是設 IE 的 `mtc0`。

第三個特別重要，因為它是**在這個問題被提出之前六天**就記下來的。

**我沒有的**：從 `eth0` 的 ISR 一路走到 TFTP 收送函式的完整呼叫圖。
我走到 `0x80402040` 依 EtherType 分派為止，剩下是讀法不是量測，
`notes/` 裡明講了而不是把箭頭畫過去。

### Q29.3 你說 `J` 之後 TFTP 死掉是因為中斷被遮。那 2026-08-21 步驟 2 為什麼「不跳轉、只關 PHY」也能殺掉它？

**兩件事都能殺掉它，這不矛盾，而且這正是那一晚設計成兩步的理由。**

PHY 關掉 → 封包根本沒有進到 MAC；中斷遮掉 → 封包進來了但沒有人被叫起來處理。
**兩條路各自充分。** 8/21 步驟 2 證明的是前者充分（可逆、三個訊號一致）；
8/22 步驟 4c 證明的是**後者也在起作用** —— 因為只還原 PHY 不夠。

**這一場加的是「後者是什麼」**：不是「還有別的東西」，而是具體的 `GIMR0 = 0`
與 `IE = 0`。而 `P9-17` 的兩個 payload 就是去分辨這兩個裡面是哪一個 —— 或兩個都是。

### Q29.4 那個「八個空白」你怎麼知道不是 printf 印的？

**因為整個第二階段裡沒有任何一段連續八個空白。** `grep -abo` 全檔，0 命中。
所以沒有任何格式字串印得出來，而 `GetLine` 唯一會一次吐多個空白的地方
是 TAB 展開，剛好八個。

**我不知道的**：那到底是一次 TAB 還是八次空白鍵。兩者的回顯一模一樣、
留下的緩衝區也一模一樣，這份紀錄分不出來。**成因是確定的，按鍵不是**，
而我把這句話寫在 `notes/` 裡而不是含糊帶過。

### Q29.5 你說 102 的假設被推翻了。那你為什麼還把 TFTP 的寫入路徑寫進 note？

**因為「它做不到這件事」和「它做得到別的事」是兩個結論，而第二個也是量出來的。**

102 問的是「TFTP 收送寫不寫得到行緩衝區或 `0x8040EAE0`」。
答案是：**寫得到**（`[0x8040DD10]` 這條線性寫入路徑上沒有任何終點檢查），
**但那一次不是它**（那次是 `probe`，一個讀請求，供應 0 byte，一個 byte 都沒寫）。

如果我只寫「不是它」，讀的人會以為那條路徑是安全的。它不是安全的，
它只是那一晚沒有被用到。**兩句話都要在，因為刪掉第二句會讓第一句變成誤導。**

### Q29.6 「指令表宣告 20 個 argv 槽、`GetLine` 只收 128 字元」—— 這兩個數字有被驗證過嗎？還是又是讀出來的？

**讀出來的，而且我把它做成一個會失敗的檢查而不是一句註記。**

`0x8040919C` 的 `li a1,128` 是 `GetLine` 的上限，`0x8040728C` 的 `li a1,20`
是 tokeniser 的槽數（同一個函式開頭的 `memset` 是 80 byte = 20 個指標，互相印證）。
兩個都是靜態的。

**它們現在的地位不是註記**：`tools/mkramboot.py` 的 `ew_lines()` 會**拒絕**
產生一行超過 128 字元或超過 20 個 token 的 `EW`，守衛案例逼它拒絕過。
所以如果這兩個數字讀錯了，錯的方式是「工具太保守，多分了一行」，
而不是「payload 被靜靜截斷一半然後 `J` 過去」。**把一個讀法的錯誤導向安全的那一邊。**

### Q29.7 你這一場改了 `PROGRESS.md` 的摘要表、`REPRODUCE.md` 的三個數字、`P9-4` 的結果欄。這不就是事後修改紀錄嗎？

**這一題問得對，而分界線是「誰擁有那份狀態」。**

- **摘要表**：它複述 `test-cases.toml` 擁有的數字，而那些數字從來沒錯過。
  改的是複本，不是本體，而且改法是把數字換成「`rtcase todo` 印出來的那個」。
- **`REPRODUCE.md` 的 322 / 543**：同一種，複述 `make count-checks` 的輸出。
- **`P9-4` 的結果欄**：這一個是真的動到紀錄，所以它**不在登記簿裡改**，
  而是寫進 `PROGRESS.md § Corrections`，並且 `P9-18` 第四格把那句更正
  **變成一個會失敗的量測**（在 `IPCONFIG` 之前對 `192.168.1.6` 送 probe）。

**沒有動到的**：任何一條 `predict` 或 `refute`。凍結雜湊 `7ade2454…` 換了，
而換它的原因是**新增三列**，`git diff` 上看得到是新增不是編輯。
</details>

## 2026-08-22（W08 收工：六刀 + 第 2 站進站之三）—— 一個敵意的讀者今天會問的八題

<details>
<summary>展開</summary>

> **今天有兩件事會被打**：一次砍掉六個登記項，以及三項全部判 `partial`。
> 前者的風險是「你是不是在挑好做的做」，後者的風險是「partial 是不是失敗的委婉說法」。
> 兩題我都直接答，不繞。

### Q30.1 你一晚砍掉六項，其中三項是全專案唯一能替 flash 提供第二來源的。這不就是把做不到的東西包裝成「刻意不做」嗎？

**「刻意不做」這四個字對那三項是不誠實的，所以登記簿沒有那樣寫。** 它把砍掉的東西
分成三類，而那三項在**「卡在儀器」**那一類，不在「範圍外」那一類。差別是寫死的：

| 類別 | 例子 | 它會不會產出關於這台的可查證事實？ |
|---|---|---|
| 範圍外（同意問題） | `P7-3` 惡意 beacon | **不會**，而且買器材也不會改變 |
| **卡在儀器** | `P9-5/6/7` SPI 夾子 | **會。而且那正是為什麼它是全篇第 14 章的第一段** |
| 刻意換掉 | `P9-10` 回刷 | 會，但已經被 `P9-12` 用低得多的不可逆度買到大半 |

**README 那段原本寫著「這些項目都不會產出關於這台的可查證事實」** —— 那句話對原本
被砍的九項是對的，對新的六項是**錯的**，所以它被重寫，不是被改個數字。

### Q30.2 那你就承認「沒有第二支儀器讀過那顆 flash」永遠不會被解決了？

**是。而且它從「待辦」變成「永久」，這句話寫在第 14 章、`weekly-results` 和登記簿三個地方。**

具體的代價：這台 flash 的每一個 byte 級主張都走同一條路 —— loader 自己的 `FLR`、
裝置自己的 UART、裝置自己的 RAM、`DB` 出來。兩次全讀 sha256 相同證明**傳輸穩定**，
不證明**讀得對**。JEDEC ID 到結案為止一次都沒讀過，所以「Eon EN25QH32B」只有封裝上
的字這一個來源 —— `flashrom` 同意它是 4096 KiB **不算**第二來源，因為那個資料庫就是
用同一個型號名索引的。

**而三項裡最便宜的那一項只要三個 byte**，它倒下的理由跟要讀 4 MiB 的那一項一模一樣，
不是因為比較難。這句話留在理由裡，因為它讓這一刀難看，而它本來就該難看。

### Q30.3 開放題 97 現在算什麼？你砍了三項，是不是等於默認答案是「板子設計問題」？

**不算，而這個區別是今天最重要的一條。** 作者的判斷是箝位、要解焊才讀得到；
**那是判斷，不是量測**。已經量到的只有：三種供電（CH341A 自己的、主機板後方 USB、
ESP8266 穩壓器）都讓晶片端停在 1.70 V，而供電端保持 3.3 V。

三種供電同一個電壓**讀起來**像箝位，但箝位與夾具路徑電阻這一場一條都沒分離。所以
cut 理由寫的是「**不做**」，不是「做不到，因為 X」，而 #97 在 carried-forward 清單裡
**一個字都沒改**。

**還有一個沒有被否決、只是沒有被執行的設計**：在 ESP8266 注入的那條線上串一顆已知
電阻，量它兩端的壓降得到電流，再比較有無串阻時的晶片端電壓 —— 電壓幾乎不動是箝位，
大幅下降是分壓。它不需要在夾具那個固定排針上插東西，**而那正是當初判定「做不到」的
唯一理由**。它記在 `BENCH-LOG.md`，**不記在 cut 理由裡**，因為它是一個提議不是一次量測。

### Q30.4 三項全部 `partial`。這是不是「沒成功」的好聽說法？

**反過來。三項的主結論都成立，寫成 `confirmed` 今晚做得到，而那才是問題。**

| | 主結論 | 錯掉的那一條子預測 |
|---|---|---|
| `P9-18` | 中斷接線讀出來了，第四格證實堆疊在 `IPCONFIG` 之前就活著 | `IRR3` 預測 bit 12–15 = 3，讀回全零 |
| `P9-19` | 2026-08-21 那次拒絕完整重現 | TAB 那一格預測會抱怨，實際是**安靜**的 |
| `P9-17` | `J` 的網路死因就是被遮掉的中斷，線上量到 ARP 0.9 ms 回應 | 預測寫「DATA 回來」，**沒有回來** |

**每一項都有一條事先凍結的子預測是錯的。** 挑主結論寫 `confirmed`，就是事後決定
哪一半算數 —— 而這份登記簿存在的全部理由就是不讓自己有那個權力。

### Q30.5 `IRR3` 那一格你說「不算分」。那不就是預測錯了就說它不算嗎？

**不是，而且方向剛好相反：不算分對我是不利的。**

`IRR3` 有預測、**沒有寫反證分支**。`rtcase check` 在 CI 裡擋的是「有結果但沒反證條件」
的整列，而這是一列裡的一條子句 —— 檢查抓不到這種粒度。

所以正確的處理是：**記下來，不計入成立也不計入不成立**。要是我把它算成「反證觸發」，
我就是在替一條當初沒寫失敗長什麼樣的預測，事後補一個失敗定義。

**這次的方向恰好對我不利**（預測是錯的，不算分等於少一筆「我誠實承認錯誤」的紀錄），
所以它看起來無害。**下一次方向會反過來**，而那時候規則已經在這裡了。

### Q30.6 `P9-17` 的 C 版沒把 TFTP 救回來。那你憑什麼說「中斷就是死因」？

**因為量的地方換了一層，而換的證據在線上。**

`tshark` 抓到：ARP 請求出去、**ARP 回應 0.9 ms 回來**（來源 MAC `56:0a:01:01:01:e8`，
而 neigh 條目是**故意先刪掉**才逼出這次全新交握的），然後三個 TFTP 讀請求零回應。

ARP 跟 TFTP 走**同一個** `eth0` ISR。所以：ISR 在跑、收得到 frame、解析得了、發得出
回應 —— **封包接收這一層恢復了，而只還原 `GIMR0` 的對照組連 ARP 都沒有**。這就是
「中斷是死因」成立的部分，而它比原本預測的位置低一層。

**TFTP 沒回來是反證，而它讓問題變小不是變大。** 登記簿凍結時留了兩個候選給這個分支
（交換器需要更多重新初始化、cache 維護或 payload 動壞了別的東西）—— **那次 ARP 往返
把兩個都殺掉了**。剩下一個更窄的：TFTP 服務自己的狀態。

### Q30.7 「TFTP 服務卡在傳輸中」聽起來很像事後編的。有證據還是只有故事？

**有兩個不共用機制的讀法同意它，而我仍然把它記成候選不是結論。**

1. **主控台的字**：健康的那次傳輸（2c）印了進度指示 `/`、`*TFTP Client Download
   Success!`、`.Success!` 三段；C 版跳完之後印了進度指示 `-`，然後**什麼完成訊息
   都沒有**。
2. **`console-lint` 的判讀，而它不知道上面那件事**：它全場只數到**一次** TFTP 重繪，
   而那一次是 2c。C 版之後那個 `<RealTek>` 被它判成**命令迴圈印的真提示字元** ——
   也就是 `J` 的 handler 回來了，不是傳輸完成了。

**要定案得讀 TFTP 的狀態機**，找出「完成」的條件、以及一個被中斷打斷的傳輸會停在哪。
那是開放題 103，**桌面題，不用再進站**。

### Q30.8 你這一晚有沒有哪一次是儀器騙了你？

**有，而且它站在會變成結論的位置上。**

第一次 `tshark` 擷取寫到 `$FWRE_WORK/dumps/`。`sudo tshark` 會降權，建不了檔，
印了 `Permission denied`、`0 packets captured`，**然後 exit 0**。

而當時我要問的問題是「線上到底有什麼回來」——「**什麼都沒有**」正好是候選答案之一。
一個回報成功的失敗，剛好落在會被讀成答案的位置。

**抓到它的是對照組，不是錯誤碼**：那份擷取裡連**我們自己送出去的**三個 TFTP 請求都
是零，而那些封包確定送出去了。`P0-4` 在 2026-08-17 記過同一句 —— **零在鏈路未經證實
之前不是證據** —— 那次是對裝置說的，這次是對自己的儀器說的。

這是第 55 個儀器 bug，比例沒有動：**沒有一個是儀器自己的自我檢查抓到的。**

</details>

## 2026-08-23（W08 桌面第八場：不通報、全公開）—— 一個敵意的讀者今天會問的七題

<details>
<summary>展開</summary>

> **今天有兩件事會被打，而且都打在同一個地方：可信度。**
> 一是「你第二次把別人 2018 年寫過的東西寫成自己的」，
> 二是「你改掉了自己貼在首頁的承諾，然後說這是有理由的」。
> 這兩題不能用技術細節繞過去，所以下面直接答。

### Q31.1 你的 README 寫著「新東西一律先通報 TWCERT/CC」。你一次都沒通報，然後今天把那句話刪了。這不就是事後合理化嗎？

**是事後，但不是合理化 —— 差別在於那句承諾從來沒有被違反過，它是被取消的。**

時間軸攤開來，它自己會說話：

| 日期 | |
|---|---|
| 2026-08-17 | 三項量到，草稿寫好，**明確記下「還沒送，因為 by-handler 搜尋還沒跑」** |
| 2026-08-18 | 第四項量到，第二份草稿，同樣記下沒送 |
| 2026-08-23 | 搜尋跑了，最強的一項是 2018 年的 CVE；決定不通報，同一個 commit 改掉 README |

**沒有任何一天是「公開了但沒通報」。** `poc/04` 從 8/17 到 8/23 一直是空殼，
而且它自己的內文就寫著它是空殼、為什麼是空殼。那六天裡那條規則是有效的、
而且是有代價的 —— 那個代價留在 git 歷史裡，不是我今天講的。

**改掉的理由也不是「反正沒人查」，是它站不住了。** 留著一句不打算執行的承諾，
比拿掉它糟：前者是假的，後者只是不好看。而拿掉的那一刻，
`docs/disclosure.md` 多了一節寫「為什麼」以及「這個理由蓋不住哪四件事」。
**你可以罵那個決定，但你查得到它。**

### Q31.2 「公開這些不會讓攻擊者多拿到任何東西」—— 你只量過一台機器，憑什麼這樣講？

**憑三件事，而第三件是我自己講出來的弱點。**

一、**這台上的兩條路都已公開、都更短**：`GET /config.dat` 一發拿明文帳密
（CVE-2019-19822/19823，2019-12），`POST /boafrm/formSysCmd` 一發拿 root
（CVE-2024-51228，2024-11，而且公開 PoC 的受影響清單直接列著
`TOTOLINK-CX-N150RT-V2.1.6-B20171121.1002`）。今天公開的每一項，
登記簿裡逐列對照它「本來就到得了嗎」，三列右邊全是「是，而且更早更便宜」。

二、**這不是推論，是這個 repo 自己量過的**：`poc/01` 是那 7,507 bytes 的實測，
`poc/02` 是那發命令注入的實測。

三、**而它蓋不住的部分我寫在同一節裡**：窗口是 2015→2018 有、2020 沒有，
所以受影響集合不是「一台 EOL」；CVE-2024-51228 點名六個 `-CX-` 型號我只測一個；
**`N300RT` 根本不是 EOL** —— CVE-2025-34319 寫的是 "prior to V3.4.0-B20250430"。
那個論證在那三件事上是**假設**，而它被標成假設。

**如果你要打，就打第三點**，那是真的軟。打「他只量一台」我已經先寫了。

### Q31.3 你第二次把公開的東西寫成自己的發現。那我為什麼要相信 `D-15` 是新的？

**你不該相信「新的」，而且我也沒有這樣寫。**

`docs/disclosure.md` 對 `D-15` 的措辭是「八個搜尋角度都空」，
接的下一句是 **"no prior art found 比 new 弱 —— 扛著它的原始碼一直是公開的，
『沒人發表』不等於『沒人看得到』"**。那句話在 2026-08-18 就寫了，不是今天補的。

至於為什麼 `D-15` 比 `D-4` 可信一點，理由是可查的而不是修辭：

| | `D-4` 的 A 半 | `D-15` |
|---|---|---|
| 搜過幾個角度 | **0**（那正是問題） | 8，跨兩天 |
| 有沒有現成的 CVE 詞彙可搜 | 有：*unauthenticated password change* | 沒有 —— 「比對一塊沒人寫過的堆疊」不是 CVE 描述會用的講法 |
| 最接近的公開品 | **一模一樣** | CVE-2007-4915：同一個函式、同一個功能、**相反的方向**（那是多一個寫，這是少一個寫）|

**但這題真正的答案是：這個 repo 的價值不該建立在「這是新的」上面。**
`D-15` 就算明天被人指出 2016 年有人寫過，那份指令級的讀法、三個 build 的對照、
以及六發請求裡多打的那兩發（證明比對「有執行」而不是「被跳過」），一樣還在。
**被撤掉兩次而每次都自己寫下來，比從來沒撤過更值得信。**

### Q31.4 你自己說 `D-15` 的實際影響是零。那你公開它幹嘛？湊數嗎？

**因為「同一個資產的第二條路」和「不同類別的缺陷」是兩句話，而只有第二句站得住。**

```
/config.dat  被送出來，是因為路徑裡沒有 htm 也沒有 asp —— 閘門根本不執行
D-15         是閘門執行了、然後被打穿
```

把 `.dat` 納入閘門，就修好了 CVE-2019-19822；**`D-15` 原封不動還在**。
一個修法能蓋住前者而蓋不住後者，那它們就不是同一個東西，不管它們今天讓你拿到
一樣的檔案。

而且 **2020 那個 build 兩個都改了**（第二對憑證沒了、`orgpassword` 那行也沒了），
所以廠商當年很可能是分開處理的 —— 那也是「兩件事」的一個外部證據。

**我第一版沒有這樣寫。** 第一版寫的是「未認證憑證洩漏接完整接管」，
那是在沒問攻擊者本來有什麼的情況下寫的影響句，跟 `report-draft-2.md`
寫「it adds reading」然後停住，是同一個錯的兩個方向。

### Q31.5 `w05-password-page.html` 在你 repo 裡一個字都沒被提到過。那 `password.htm` 帶明文密碼這個發現，算不算數？

**主張算數，那個檔案不是它的證據 —— 而這個區分是我自己寫進 note 裡的，
不是你抓出來的。**
...
| | 出處 | 有沒有出處紀錄 |
|---|---|---|
| **主證據** | 這台自己 rootfs 的 `/var/web/password.htm` 模板，第 18–19 行的兩個 `getInfo` SSI | **有**，`qemu-env-2018` 是從這台的 dump 建的 |
| 旁證 | `dumps/w05-password-page.html`，5,332 bytes，SSI 全展開、變數帶著值 | **沒有。裝置還是模擬、哪一次跑的，都不知道** |

模板裡那兩行是「伺服器會把帳密塞進頁面」的**機制**，那不需要旁證就成立。
旁證只是「而且確實展開了」。**所以我把 note 的重量放在模板上，並且把那個洞
寫成開放題 106，附上關掉它的方法**：下次 `A3.13` 那一發本來就要打，多存一個 body、
`cmp` 一次就好。

順帶一提，第三個獨立來源是姊妹機種的 CVE：CVE-2018-13309（用密碼觸發的 XSS）、
13310（用帳號觸發的 XSS）、13317（GET 就拿到明文密碼）—— **三個 CVE 都掛在
`password.htm` 上，而它們能存在的前提就是那頁面裡有那兩個值。**

### Q31.6 你拿 ISE 那篇 blog 當 `D-18` 的 prior art，但你自己說你沒讀到。這不就是你整個專案在罵的那種做法嗎？

**是，所以它在檔案裡是這樣標的：**

> ⚠️ **單一來源，來自搜尋引擎摘要。** 三次抓取都沒回應，`web.archive.org`
> 從這條工具鏈連不上，作者自己也打不開。**在有人真的讀過那篇之前，
> 下游不得把這句當成已確立。**

有三個選項，我選了第三個：

1. 當成事實寫進去 —— 那才是我在罵的做法
2. 整段丟掉 —— 那正是 CVE-2024-51228 漏兩週的機制：**沒寫下來的線索等於沒有**
3. 寫進去、標成未驗證、開一個開放題（#108）並說明怎麼關

**而且我把它放進「不利於自己」的方向**：如果那句話是真的，
`D-18` 的行為觀察就比這個專案早八年，只剩機制是自己的。
**一個未驗證的線索被記在削弱自己的方向，跟被記在有利的方向，可信度不一樣。**

### Q31.7 A、B、C 三項你都不寄，理由一項一個。看起來就是「都不想做」然後回頭補理由。

**登記簿把它們分成三類，而分類是有後果的，不是修辭：**

| 項目 | 哪一類 | 一句話 |
|---|---|---|
| A | **不是我們的** | CVE-2018-13315，2018-07。沒有東西可以通報 |
| C | **刻意換掉** | 機制沒被寫過；但類別飽和，而且在一台已經有公開未認證 root 的機器上，DoS 是嚴格較弱的那個 |
| B、`D-15`、其餘 | **政策選擇** | 它們**可以**被通報。沒有，理由是那一整節 |

**這三類不能互換，而且登記簿明講了為什麼**：把它們攪在一起，
「我選擇不做」就會變成「我做不到」，而那是往好聽的方向漂。

**而且第四類是空的，這件事我特別寫下來**：這裡沒有任何一項是「卡在儀器」。
卡在儀器的項目全部在 `test-ledger.md` 的砍除清單裡，各自帶著
「什麼條件會讓它回來」—— 那份清單上有三項是 SPI 夾子供電不足，
那才是真正的「做不到」，而它們長得跟今天這三項完全不一樣。

**你可以說作者選擇不通報是錯的。** 那是一個決定，寫在檔案裡、有日期、有理由、
有它蓋不住的四件事。**但它不是一個被包裝成別的東西的決定。**
