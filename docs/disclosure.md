...
dropped on 2026-08-23.** The rule is now two: **findings and reproductions are
published; tradecraft is not published at all.**

The middle clause is not being quietly forgotten. It ran the project for six
days, it cost this repository something it wanted to write — `poc/04` was a stub
for that whole period and says so in its own text — and it was dropped for a
reason that can be checked and argued with. That is §"The decision of
2026-08-23", below, and it is deliberately placed before the register rather
than after it.
...
**Decided by the author on 2026-08-23. Nothing in this register was sent to
TWCERT/CC, to the vendor, or to anyone else, and nothing will be. The
reproductions are published here instead, in `poc/`.** The 90-day clock this
file describes was never started and now never will be; `README.md`'s promise to
report first stood until this date and was changed in the same commit as this
section, because a promise nobody intends to keep is worse than no promise.

### The argument, in the order it was actually made

The order matters, because the first three steps are what a reader will want to
attack and the fourth is where the argument stops being general.

**1. On the build this unit runs, an unauthenticated attacker on the LAN already
has root, publicly, with a working proof of concept.** CVE-2024-51228,
disclosed 2024-11-27, `POST /boafrm/formSysCmd` → `system()`, no `Authorization`
header. The public repository behind it names
`TOTOLINK-CX-N150RT-V2.1.6-B20171121.1002` — this exact build string — in its own
affected-versions list. It is reproduced here in
[`poc/02-command-injection.md`](../poc/02-command-injection.md).

**2. On all three builds, the plaintext administrator credentials are already
one unauthenticated GET away, publicly, since 2019.** `GET /config.dat` returns
7,507 bytes beginning `COMPCS` with no credentials sent
...
these devices.** Check it item by item rather than taking the sentence:

| published here | what it gets an attacker | already reachable by |
|---|---|---|
| `D-15` — empty username and empty password pass the gate | gated pages, including `password.htm`, which inlines the plaintext credentials (see [`password-page-credentials.md`](../notes/password-page-credentials.md)) | `GET /config.dat`, the same credentials, public since 2019, **no bypass required** |
| `D-4` B half — an empty stored password skips the comparison | the same gated pages | the same, and it needs a flash write first where `config.dat` needs none |
| `D-11` — one POST removes the web server until power cycle | denial of service against `boa` | CVE-2024-51228 gives root; killing a process from root is not a vulnerability |

**Every row's right-hand column is public, older, and cheaper than the left.**
That is the whole argument, and it is why "publishing this is dangerous" is not
a claim this file is willing to make.

**4. The vendor is not a route.** Independent Security Evaluators reported
twelve CVEs against the sibling A3002RU in 2018 — three emails, 2018-05-30,
2018-07-02, 2018-07-03 — and record **no response to any of them**. One of those
twelve, CVE-2018-13315, is a defect this project independently re-found on this
unit eight years later, still present. A coordinator adds a step and a delay to a
path with no demonstrated exit.

### What this argument does NOT cover, stated because it is the weak half

- **The window is 2015 → 2018 present, 2020 absent**, so the affected set is not
  "one end-of-life unit". It is whatever ships a 2015–2018-era `boa`, and
  **this project has measured exactly one such device.**
- **Five of the six `-CX-` models CVE-2024-51228 names are untested here** —
  A3002RU, N300RT (three builds), N302RE. Step 3 above assumes CVE-2019-19822
  reaches them too, which is likely from the SDK lineage and **is an assumption
  about hardware nobody here has touched.**
- **`N300RT` is not end-of-life.** CVE-2025-34319 names firmware "prior to
  V3.4.0-B20250430", so that model was still receiving builds in 2025 while
  CVE-2024-51228 names its 2017 and 2019 builds. *"The device is EOL"* is a true
  statement about this unit and **not** a statement about the affected set, and
  it was nearly used as one.
- **Nothing here was measured on the WAN side.** Every claim is LAN-side, in the
  configuration this unit shipped in.

If any of those four turns out to matter, the argument in step 3 weakens for the
device it matters on, and this section should be read as wrong rather than
patched quietly.

### Three kinds of "not reported", kept apart

Collapsing these is how *"I chose not to"* becomes *"I could not"*, which is the
flattering direction:

| item | which kind | in one sentence |
|---|---|---|
| `D-4` A half (unauthenticated password change) | **not ours** | It is CVE-2018-13315, published 2018-07-03. There is nothing to report |
| `D-11` (one POST kills `boa`) | **traded away on purpose** | The mechanism is unpublished; the class is saturated and the impact is strictly below a public root RCE on the same device. Reporting it buys nothing anyone can act on |
...
|---|---|---|---|---|---|
| **D-1** | ~~`form_formRoute` / `subnet` reaches `system()` in **all three** builds~~ | `BoaGate` R2 — **and the tool was wrong** | yes — PROGRESS open #6 | ❌ **withdrawn 2026-08-17** | Two independent reasons, and the order they arrived in matters. **Prior art, found before the test:** Cisco Talos TALOS-2023-1894 / CVE-2023-41251 reports this exact parameter in the same Realtek rtl819x SDK family as a 100-byte `sprintf` stack overflow with **no `system()` anywhere** — published 2023, and a search by *handler* found it on the first page where a search by *product* had returned nothing. **Then the measurement:** `P3-2` fired on the device produced zero command execution, while `localPin` on `formWsc` produced four ICMP echo requests through the same oracle. `BoaGate` R2 mis-classified an `sprintf` site as a `system()` site, and that rule feeds conclusions about all three builds. **A correction to the withdrawal itself, 2026-08-23, and it is kinder to the instrument than the withdrawal was.** `subnet` *does* reach `system()` in this SDK — **CVE-2018-13314 and CVE-2018-13316** report exactly that on `formAliasIp` (`ipAddr` and `subnet`, A3002RU 1.0.8, 2018). So R2 was not hallucinating a pattern; it attached a real pattern to **the wrong handler**. The withdrawal stands — `P3-2` fired on `formRoute` and produced zero command execution — but *"the tool was wrong"* is less useful to the next version of the tool than *"the tool matched a genuine sink family and lost track of which handler owned it"* |
...
| **D-4** | An empty stored administrator password skips the credential comparison entirely, **and an unauthenticated request can set it empty** | measured on the device 2026-08-17; the branch at `0x0040bd18` read at instruction level in W04-2 | **yes, and that is a defect** — `runsheet.md` `A3.11.2` carries the complete request. This column said `no` until 2026-08-18 | ⚙️ **split 2026-08-23: the A half is CVE-2018-13315 and is not this project's; the B half stands. Published, not reported** | Register `P10-4` **and `P10-3`**, and 2026-08-23 established that the pair is **two things, not one**. This row used to say *"reachability matters more than the branch: if no unauthenticated path can set it empty, this is a curiosity"*. **There is such a path and it needs nothing.** `formPasswordSetup` carries `Cusername`/`Cpassword` fields for the current credentials and the handler does not check them, so an unauthenticated POST that does not know the current password changes it. Set it empty and `password.htm` returns 200 and 5,322 bytes of real HTML with no `Authorization` header at all — and a **wrong** password is also accepted, so the comparison is skipped rather than matched. **The per-handler search ran on 2026-08-23 and it took one query.** `P10-3` — the unauthenticated password change — is **CVE-2018-13315**: *"Incorrect access control in formPasswordSetup in TOTOLINK A3002RU version 1.0.8 allows malicious users to change the admin user's password via an unauthenticated POST request"*, published 2018-07, CVSS 9.8, CWE-20, confirmed against four independent sources (NVD, Vulmon, CXSecurity, GitHub Advisory `GHSA-vw86-c7px-f6g6`). Independent Security Evaluators' own title for it is *"Missing Server-side Validation of Current Password During Password Change"* — **the same sentence this row had been writing for six days.** Same handler name, same mechanism, sibling model, eight years earlier. **This is `D-1` a second time**, and worse: `D-1` needed a Talos advisory to find, this one is on NVD under the handler's own name. `P10-4` — the empty stored password skipping the comparison — was **not** found in that search and is not in ISE's twelve; it stays a candidate, with the caveat that it is only reachable *through* the A half and therefore through an eight-year-public CVE. Both are now published with their requests in [`poc/04-auth-takeover.md`](../poc/04-auth-takeover.md); neither was reported to anyone. Full search record: [`prior-art.md`](../notes/prior-art.md) §2026-08-23 |
| **D-5** | Two published advisories name endpoints that exist in no dispatch table (`formWlwds`, `fromStaticDHCP`) | three `root_form[]` recoveries | yes — [`cve-status.md`](../notes/cve-status.md) | ✅ **published; not sent to a CNA either, per the 2026-08-23 decision** | Not a vulnerability: a correction to a public record. Nothing is embargoed and nothing ever was. **A third example arrived 2026-08-23** and it makes the pattern hard to call a typo: CVE-2018-13307 names the handler **`fromNtp`** — `from`, not `form` — alongside `formWlwds` and `fromStaticDHCP`. Three published advisories, three endpoint names that appear in no recovered `root_form[]`, and two of the three differ from a real handler by the same two transposed letters. The likeliest reading is that the advisories were written from the HTML forms rather than from the dispatch table, which is a statement about how this family gets audited |
...
| **D-18** | **The gate's session arm is keyed on the client's IP address and expires against a variable nothing writes, so it stops working 601 seconds after boot** | [`auth-session-ip.md`](../notes/auth-session-ip.md) — the comparison at `0x0040bff8`–`0x0040c060`; `beforeuptime` at `0x004899dc` has **one** reference in the whole binary and it is the read, confirmed by Ghidra and by an independent encoding scan whose control in the same run returns a read *and* a write | mechanism and addresses | **held — static only** | No register row: it came out of reading forty instructions past the question the listing was generated for. Two consequences and the second is the one that matters. **First**, `P2-7`'s "authorisation is per-request HTTP Basic" is correct in practice and incomplete in mechanism — there is a second arm and it is dead. **Second**, for the first ten minutes of uptime it is *not* dead, so the device has a security state nobody has measured: a gated page served to whichever address logged in last, with no credentials on the request. The emulator cannot reach that state — `sysinfo()` under `qemu-user` returns the **host's** uptime — so this needs the bench, in `A3.2`, which is the only station that owns the clock. Prior art: Talos's CVE-2023-47677 describes a CSRF protection on this SDK's `boa` with a *different* mechanism; not resolved whether they are the same feature. **A second and closer piece arrived 2026-08-23, and it is behavioural rather than mechanical.** Independent Security Evaluators, writing about the sibling A3002RU in 2018: *"the server doesn't send session tokens — as long as someone has signed in recently (approximately 5 minutes), any request sent to the router will be considered authenticated."* That is this feature seen from outside: ~5 minutes against the 601 seconds read here, and no session cookie in either account. **So the observation is eight years old and only the mechanism is this project's** — the IP key, the `beforeuptime` read at `0x004899dc` with no writer anywhere in the binary, and therefore the expiry that never fires after boot. ⚠️ **That quotation is single-sourced from a search summary.** `blog.securityevaluators.com` did not answer three fetch attempts and `web.archive.org` is unreachable from this toolchain; the author could not open it either. **Until someone reads the post itself this row cites a claim it has not verified**, and says so rather than dropping it |
| **D-15** | **The Basic-auth path compares the supplied credentials against a second pair of stack buffers that nothing ever writes, and a request whose username and password are both empty matches them — skipping the whole authorisation block** | [`uninit-credential-pair.md`](../notes/uninit-credential-pair.md) — the three reads at `0x0040bd4c`, `0x0040bd7c`, `0x0040bd94` and the absence of any write across all 1,964 bytes of `process_header_end`; fired on **two** emulation profiles | everything, from 2026-08-23: the mechanism, the addresses **and the request**. That column said *"no ready-to-send request appears anywhere in this repository, and none is to be added while this row says held"* from 2026-08-18 until 2026-08-23, which is the whole life of the rule that sentence enforced | ✅ **published 2026-08-23, deliberately not reported — and it is still the strongest item in this table** | Register `P2-9`. Distinct from `D-4`: there the *stored* password is empty and the comparison is skipped at `0x0040bd18`; here the stored password is `admin`, the comparison runs, and it succeeds against a buffer nothing filled in. It reproduces on the **published** V2.1.2 image with a different `boa` binary, so it is not a property of this unit's unpublished build — and **V3.4.0 does not have it**: its `process_header_end` equivalent `FUN_00409fd8` carries one credential pair, both halves filled by `apmib_get` immediately above the comparison, and no `req->0xb0 = 2` branch at all. So the window is 2015 → 2018 present, 2020 removed, which bounds who to tell and also means the vendor may already know. **Two of the three preconditions are now met, 2026-08-18.** (a) *What the higher level buys:* **nothing.** `req->0xb0` is read at exactly two instructions, both inside `process_header_end`, and the second — `0x0040be24`/`0x0040be2c` — branches past the entire authorisation block on any non-zero value, so 2 and 1 are equivalent. The row's wording is corrected above from "at a higher privilege level" to "skipping the whole authorisation block", which is both more accurate and more serious. (b) *Prior art:* searched four ways — by function (`process_header_end` + uninitialised stack), by version (`Boa/0.94.14rc21`), by SDK (rtl819x Jungle + auth bypass), by symbol (`check_auth_flag`) — and **nothing matches**; Talos's fifteen reports on this SDK contain no authentication defect at all, and CVE-2007-4915 is the same function with the opposite mechanism. **What is now known that weakens "original":** the SDK source is public (two vendors' GPL drops on GitHub), and it shows the buffers are `admin_name`/`admin_password` fed by `MIB_SUPER_NAME`/`MIB_SUPER_PASSWORD` — which **no build in this family fetches**, 2015 included. So the defect is visible to anyone reading the source next to any of these binaries. (c) *Confirmation on the device:* **done, and this row did not say so for five days.** `BENCH-LOG.md` `T-43`, 2026-08-18 19:44, six requests: empty-username-and-empty-password returned 200/333 with a body **byte-identical** to the real-credential body, no header returned 302/132, and `/password.htm` went 302/132 → 200/**5332**. The two requests the emulated run did not have are what make it decisive — empty-user-with-a-password and a-user-with-empty-password **both returned 302**, so the comparison is *running* rather than being skipped, which is exactly what separates this from `D-4`. **A fourth thing, found 2026-08-23 and it changes what the page is worth:** `/password.htm` on this build inlines the plaintext administrator username *and* password into two JavaScript variables, so what this bypass reads is the credentials themselves — and of the three builds only this one does that ([`password-page-credentials.md`](../notes/password-page-credentials.md)). **That reads as an escalation and is not one:** `GET /config.dat` has handed over the same two values, unauthenticated, since CVE-2019-19822 in 2019. Prior art re-searched 2026-08-23 from four further angles (Boa version alone, `process_header_end`, rtl819x + auth bypass, the SDK source by symbol) and **still nothing**; Talos's fifteen rtl819x reports contain no authentication defect. Request published in [`poc/05-auth-bypass.md`](../poc/05-auth-bypass.md) |
...
| **D-19** | **One unauthenticated SOAP request terminates `/bin/miniigd`**, and the trigger is any `NewInternalClient` that `inet_addr()` rejects — not a shell metacharacter | [`three-unread-binaries.md`](../notes/three-unread-binaries.md) §2, 2026-08-19 subsection, and `BENCH-LOG.md` `T-79` / `T-82` / `T-83`. Three requests on three boots, each ending in a closed connection with no response and `52869` refusing afterwards; `ps` over a telnet shell two minutes later shows **no `miniigd` process at all** and nothing respawns it, so only a power cycle brings the daemon back. The control is what makes it a result: `NewInternalClient=10.1.1.1` is answered HTTP 200 and the daemon survives it and a subsequent read, while twenty-two `A` characters — **no metacharacter anywhere** — kill it exactly as a backtick payload does | **no** — the mechanism is named here and nothing else | ⚙️ **searched 2026-08-23, nothing matched; published, not reported** | **This row was carried as `NOT SEARCHED` from 2026-08-19 to 2026-08-23**, and the search has now run: UPnP/IGD denial of service on the Realtek `rtl819x` SDK, by behaviour rather than by binary name. **Everything returned was CVE-2014-8361** — the SOAP *command injection*, on the same `AddPortMapping`, through the same `NewInternalClient` — and nothing at all describing the daemon **terminating**. So the parameter is the published one and the outcome is not, which is an uncomfortable place to be: the most likely reading is that this build carries a variant of the 2014 defect that fails closed instead of executing, and **that is a hypothesis, not a finding.** Two things remain unmeasured and both change the severity — **why** it dies (the unbounded `strcpy` at `0x0044851c` is on the path but a 22-byte value is a poor fit for it), and whether the daemon is reachable from the WAN, which `P8-7`'s second half has not settled. Reported severity would have been *"LAN-side, single request, recoverable by power cycle"*, and the honest framing is that a device whose owner never touches UPnP loses a service they were not using — on a device that already has public unauthenticated root. Not reported, per the 2026-08-23 decision. Original text follows. The search has to cover UPnP/IGD denial of service on the Realtek `rtl819x` SDK by behaviour rather than by binary name, the way `D-17` had to, because `miniigd` self-reports `Server: miniupnpd/1.4` and that is a different codebase's name. Two things also unmeasured, and both change the severity: **why** it dies — the unbounded `strcpy` at `0x0044851c` is on the same path but a 22-byte value is a poor fit for it — and whether the daemon is reachable from the WAN at all, which `P8-7`'s second half has not settled. Reported severity would be "LAN-side, single request, recoverable by power cycle", and the honest framing is that a device whose owner never touches UPnP loses a service they were not using |
...
**Superseded 2026-08-23.** Nothing in this table has been reported to anyone and
nothing now will be — see §"The decision of 2026-08-23". The ranking sentence
that stood here (*"`D-15` is now the most serious item here, and `D-4`, `D-11`
and `D-12` are next"*) survives only in part: **`D-15` is still the strongest
item**, `D-12`'s image-validation half is CVE-2023-34435, and **`D-4` lost its
larger half to CVE-2018-13315**. A table that only ever grows is a table nobody
is checking; so is a ranking nobody re-runs.
...
**`D-4` and `D-11` had theirs on 2026-08-23, and `D-4`'s overturned half of the
row.** The A half is **CVE-2018-13315**, published 2018-07 against the sibling
A3002RU, found by searching the handler name — the same query shape, and the same
outcome, as `D-1`. That is **twice** this register has written up as its own
something a one-query by-handler search would have refused, and both times the
search had not been run because the work already felt finished. `D-11`'s search
returned no matching mechanism and a saturated class. Both are recorded in
[`prior-art.md`](../notes/prior-art.md) §2026-08-23.

**A third staleness was found in the same pass, in the file whose whole job is to
prevent it.** `prior-art.md` carried a section headed *"Not searched yet — three
items"* listing `D-15`, `D-17` and `D-12` — while the section **a hundred lines
above it in the same file** recorded all three as searched, with their results.
Both were written on 2026-08-18. No tool reads that file either, so nothing
caught it: the identical shape to this file's own governance defect below, and
...
> **Steps 3–5 are superseded, 2026-08-23, and are kept rather than deleted.**
> Nothing is being reported, so there is no coordinator, no 90-day clock and no
> decline to record — see §"The decision of 2026-08-23". **Steps 0, 1, 2 and 2a
> are not superseded and are the load-bearing half anyway**: they are what
> `D-1` and `D-4` were both written without, at a cost of two withdrawn claims.
> The reporting half is deleted from practice but stays legible here, because a
> procedure that quietly loses three of its six steps is indistinguishable from
> one that was never followed.
