...
# 2026-08-22（六）W08 第 2 站進站場次之三 —— 實錄，寫在跑完之後

**跑了什麼**：`A2.1` → `A2.2` → `A2.9` 步驟 0 → `A2.4` → `A2.9` 步驟 1 → 步驟 2 →
步驟 3 → 斷電重開 → `A2.2`。照桌面第六場「五、下一次進站的計畫」的順序，中間沒有
插別的。`A2.3`、`A2.5`、`A2.6`、`A2.7`、第 5 站都沒有跑。

| | |
|---|---|
| 電源循環 | **2 次**（一次進場，一次收工前的驗證） |
| **flash 寫入** | **0 次。整場一個 byte 都沒寫。** |
| RAM / 暫存器寫入 | `EW` 到 `0x80540000`（80 byte payload，兩版）與 `0xBB804104`（五個交換器暫存器，每次 `J` 之後重寫）—— **全部斷電即還原** |
| `J` 跳轉 | **2 次**，兩次都回到 `<RealTek>` |
| 登記簿 | `P9-18` `P9-19` `P9-17` 三項 **`partial`**。W08 由 3 outstanding 變 **0**，`8 / 8` |
| 逐字實錄 | `$FWRE_WORK/dumps/w08-a29.log`（2,447 bytes）、`/tmp/w08-c-wire.pcap`（5 個封包） |

> 🔴 **三項全部判 `partial`，而不是 `confirmed`。** 每一項都有一條自己事先凍結的
> 子預測是錯的。三項都寫成 `confirmed` 在今晚是做得到的 —— 主結論都成立 ——
> 而那正是「事後決定哪一半算數」。**不四捨五入。**

## 一、紀錄卡

```text
T-96  P9-18   中斷接線的三個讀值，以及一個只能量一次的格子      2026-08-22 05:0x
可行性: ★★★★★   驗證狀態(測前): unverified（純靜態，未上機）
...
送出（逐字）:
      ...
      DW B8003014 1
原始回應:
      B8003000:  00008100  88000004  00000000  30050004
      B800300C:  30050004  00000000  00000000  00000000
      B8003014:  00000000  00000000  00000000  00000000

判定 —— 逐格對凍結條件:
  GIMR0 bit 15 = 1        -> request_IRQ(15,"eth0") 跑過了。停止條件 1 不觸發
  GIMR0 != 0              -> 停止條件 2 不觸發
  GIMR0 bit 27 = 0        -> 與預測相同
  GIMR0 bit 8  = 1        -> 【沒有下注的那一格】0x80402FB8 的 |= 0x100 在乙太網
                             初始化路徑上。凍結時寫的指示是「去看 0x80402F80 的
                             呼叫者」—— 那是桌面題，不是今晚的答案
  IRR1 bit 28-31 = 3      -> 反證 (c) 不觸發
  IRR3 bit 12-15 = 0      -> 【反證】預測寫 3。而 IRR3 這一條沒有寫反證分支

計畫外，免費拿到的:
  整行讀比只讀一個 nibble 貴 0 元:
      IRR1 = 0x30050004 -> source 8 優先權 4、source 12 優先權 5、source 15 優先權 3
      而 GIMR0 只解遮 8 與 15。source 12 有優先權卻被遮著
判定: 🔶 partial —— 三格成立，一格反證，而反證的那一格不算分
反證檢查: 測前寫「(a) GIMR0 的 bit 15 是 0 → request_IRQ(15,eth0) 沒有跑到，
      P9-17 的兩個 payload 先不要送」「(b) GIMR0 整個讀回 00000000 → 停手」
      「(c) IRR1 的 bit 28-31 不是 3 → 路由暫存器的模型錯了，installs 那張表
      要降級」。實際:(a) bit 15 = 1，不觸發;(b) 讀回 0x00008100，不觸發;
      (c) bit 28-31 = 3，不觸發。
      **而 IRR3 那一條有預測、沒有寫反證分支** —— 它讀回全零而預測寫 3，
      所以它只能被記下來、不能被算分。這是這張卡自己的失誤，記在這裡
```

```text
T-97  P9-18 第四格   IPCONFIG 之前，網路堆疊起來了沒            2026-08-22 04:4x
※ 這一格只有在 A2.4 還沒跑的時候有意義。跑過就永遠答不了了

送出:
      sudo ip addr add 192.168.1.10/24 dev enxfc19286184c9
      loader-tftp.py probe --host 192.168.1.6 --timeout 3 --retries 3
原始回應:
      ok  DATA opcode 3 from 192.168.1.6:2098, 0 bytes in 1 block
      rx_packets 0 -> 2
      ip neigh: 192.168.1.6 lladdr 56:aa:a5:5a:7d:e8 REACHABLE

判定: ✅ 成立。2026-08-22 桌面對 P9-4 那句話的更正，量到了
反證檢查: 測前寫「(d) 第四格沒有回應 → 網路堆疊不是在 escape 視窗起來的，
      P9-4 結果欄那句『只在序列埠打過 IPCONFIG 之後才存在』是對的，而
      notes/loader-interrupts-and-console.md 對 0x80401D18 的讀法要改寫」。
      實際:DATA opcode 3 從 192.168.1.6:2098 回來，rx_packets 0 -> 2，
      ip neigh REACHABLE —— 前件不成立，(d) 不觸發

計畫外，而且它是這一格的第二個獨立證人:
      IPCONFIG 之前  192.168.1.6 -> MAC 56:aa:a5:5a:7d:e8
      IPCONFIG 之後  10.1.1.1    -> MAC 56:0a:01:01:01:e8
      0a 01 01 01 就是 10.1.1.1。所以中間四個 byte 是 IPCONFIG 的 handler 寫的,
      不是開機時寫的 —— 也就是說,堆疊在 192.168.1.6 上回應的時候,身上帶著一個
      不可能來自 IPCONFIG 的 MAC,因為 IPCONFIG 從來沒跑過。
      登記簿原本記的「loader 從 IP 合成 MAC」對了機制、錯了時機

儀器缺陷（runsheet,當場發現）:
      A2.9 步驟 0 沒有把主機介面拉起來,而它當時是 administratively DOWN。
      照著寫的跑法會在一個「一次開機只能量一次」的格子上做出偽陰性
```

```text
T-98  P9-19   假的提示字元與前導空白，四格加一支儀器            2026-08-22 05:1x
可行性: ★★★★★   驗證狀態(測前): unverified

2a 前導空白（含對照組,同一場,同一條指令,只差一個空白）:
      ' DW 80540000 2'  -> Unknown command !
      'DW 80540000 2'   -> 80540000: A9FDA5F8 40713F77 AB0C8A74 B7566FE0
      判定: 成立
      ※ 對照組原本被跳過,是問出來才補的。沒有它,「因為空白被拒」跟
        「這條指令現在本來就不會動」長得一模一樣

2b TAB:
      八個空白 -> 出現了,而且一場之內出現三次
      Unknown command ! -> 【沒有出現】。兩次 TAB+Enter 都是安靜地回一個提示字元
      判定: 一半成立一半反證。預測把兩條路混成一條 ——
            整行只有空白 -> 0x804091C8 的 blez,安靜
            空白後面接 token -> 抱怨

2c 假提示字元（本列存在的理由）:
      TAB（不按 Enter）-> 主機送 probe -> 等 .Success! 後的 <RealTek> -> 直接打
      'DW 80540000 2'  -> Unknown command !
      判定: 成立,而且逐 byte 對得上
      log 裡那一行 <RealTek> 與 DW 之間【沒有空白】—— 回顯只顯示重繪之後打的東西,
      而緩衝區裡留著重繪之前的八個空白。機制與錯覺在同四行裡

2d 對照組:
      一個裸 Enter -> 安靜地回提示字元,不抱怨
      同一行再打一次 -> 正常執行
      判定: 成立

2e 儀器:
      console-lint.py w08-a29.log
      -> 33 prompts (1 printed by the TFTP path), 3 rejection(s), 0 unexplained, exit 0
      -> 把 2c 的真實緩衝區內容重建成「八個空白 + DW 80540000 2」,
         也就是裝置看到的那一行,而螢幕從來沒顯示過
      2e 判定: 成立

      而且它抓到一件沒有人回報的事:
      offset 0x198  control-bytes  '\x1b[A\x1b'  —— 一次方向鍵,
      而這一節的開頭就寫著不准按方向鍵。
      【儀器做了操作者的口述沒有做的事】

判定: 🔶 partial —— 2a / 2c / 2d / 2e 成立，2b 的第二半反證
反證檢查: 測前寫「(a) 帶前導空白的那一行照常執行 → tokeniser 的讀法錯了」
      「(b) 按 TAB 回顯的不是八個空白（或什麼都沒有）→ GetLine 的 TAB 展開
      讀法錯」「(c) probe 跑完之後主控台沒有印出 <RealTek> → 假提示字元這個
      機制不存在」「(d) 第（3）格沒有被拒絕 → ISR 動到了主執行緒的狀態」
      「(e) console-lint 的判讀和主控台看到的不一致 → 先修工具」。
      實際:五條的前件全部不成立。**但 2b 的預測正文還有第二半** ——
      「八個空白，然後 Unknown command !」—— 而那個抱怨沒有出現，
      兩次 TAB+Enter 都是安靜地回提示字元。那一半沒有被任何一條反證涵蓋到，
      所以它是反證而不是「(b) 觸發」:整行只有空白走 0x804091C8 的 blez
```

```text
T-99  P9-17   把中斷還原,TFTP 會不會回來 —— B 版（對照組）      2026-08-22 05:4x
※ B 是設計成會失敗的那一半。B 活了就代表整條讀法錯了

送出（手打,console-dump.py 的 FORBIDDEN 擋掉 EW/J,刻意的）:
      EW 80540000 3C090000 35298100 3C0AB800 AD493000 00000000 ×6
      EW 80540028 00000000 ×8 03E00008 00000000
      DW 80540000 20        -> 二十個 word 與 mkramboot 的清單逐字相同
      J 80540000            -> ---Jump to address=80540000 -> 回到 <RealTek>
      DW B8003000 1         -> 00008100   ← payload 真的執行過的直接證據
      DW BB804104 5         -> 五個都是 ...38（J 自己清的,沒有人碰過）
      EW BB804104 007F0039 047F0039 087F0039 0C7F0039 107F0039
      DW BB804104 5         -> 五個回到 ...39
      probe --host 10.1.1.1 --timeout 3 --retries 3

原始回應:
      FAIL  no reply from 10.1.1.1 after 3 attempts at 3.0s
      rx_packets 4 -> 4     （不動）
      ip neigh  STALE -> FAILED

判定: ✅ 對照組如預測失敗 —— 這正是 B 存在的用途
反證檢查: 測前寫「(a) B 就復活了 → J 的 mtc0（0x804092B0-0x804092C0）沒有
      真的清掉 IE，或者 IE 不是收送路徑的必要條件。那時
      notes/loader-interrupts-and-console.md 對那四個指令的讀法要重寫」。
      實際:三個不共用程式碼的訊號一致說它沒有復活 —— probe 三次無回應、
      rx_packets 4 -> 4 不動、ip neigh 由 STALE 變 FAILED。(a) 不觸發

※ DW 80540000 20 的對照:寫入前那個位址是 A9FDA5F8 40713F77 AB0C8A74 B7566FE0
  —— 亂數,不是零。要是零,「寫進去了」跟「什麼都沒發生」會長得一樣
※ payload 是在站上重做的:步驟 1 讀到 GIMR0 = 0x00008100,桌面預測的是 0x8000。
  runsheet 那條「用讀回來的數字,不用桌面預測值」就是為了這個
```

```text
T-100 P9-17   C 版（GIMR0 + IE）—— 一個沒有預期到的第三種結果   2026-08-22 05:5x
※ 只重送第二行 EW。第一行 B 與 C 逐字相同,已經在 RAM 裡。
  所以整台機器只有那五個 word 變了 —— 真正的單一變數

送出:
      EW 80540028 40016000 00000000 3421001F 3821001E 40816000 00000000 ×3 03E00008 00000000
      DW 80540000 20        -> 逐字對上 C 版清單
      J 80540000

原始回應（逐字,而這一段是這一場最值錢的東西）:
      ---Jump to address=80540000
      File Start: 80500000,length=00000000
      **TFTP GET File probe,Size 00000000 Byte
      -^H<RealTek>

      【我沒有送任何 probe。】那是 B 版逾時的那一個請求 —— 它一直躺在接收路徑上,
      中斷掛著、被遮著,而 payload 把 IE 設成 1 的那一瞬間它被取走了

      第二個證人,計畫外:
      0xB8003004  88000004（任何 J 之前）
               -> 88000104（B 的 J 之後,IE 仍然是 0）
               -> 88000004（C 的 J 之後）

      DW B8003000 1  -> 00008100
      DW BB804104 5  -> 五個回到 ...39

      probe --host 10.1.1.1（跑了四次,其中一次帶封包擷取）:
      FAIL  no reply,三次都沒有
      rx_packets 4 -> 5 -> 6（每次 +1）
      ip neigh  FAILED -> REACHABLE
      主控台在這幾次 probe 期間【什麼都沒有印】

線上實際發生的事（tshark,把 neigh 條目先刪掉逼出一次全新的 ARP）:
      ...
      【ARP 0.9 ms 回應,三個 TFTP 請求零回應。】

判定: 🔶 partial
  成立 —— 而且比預測寫的位置低一層:IE 是封包【接收】缺的那一塊。
           只還原 GIMR0 永遠不可能補上它。線上量到的
  反證 —— 預測寫的是「DATA 回來」。沒有回來
反證檢查: 測前寫「(b) C 也不復活 → 中斷不是充分原因，剩下兩個候選
      （cache 維護或 payload 動到別的狀態、交換器在介面被關掉之後需要比 bit 0
      更多的重新初始化）活著。這是負面結果不是失敗」「(c) J 之後回不到提示字元
      → P9-16 的回程只對兩個指令的 payload 成立」「(d) 設完 IE 之後主控台不再
      回應 → 中斷風暴」「(e) DW 讀回來的二十個 word 與 mkramboot 印出來的不一致
      → 先修 EW 的打法，不要跳」。
      實際:(c)(d)(e) 三條前件都不成立 —— J 帶著二十個 word 兩次都回到提示字元、
      IE 設起來之後主控台照常回應、DW 20 兩次都逐字對上。
      **(b) 觸發了一半**:TFTP 沒有復活，所以那一條的前件成立 —— 但它寫的後件
      是錯的。ARP 在線上 0.9 ms 回應，把它列的兩個候選都殺掉了，
      所以中斷【是】封包接收的充分原因，剩下的是更窄的第三個候選
  反證 (c)(d)(e) 都不觸發:J 帶著【二十個 word】的 payload 兩次都回到提示字元
           (P9-16 只在兩個指令上量過,所以它的適用範圍比量到的更寬);
           IE 設起來之後主控台照常回應,沒有中斷風暴;
           DW 80540000 20 兩次都逐字對上,而且是對著亂數不是對著零
```

## 二、負面的那一半比正面的那一半值錢，而這次可以說清楚為什麼

登記簿在凍結 `P9-17` 的時候寫過：C 要是不復活，**剩下兩個候選活著** ——
「cache 維護或 payload 動到別的狀態」與「交換器在介面被關掉之後需要比 bit 0 更多的
重新初始化」。

**那次 ARP 往返把兩個都殺掉了。** 交換器兩個方向都通得過（請求出去、回應進來），
而 TFTP 底下那層堆疊活著（它解析了一個 frame 並且發出了回應）。所以問題不在交換器，
也不在被打壞的狀態。

**剩下的是一個更窄、而且是新的候選：TFTP 服務自己的狀態。** 而它有證據，不是猜的：

| | 健康的那次（2c） | C 的 `J` 之後 |
|---|---|---|
| 進度指示 | `/` + `\b` | `-` + `\b` |
| 完成訊息 | `*TFTP Client Download Success!` | **沒有** |
| | `.Success!` | **沒有** |
| 後面那個 `<RealTek>` | `console-lint` 判為 **TFTP 重繪** | `console-lint` 判為 **真的提示字元** |

`console-lint` 全場只數到 **1 次** 重繪，而那 1 次是 2c。所以第二個讀法獨立地同意
第一個：**那次傳輸開始了、沒有結束**，而你看到的提示字元是 `J` 的 handler 回來時
命令迴圈印的，不是傳輸完成印的。

**這是候選，不是結論。** 要定案得回桌面讀 TFTP 的狀態機，找出「完成」的條件，以及
一個被中斷打斷過的傳輸會停在哪。

## 三、這一場的儀器問題

**`tshark` 失敗了而且回報成功。** 第一次擷取寫到 `$FWRE_WORK/dumps/`，
`sudo tshark` 會降權，寫不進去 —— 印了 `Permission denied`、`0 packets captured`，
然後 **exit code 0**。零封包當時看起來像「線上真的什麼都沒有」，而那正好是
我要問的問題的答案之一。

**抓到它的是控制組而不是錯誤碼**：那次擷取連**我們自己送出去的**封包都是零，
而那些封包確定送出去了。`P0-4` 記過同一課 —— 「零在鏈路未經證實之前不是證據」。
改寫到 `/tmp` 之後五個封包全在。

**`$VAR` 在 Git Bash → WSL 的派送裡會被吃掉。** `for R in ...; do ... DW $R 1` 送出去
的是 `DW 1`，三次。沒有傷害（純讀），但浪費三條指令，而且它讓一件事被看見：

**`DW` 只給一個參數不會當掉。** 它讀了 `0x80000004` 然後正常返回。`FLW` 不檢查 argc
這件事，不是 `FLW` 獨有的。**機制沒有追**，記在這裡當觀察。

## 四、A2.4 的收尾規則與 A2.9 的先決條件互相矛盾，而沒有人寫下來

`A2.4` 結尾寫著「❌ **這一節結束後拔電重開。不要從 `IPCONFIG` 過的狀態直接 `J`**」。
`A2.9` 的先決條件寫著「步驟 2 與步驟 3 需要 `A2.4` 跑過」，而步驟 3 就是 `J`。

**今晚照 `A2.9` 走，並且它是對的** —— 步驟 2c 與步驟 3 的 `probe` 都需要 `IPCONFIG`，
而 `AUTOBURN 0` 讓上傳到不了 flash，那是保護不是風險。但 `A2.4` 那條規則是一句
**範圍超過它的觀察**的通則，跟 `runsheet` Part B 那句「`J` 之後沒有軟體的路回去」
同一個形狀，而那一句 2026-08-22 凌晨已經被推翻過一次。

## 五、下一場從哪裡開始

**這一場沒有留下任何要在裝置上做的事。** 三個新問題全部是桌面題：

1. **TFTP 服務為什麼在被中斷過的傳輸之後不再回應？** 讀狀態機，找完成條件。
   這是 `P9-17` 那一半反證留下來的東西，而它比原本的問題窄。
2. **`GIMR0` bit 8 是誰設的？** 讀 `0x80402F80` 的呼叫者。凍結時就寫好了指示。
3. **`IRR3` 為什麼是 0？** `0x8040A398` 寫 source 27 優先權那個讀法錯了，或那段
   沒有跑到。`installs` 那張表的 source-27 那一列先降級。

**W08 的登記簿到今晚是 `8 / 8`，0 outstanding。**

# 2026-08-23（日）W08 桌面第八場 —— 一封沒寄出去的信，一個八年前就在 NVD 上的 CVE，以及進站計畫多出來的一行

**桌面場，路由器全程沒有通電，夾子沒有碰過任何東西，CP2102 沒有接。今天沒有紀錄卡。**

這一則存在的理由跟前幾則桌面場一樣，但這次改的東西比較小、比較容易漏：
**進站計畫只多一行，而那一行是為了補一個已經量過卻沒留下證據的東西。**
今天真正變動的是揭露政策，那件事屬於 `PROGRESS.md` 和 `docs/disclosure.md`；
這裡只寫「下次插電之前要知道什麼」。

## 一、今天為什麼會回頭看這件事

作者問「之前抓到的漏洞後來要不要寄信」。查下去發現三件事，順序是這樣：

1. `docs/report-draft-2.md` 自己列的兩個封鎖條件，**在寫下它的那一天就都解掉了**
   —— prior-art 四路搜完（2026-08-18）、實機確認（同日 19:44，`T-43`）。
   檔案五天來一直寫著「not done」。
2. 因為檔案說沒準備好，那一項就沒有被重新考慮，**於是它被當成不重要**。
3. 而真正該搜卻沒搜的是 `D-4` 和 `D-11`。搜了 `D-4`，一個查詢就撞到
   **CVE-2018-13315**（2018-07，姊妹機種 A3002RU，同一個 handler，同一個機制）。

**教訓不是「要記得搜」，是「一個被證實的預測讀起來像是結束了」。**
`P10-3` 在矽上一次就成立、反證條件也沒踩到，那種結果不會讓人覺得還有功課沒做。
`D-1` 當初也是這個形狀。

## 二、進站計畫：只改一行，其餘不動

**`A3.13` 那一步從「記回應大小」改成「把回應 body 存下來」。**

理由是開放題 106：`$FWRE_WORK/dumps/w05-password-page.html`（5,332 bytes）
是目前唯一一份「已展開 SSI、帶著明文憑證」的實際回應，而**它在 repo 裡沒有任何
一個檔案提到過** —— 不知道是裝置回的還是模擬回的、哪一次跑的。
`notes/password-page-credentials.md` 因此只能把它當旁證，主證據用 rootfs 裡的模板。

補法很便宜，而且不用多一次電源循環：`A3.13` 的 bypass 那一發本來就要打，
把 body 存下來、跟那份 `cmp`，開放題 106 就關了。**六發變六發，只是多一個 `-o`。**

順帶會回答開放題 107（5,322 對 5,332 差十個 byte，算不出來）—— 但只有在
同一輪也把 `P10-4` 那個狀態的 body 存下來才行，而那要寫 flash，**這一次不做**。
107 留著。

**其餘一個字都沒改。** 站別順序、停損條件、`A3.11.2` 的還原步驟全部照舊。

## 三、揭露政策改了，對進站程序的影響：只有一個，而且是減法

從今天起 **不通報任何人，重現全部公開**（理由與它蓋不住的四件事在
`docs/disclosure.md`）。對站上的影響只有一條：

**`A3.11.2` 那個治理缺陷不用再處理了。** 它把一個未通報項目的完整 `curl` 寫進了
committed 檔案，而 2026-08-18 把「要不要回頭遮蔽」留給作者決定。現在兩個理由讓它
消失：那一項是 CVE-2018-13315（本來就公開），而且政策也改了。

⚠️ **但這是運氣，不是流程。** 那個缺陷不是被解決的，是被兩件無關的事蓋過去的。
如果 `P10-3` 不是八年前的 CVE，它今天還在。開放題 70 不因此關閉。
...
**如果那一場還有餘裕才做 `A3.13`**，而它現在多了「存 body」那一行。
`A3.13` 需要的是開機且服務中的狀態（第 3 站），跟上面那串第 2 站的計畫不同站，
所以它是「有時間才做」而不是「插進去做」——
**一個站別要一次電源循環，這件事沒有因為今天多一個開放題而改變。**


---

# 2026-09-25（五）W09 桌面第一場 —— 我為了檢查文件而寫的兩支工具，第一次跑出來的 39 條發現裡有 31 條是它們自己錯

**桌面場，裝置沒有上電，夾子沒有動，一個 byte 都沒有量。** 這一週是 W09，
交付物是「發布」，不是「量測」。照規矩這種日子也要留一筆，而且正是這種日子
最容易漏掉 —— 因為沒有東西可以逐字貼上來。

## 一、進場前的想法：這一場要做什麼，什麼結果會讓我停手

W09 的工作是把這個 repo 變成一個陌生人打得開的東西。兩份外部評審把問題列出來了，
而**評審本身也要被查核** —— 我照做之前先驗了一遍，結果三條已經過期：

| 評審說 | 實際 |
|---|---|
| 「`main` 是死的，停在 2026-08-07，5 個 commit」 | **`origin/main` 活著**，PR #23 在 2026-08-21 合進去，23 個 merge、276 個檔案。死掉的是**本機那個 `main` ref**，因為我從來沒有 `git checkout main && git pull`。評審讀的是我的工作目錄，不是 GitHub |
| 「`D-19` 標記為 `NOT SEARCHED`」 | 2026-08-23 就搜過了（`docs/disclosure.md:155`），評審日期的**隔天** |
| 「`study/writeup-導讀.md` 沒被追蹤，404」 | 有追蹤。大概是 CJK 檔名讓對方的檢查沒對上 |

**停手條件先寫下來**：如果 `make ci` 在搬完 `journal/` 之後綠不回來，就 `git reset --hard`
回到搬之前，改走「不搬檔、只改 README 導讀」那條路。這一條沒有用到。

## 二、先做儀器，再動檔案

順序是刻意的。要搬 6 個檔案、改 1,038 條連結，而**在搬之前必須有一支能看見連結壞掉的工具**，
否則搬完只能用眼睛看。

`tools/check-links.py` 的關鍵決定只有一個：**問 git，不要問檔案系統。**
`README.md` 連到 `plan/` 連了十一週，而 `plan/` 在 `.gitignore` 裡——
**它在我的磁碟上，`ls` 找得到，任何用 `os.path.exists` 寫的檢查都說它沒事。**
檔案系統跟 repository 不一致，而讀者拿到的是後者。

## 三、三個儀器 bug，全部往同一個方向錯

這一段是今天真正的產出，不是那 1,038 條連結。

**第一次跑，21 條斷連結。其中 18 條是我的工具。**

- **57**：slug 函式把**連續空白**縮成一個 hyphen。拿掉標點會留下它的空隙，
  所以 `codes — a` 在 GitHub 上是 `codes--a`，兩個 hyphen。**14 條正確的連結被報成壞的。**
- **58**：同一支工具在 slug 之前把行內 code 清成空白，於是
  `### 7.4 \`J2\` and the power switch` 的 `J2` 不見了。GitHub 是對**算繪後**的標題做 slug。
  **又 4 條。**

第二支 `tools/check-numbers.py` 第一版掃散文抓數字，跑出 18 條，**13 條是它自己**——
英文裡 `141 registered tests`（總數）跟 `Three registered tests are frozen against it`（子集）
是同一個文法。

- **59**：所以整支重寫成「宣稱點表」：檔案 + 句型 + 它的產生器。
  代價是新句子裡的新數字要人工登記；換來的是**一個宣稱點如果不再匹配，那本身就是失敗**，
  檢查覆蓋率不會安靜地縮小。

**值得寫下來的不是數量，是方向。** 前面 56 個儀器 bug 絕大多數是**安靜地放行**——
census 回 1 而不是 589、freeze 檢查雜湊一個空 list、封包擷取印 `0 packets captured` 然後 exit 0。
今天這三個全部是**無中生有**。而無中生有的儀器比安靜放行的更危險，理由跟儀器無關：
**假警報會被照做。** 看到「這個 anchor 壞了」的第一反應是去修 anchor，
而那 14 條正確的連結差一步就被我「修」成 14 條壞的。

抓到它們的方法便宜到有點難堪：**在動第一條發現之前，先把 18 條都讀完。**
18 條 anchor 失敗全部共用一個形狀——每一條都跨過一個 em dash、斜線或逗號——
那不是語料裡有 18 個錯，那是讀的人有 1 個錯。

## 四、順著一個數字，挖到 writeup 裡最嚴重的一個錯

`README` 草稿寫「six builds」，`writeup/02-corpus.md` 標題寫「Five builds」。
去對的時候發現的不是一個數字問題：

- `writeup/02-corpus.md` 的表格裡有一列是 **`V4.1.5cu`**。
  **這個字串在整個 repo 裡只出現在那一個檔案。** 沒有 `SOURCES.json` 條目、
  沒有 manifest、沒有 report、沒有第二個檔案提過它。**從來沒有下載過，也從來沒有分析過。**
- 同一張表**漏掉了三個真的分析過的 image**（兩個 N300RT、一個 N200RE），
  而三章之後有一個結論整個靠它們。
- 而且那一章說「五個 image」，表格五列（含本機 dump），然後下面一節叫「第六個 image」——
  **四百字之內自相矛盾。**
- `writeup/07-across.md` 的 `root_form[]` 那一列寫 `57 | 58 | 57`。
  **正確是 `59 | 57 | 49`**，三個全錯，而且錯成一個「幾乎一樣」的形狀——
  真實數字說 2020 版**少了十條路由**，那是整張表最有意思的一格，被假的對稱蓋掉了。

**正確答案從來沒有藏起來**：`notes/three-way-read.md:107` 就是 `59 | 57 | 49`，
`notes/dispatch-table.md` 有位址，三份 report 自己也是。
**那一章不是跟證據不合，是跟這個 repo 自己的筆記不合，而沒有任何東西在比對這兩邊。**

同一張表的其他每一列都對得上 `reports/`（gate 那列 `5 | 6 | 8` 就是三份 gate report 的
`findings_by_rule.R2`）。**一張每一格都可查、而其中一格沒被查的表，比一張沒人查的表更糟。**

## 五、下一次進站，有一件事變了

**站別順序、停損條件、電源循環的算法——一個字都沒改。** 上一場寫的
`A2.1` → `A2.2` → `A2.9` 步驟 0 → `A2.4` → `A2.9` 步驟 1 → 2 → 3 → 斷電重開 → `A2.2`
原封不動，`A3.13` 那個「有時間才做、要存 body」也還在。

**變的是路徑。** `runsheet.md` 現在是 `journal/runsheet.md`，`RUNBOOK.md` 是
`journal/RUNBOOK.md`，`BENCH-LOG.md`（這個檔）是 `journal/BENCH-LOG.md`。
進站第一件事是開 runsheet，而那個路徑是手指記住的東西，不是腦子記住的。
`make doctor`、`make ci`、`tools/` 底下每一支指令**都沒有變**——
六個 checker 的路徑常數已經改好，`make ci` 跑綠了才寫這一段。

**另外開了一個開放題（110）**：沒有任何檢查在比對 `writeup/` 裡的數字跟它來源的 report。
今天那個 `57 | 58 | 57` 就是從這個縫掉下去的，而 `check-numbers.py` 只管前門的總數，
**管不到一章裡引用的單一量測值**。這一題不用進站就能答，但它不是今天的工作。


---

# 2026-09-25（五）W09 桌面第二場 —— 本機全綠、遠端全紅，而中間那份 repo 一個 byte 都沒變

**桌面場，裝置沒有上電。** 這一場是 `git push` 之後才開始的，而它會存在只有一個理由：
**我去看了遠端 CI。**

## 一、發生什麼

`make ci` 在這台機器上綠的。push 上去，GitHub 上兩個 run 都紅。

`check-numbers` 報 **14 個過期數字**。而那 14 個，在這台機器上全部是對的。

原因在 `count-checks.sh` 一個幾個月前寫的 shell 預設值：
...
**跑不起來的套件，被算成 0 個 check，然後加進總數。**
GitHub 那個 job 沒有 pytest、沒有 flashrom，所以總數是 **468**，
這台是 **626**，而工具拿 468 去對前門的 626，說前門錯了。

## 二、為什麼這一個特別值得寫下來

前面五個儀器 bug（57–60 加上這個之前的）我都是在這台機器上抓到的。
**這一個在這台機器上抓不到，而且永遠抓不到** —— 因為 bug 不在 repo 裡，
bug 是**兩台機器之間的差**。repo 在兩邊是 byte-identical 的。

而它其實是這個專案自己那條規矩，從一個沒講過的方向走進來：
**「工具回報 0 也是一種主張」**。`${n:-0}` 這五個字元每一次少一個相依套件就在做那個主張，
而且是在一個沒有人會看的地方做的。

它變得可見，只因為終於有東西**比對了兩個環境** —— 跟橫著讀六個 build 是同一個動作。

## 三、修法，以及修法本身的邊界

`count-checks.sh` 現在把「沒印出數字」記成 `--` 而不是 `0`，
並且多印一行 `INCOMPLETE:<哪幾個>`。
`check-numbers.py` 看到那一行就**不判** `checks` / `guards` / `pytest`，
而且**每一次都把「沒判」印出來**。

**這留下一個洞，而且要講清楚：** 在 GitHub 上，那三個數字永遠不會被驗證。
只有這台機器驗得了它們。跟 `check-gates` 一樣（`plan/` 不在 runner 上）——
差別只在於現在它會說出來，而不是安靜地算一個地板值。

`tools/test-check-numbers.sh` 新增，12 個案例，其中三個就是在釘這件事：
INCOMPLETE 要跳過、完整的要採用、**而且一張沒有 INCOMPLETE 的表不可以被當成不完整**
（沒有第三個，前兩個就跟「永遠跳過」分不出來）。

## 四、下一次進站，什麼都沒變

站別順序、停損條件、`A3.13` 存 body —— 一個字都沒改。
今天兩場都沒有碰到裝置。
