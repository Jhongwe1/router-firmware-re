...
1. **走 console 拿完整 4 MiB。** 儀器要能:上機自檢(雙 `FLR` + `CMP`)、分塊驗證、掉字元自動重讀、**拼不完整就不吐檔案**。
2. dump 落地後:三個結構檢查(對照 8/15 console 讀到的窗口)、抽 2018 的 `/bin/boa`、三版橫著讀。
3. CH341A 的 pin 28 補量一次,把「焊接失敗」這個假設證實或推翻,結果寫回這裡。
4. 那顆跳線的實驗。
5. `LSP5526`(從 Day 2 拖到現在了)。
