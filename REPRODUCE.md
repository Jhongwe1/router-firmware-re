...
|---|---|---|---|
| **T1** | this clone and an internet connection | the two **published** firmware images, every report derived from them, and **638 checks that prove this project's own instruments can fail** (`make count-checks` re-derives that number and prints the table it comes from) | 30 min, most of it downloads |
...
**508 guard cases across twenty-four suites, plus 130 parser tests, and `make ci`
runs all of them** — **638 checks** from a clone, with no device. The number is
