...
| 層 | 動到裝置 | 為什麼這一節存在 | 最後驗證 |
|---|---|---|---|
| T2 | **一個 flash byte 都不寫。** 步驟 1 全是 `DW`；步驟 2 只是打字；步驟 3 用 `EW` 寫 RAM 與五個交換器暫存器，斷電即還原 | [`RUNBOOK` §8.12.47](RUNBOOK.md) | 尚未執行（桌面 2026-08-22 設計，預測已凍結） |
...
**預期**：主控台上出現**恰好八個空白**，然後 `Unknown command !`。
