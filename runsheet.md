...
| `A2.9` | 中斷：先讀線路，再用 payload 把它接回去 | `P9-18` `P9-17` `P9-19` |
...
>
> ⚠️ **`A2.9` 是這條規則明講的例外，而且它是對的。** `A2.9` 的先決條件寫著
> 「步驟 2 與步驟 3 需要 `A2.4` 跑過」，而步驟 3 就是 `J` —— 兩節直接矛盾，
> 而在 2026-08-22 之前沒有人寫下來。**照 `A2.9` 走**：2c 與步驟 3 的 `probe`
> 都需要 `IPCONFIG`，而 `AUTOBURN 0` 讓上傳到不了 flash，那是保護不是風險。
> 上面那條是一句**範圍超過它的觀察**的通則，跟 Part B 那句「`J` 之後沒有軟體的
> 路回去」同一個形狀 —— 而那一句 2026-08-22 凌晨已經被 `P9-16` 推翻過一次。
...
> ❌ **停止條件，五條：**
> 1. 步驟 1 任何一格的 `flash#` 數字隨第四個參數改變 → **整節停**。靜態讀法錯了，
>    回去讀 printf 的 o32 vararg 配置，不要再試別的值。
> 2. 步驟 2b 讀到的五個值有任何一個 bit 0 本來就是 0 → **不要寫**。這五個 register
>    不是我以為的那五個，前提錯了。
> 3. 步驟 2f 寫回原值之後 `probe` 沒有恢復 → 這個開關在這台上不可逆。
>    **之後任何一場都不准再碰這五個 register**，而且 `J` 的副作用比目前寫的重。
> 4. 步驟 3b 的 `DW` 讀回來不是 `03e00008` / `00000000` → **不要跳**。
>    `EB` 打錯一個 byte，`J` 過去就是一次隨機執行。
> 5. **任何一格出現「換一個值再試試看」的念頭 → 停，收工，回桌面。**
>    需要試第二個值才知道答案，代表預測不夠具體；而在裝置前面搜尋不是量測，
>    它只是把電源循環換成一個沒有反證條件的結果。
>    **這一條是本節四個步驟共用的，不是只針對步驟 4。**

> 📌 **這五條在 2026-08-22 之前不存在。** 本節步驟 4 的結語寫著「見下面的停止條件
> 第 5 條」，而這一節從頭到尾沒有任何編號的停止條件 —— 最接近的一份在 `A2.7`，
> 在**上面**，而且只有四條。`BENCH-LOG.md` 2026-08-22 也照著引用了同一個不存在的
> 編號。**指向一條不存在的規則的指標，比沒有指標更糟**：讀的人以為自己被規則擋著，
> 而實際上什麼都沒有擋。內容一直都在（就是步驟 4 那句「不要在裝置前面設計它」），
> 缺的是它被編號、被放在被指到的位置。`tools/check-runsheet.py` 現在會檢查
> 「停止條件第 N 條」這種引用真的解析得到。

### A2.9 🔌 中斷：先讀線路，再用 payload 把它接回去（關 `P9-18` · `P9-17` · `P9-19`）

| 層 | 動到裝置 | 為什麼這一節存在 | 最後驗證 |
|---|---|---|---|
| T2 | **一個 flash byte 都不寫。** 步驟 1 全是 `DW`；步驟 2 只是打字；步驟 3 用 `EW` 寫 RAM 與五個交換器暫存器，斷電即還原 | [`RUNBOOK` §8.12.47](RUNBOOK.md) | **2026-08-22 全節執行**，零 flash 寫入，`J` 兩次都回到提示字元，三項判 `partial` |

> 🔴 **`<RealTek>` 底下不要按方向鍵，也不要按 TAB。** 方向鍵送出 `1b 5b 41`；
> **TAB 被 `GetLine` 展開成正好八個空白**（`0x8040713C`–`0x80407168`），寫進行緩衝區
> 也回顯出來，然後 `argv[0]` 變成空字串。2026-08-21 就是這樣掉了一行乾淨的 `EB`。
> **步驟 2 會故意重現這件事，那是本節唯一允許按 TAB 的地方。**

**先決條件**：板子停在 `<RealTek>`（`A2.2` 剛跑完）；**步驟 0 必須在 `A2.4`
之前**；步驟 2 與步驟 3 需要 `A2.4` 跑過（`IPCONFIG` 是 RAM 狀態，每次開機重設）

**桌面預備**（進站之前做完，站上不重算）：

```bash
# 兩個 payload，只差五個 word，而且五個都落在第二行 EW 上。
# GIMR0 的值先用預測值產生一份，步驟 1 讀到真值之後如果不同，用真值重跑。
python3 tools/mkramboot.py --irq-restore 8000 --load 80540000 \
        --report "$HOME/fwre-work/dumps/w08-irqboot.json"
python3 tools/mkramboot.py --irq-restore 8000 --load 80540000 --no-set-ie \
        --report "$HOME/fwre-work/dumps/w08-irqboot-noie.json"
```

---

#### 步驟 0 —— `IPCONFIG` 之前，網路堆疊起來了沒（`P9-18` 第四格）

**這一格只有在 `A2.4` 還沒跑的時候有意義。** 讀法說 `0x80408468` 在進命令迴圈之前
就把乙太網初始化完、把 `IE` 設起來、把 TFTP 服務起在編譯進去的 `192.168.1.6`
（`0x80401D1C`），而 `IPCONFIG` 的 handler 只換位址。

> 🔴 **先把介面拉起來，而且要等到 carrier。** 2026-08-22 這一格差一點做出偽陰性：
> `enx…` 當時是 administratively **DOWN**，而本節原本只寫 `ip addr add` —— 加位址
> **不會**把介面帶起來。**這一格一次開機只能量一次**（`A2.4` 一跑就永遠答不了），
> 所以一次偽陰性就是一次白跑的電源循環。
...
**先看到這個才往下**（`carrier` 在介面 DOWN 的時候讀回 `Invalid argument`，不是 `0`）：

```text
1
```

> ⚠️ **`carrier` 是 1 不代表線路真的活著。** 這片 rtl8153 有紀錄會對空氣宣告
> carrier（`PROGRESS.md` 2026-08-22）。它只是「可以往下走」的門檻，**判定看 probe**。

```bash
python3 -u tools/loader-tftp.py probe --host 192.168.1.6 --timeout 3 --retries 3 \
        --report "$HOME/fwre-work/dumps/w08-p918-default-ip.json"
sudo ip addr del 192.168.1.10/24 dev enxfc19286184c9
```

| 讀到 | 意思 | 判定 |
|---|---|---|
| `ok DATA opcode 3 from 192.168.1.6:…` | 堆疊在 escape 視窗就起來了，`IPCONFIG` 換的是位址不是開關 | ✅ 這一格成立 |
| 三次都沒有回應 | 堆疊要等 `IPCONFIG` —— `P9-4` 結果欄那句話是對的 | ❌ 反證。記下來，`notes/loader-interrupts-and-console.md` 對 `0x80401D18` 的讀法要改寫 |

---

#### 步驟 1 —— 中斷接線的三個讀值（`P9-18`）

**全部是 `DW`，零寫入。這一步可以否決步驟 3。**

```bash
python3 -u tools/console-dump.py cmd --at-prompt --port /dev/ttyUSB0 DW B8003000 1
python3 -u tools/console-dump.py cmd --at-prompt --port /dev/ttyUSB0 DW B800300C 1
python3 -u tools/console-dump.py cmd --at-prompt --port /dev/ttyUSB0 DW B8003014 1
```

**預期**（`DW` 的長度是十進位的 word 數）：
...
> 📌 **這是 2026-08-22 量到的，不是預測。** 三件事寫在這裡而不是留給下一個人重推：
> **`GIMR0` bit 8 是 1**（那一格當初刻意沒有下注，落在「`0x80402FB8` 在乙太網
> 初始化路徑上」這一邊，後續是開放題 104）；**`IRR3` 是全零**，而預測寫 bit 12–15
> = 3 —— **那一條沒有寫反證分支，所以它不算分，只被記下來**（開放題 105）；
> `DW` 的長度是**十進位的 word 數**，但輸出**一行四個 word 無條件進位**，所以
> `1` 印四個、`5` 印八個。

| 讀到 | 意思 | 判定 |
|---|---|---|
| `GIMR0` 的 **bit 15 = 1**、**bit 27 = 0** | `request_IRQ(15,"eth0")` 跑過了；`SPEED` 的初始化把舊值寫回去了 | ✅ 往下走 |
| bit 8 是 1 | `0x80402FB8` 在乙太網初始化的路徑上 | 📌 兩種都要記，**不准事後改判哪一種是預期** |
| bit 8 是 0 | `0x80402FB8` 不在那條路徑上，那個 `\|= 0x100` 屬於別的子系統 | 📌 同上 |
| **bit 15 = 0** | `request_IRQ` 沒跑到 | ❌ **停止條件 1。步驟 3 不要送。** |
| **`GIMR0` 整個是 `00000000`** | 提示字元下中斷全被遮著 | ❌ **停止條件 2。整節停**，兩個讀法至少有一個錯 |
| `IRR1` 的 bit 28–31 不是 3 | 路由暫存器的模型錯了 | ❌ 反證，記下來，`installs` 那張表要降級 |

> 🔴 **把 `GIMR0` 的完整八位數抄在紀錄卡上。** 步驟 3 的 payload 要用**這個數字**，
> 不是記憶裡的，也不是桌面預備時的預測值。不同就用真值重跑 `mkramboot.py`。

---

#### 步驟 2 —— 假的提示字元與前導空白（`P9-19`）

**先決條件**：`A2.4` 跑過（步驟 2c 要用 `probe`）。四格都只是打字，零寫入。

**2a. 前導空白**（手打，第一行前面**恰好一個空白**）
...
| 讀到 | 意思 | 判定 |
|---|---|---|
| 第一行 `Unknown command !`，第二行正常印出兩個 word | tokeniser 在測空白之前就存了 `argv[0]`（`0x80407290` / `0x804072D4`） | ✅ |
| 第一行照常執行 | 靜態讀法錯了 | ❌ 反證。`notes/loader-interrupts-and-console.md` 的 102 那半段要重寫 |

**2b. TAB**（手打：按一次 TAB，再按 Enter）

**預期**：主控台上出現**恰好八個空白**，然後**安靜地回一個新的提示字元**。

> 🔴 **2026-08-22 更正：這裡不會出現 `Unknown command !`。** 原本寫會，而它
> 把 dispatcher 分得很清楚的兩條路混成一條：**整行只有空白** → `0x804091C8`
> 的 `blez`，安靜；**空白後面接 token** → 抱怨。八個空白本身出現了，同一場
> 出現三次，那一半是這一格真正要的東西 —— 它是 2026-08-21 那次的成因。
> **步驟 2c 不受影響**，因為 2c 的空白後面接了一條指令。

**2c. 假提示字元 —— 這一格是重現 2026-08-21**

順序不能換：**先按一次 TAB（不要按 Enter）**，再從主機跑

```bash
python3 -u tools/loader-tftp.py probe --host 10.1.1.1 \
        --report "$HOME/fwre-work/dumps/w08-p919-probe.json"
```

等主控台印完 `.Success!` 和它後面那個 `<RealTek>`，**不要按 Enter**，直接手打：
...
| 讀到 | 意思 | 判定 |
|---|---|---|
| `Unknown command !` | 那個 `<RealTek>` 是 `0x80401CD0` 在中斷裡印的，行緩衝區裡還留著八個空白 | ✅ |
| 正常執行 | 中斷印出來的東西**有**清到行緩衝區 | ❌ 反證，而且比預測有意思 —— ISR 動到了主執行緒的狀態 |

**2d. 對照組**：按一次 Enter（預期**靜靜地**回一個新提示字元，不印任何抱怨），
同一行再打一次 `DW 80540000 2` —— 預期正常。

**2e. 收工後把整場 log 餵給儀器**（桌面，回到主機上跑）

```bash
python3 tools/console-lint.py "$HOME/fwre-work/dumps/w08-a29.log"
```

**預期**：每一個 `Unknown command !` 都被歸到
`leading-whitespace` / `control-bytes` / `not-a-command` 其中之一，
`unexplained` 是 0，`repaints` 至少 1。**有任何一個 `unexplained` 就先修工具。**

---

#### 步驟 3 —— 把中斷還原，看 TFTP 回不回來（`P9-17`）

**只有步驟 1 的 `GIMR0` bit 15 是 1 才做這一步。**

**3a. 先抄五個 PHY 原值**（跟 `A2.8` 步驟 2b 同一條）

```bash
python3 -u tools/console-dump.py cmd --at-prompt --port /dev/ttyUSB0 DW BB804104 5
```

**3b. B 版：只還原 `GIMR0`**（手打；兩行 `EW` 由 `mkramboot.py --no-set-ie` 印出）

打完之後**先驗證再跳**：

```bash
python3 -u tools/console-dump.py cmd --at-prompt --port /dev/ttyUSB0 DW 80540000 20
```

**對不上就不要跳。** 對得上再手打 `J 80540000`。
...
**預期**：`GIMR0` 回到步驟 1 抄下來的值（**payload 跑過了的直接證據**）；
五個 PHY bit 0 全是 0（**`J` 自己清的**，沒有人碰過）。

**3d. 還原五個 PHY bit，再 `probe`**（手打 `EW`，值來自 3a）

```bash
python3 -u tools/loader-tftp.py probe --host 10.1.1.1 --timeout 3 --retries 3 \
        --report "$HOME/fwre-work/dumps/w08-p917-b.json"
ip neigh show 10.1.1.1
cat /sys/class/net/enxfc19286184c9/statistics/rx_packets
```

**預期：仍然逾時。** `IE` 還是 0，所以中斷不會被取。

| 讀到 | 意思 | 判定 |
|---|---|---|
| 三次無回應，`ip neigh` `FAILED`，`rx_packets` 不動 | `GIMR0` 單獨不夠 | ✅ 往下走 3e |
| **有 DATA 回來** | `J` 的 `mtc0` 沒有真的清掉 `IE`，或 `IE` 不是必要條件 | ❌ 反證。**記下來，跳過 3e**，回桌面重讀 `0x804092B0`–`0x804092C0` |

**3e. C 版：`GIMR0` + `IE`**（手打；兩行 `EW` 由 `mkramboot.py` 不帶 `--no-set-ie` 印出。
**第一行跟 B 版逐字相同，只有第二行的前五個值不一樣**）

同樣先 `DW 80540000 20` 驗證，再 `J 80540000`，再還原五個 PHY bit，再 `probe`。

| 讀到 | 意思 | 判定 |
|---|---|---|
| DATA 回來，`ip neigh` `REACHABLE`，`rx_packets` 前進 | **`J` 之後網路死掉的未歸因那一半就是被遮掉的中斷，而且是 `IE` 那一半** | ✅ `P9-17` `confirmed` |
| 還是逾時 | 中斷不是充分原因 | ❌ 反證。**這是負面結果不是失敗** —— 剩兩個候選：cache 維護或 payload 動到別的狀態、交換器需要比 bit 0 更多的重新初始化。**不要當場試別的值**，見停止條件 5 |
| 主控台在 `J` 之後不再回應 | 中斷風暴：`GIMR0` 打開時有掛著的事件而 ISR 清不掉 | ❌ 斷電重來。flash 一個 byte 都沒動過，代價是一次電源循環，**而它本身是一個發現** |

---

#### 收尾

**這一節結束時 loader 被 `J` 動過兩次。** 跟 `A2.8` 同一條規則：
**接下來要跑任何別的第 2 站小節之前，先斷電重開、重跑 `A2.2`。**

> ❌ **停止條件，五條：**
> 1. 步驟 1 的 `GIMR0` bit 15 是 0 → **步驟 3 不要送。** `request_IRQ` 沒跑到，
>    整條「中斷驅動」的讀法失去支柱。
> 2. 步驟 1 的 `GIMR0` 整個讀回 `00000000` → **整節停。** 兩個讀法至少有一個錯。
> 3. 步驟 3b / 3e 的 `DW 80540000 20` 對不上 `mkramboot.py` 印出來的清單 →
>    **不要跳。** 先修 `EW` 的打法：一行最多十八個值，`GetLine` 只收 128 個字元。
> 4. 3d 或 3e 之後主控台不再回應 → 走 `A4.2`。**flash 一個 byte 都沒被寫過**，
>    所以斷電重開一定回得來；真的回不來，那本身就是這一節最大的發現。
> 5. **任何一格出現「換一個值再試試看」的念頭 → 停，收工，回桌面。**
>    跟 `A2.8` 的第 5 條同一條，而這一節更需要它：兩個 payload 的差別是設計出來的
>    單一變數，在裝置前面加第三個變數會讓前兩個也失去意義。


### B-W08 增補之六 —— `A2.9` 實跑（2026-08-22）

**跑了什麼**：`A2.1` → `A2.2` → `A2.9` 步驟 0 → `A2.4` → `A2.9` 步驟 1 → 步驟 2 →
步驟 3 → 斷電重開 → `A2.2`。**照增補之五的順序，一格都沒有換。**
...
| 登記簿 | `P9-18` `P9-19` `P9-17` 三項 **`partial`**；W08 收在 **8 / 8**，0 outstanding |

**這一場改了 `A2.9` 與 `A2.4` 兩節本身，不只是填結果：**

| 改了什麼 | 為什麼 |
|---|---|
| `A2.9` 步驟 0 加 `ip link set … up` 與 carrier 等待 | 介面當時是 administratively DOWN，而本節只寫了 `ip addr add`。**這一格一次開機只能量一次**，照原文跑會做出偽陰性 |
| `A2.9` 步驟 1 的預期輸出換成量到的數字 | 原本寫 `0000?100` / `3xxxxxxx`，那是預測不是輸出。**一個「大致對」的預期輸出，會讓操作者在真的不對的時候也覺得大致對** —— 跟增補之四那一條同一句 |
| `A2.9` 步驟 2b 的預期輸出反過來 | 預測寫「八個空白**然後** `Unknown command !`」，實際是**安靜**。整行只有空白走 `blez`，空白接 token 才抱怨 |
| `A2.4` 加一條指向 `A2.9` 的例外 | `A2.4` 結尾要求「不要從 `IPCONFIG` 過的狀態 `J`」，`A2.9` 的先決條件正好要求那樣。**兩節直接矛盾，而沒有人寫下來** |

> 🔴 **`A2.5` 從此不必再跑，而理由變了。** 2026-08-22 凌晨寫的是「這一場沒有要接著
> 跑 `P9-10`」。**`P9-10` 現在被砍了**，所以那句話從「這一場沒有」變成「以後也不會有」。

> 📌 **`P9-16` 的適用範圍比它量到的更寬。** 它是在一個**兩個指令**的 payload 上證明
> 「`J` 之後回得來」的，而這一場兩次 `J` 都帶著**二十個 word**的 payload 回到提示字元。
> 寫在這裡，是因為反過來的錯誤（把一次觀察寫成通則）這份檔案已經犯過一次。
...
## B-W08 增補之五（2026-08-22 桌面，寫在下一次插電之前）

**這一則開一個新小節 `A2.9`，補上 `A2.8` 一組不存在的停止條件，並且更正一句
別的地方寫下的話。**

**新小節 `A2.9`**：`P9-18`（讀中斷接線）→ `P9-19`（假提示字元與前導空白）→
`P9-17`（兩個 payload 的階梯）。三項都零 flash 寫入、都可返回、都不需要電源循環。
...
| 為什麼是這個順序 | |
|---|---|
| `A2.9` 步驟 0 在 `A2.4` **之前** | 它問的是「網路堆疊在 `IPCONFIG` 之前就起來了沒」。`A2.4` 一跑，這一格就永遠答不了了 |
| 步驟 1 在步驟 3 之前 | 步驟 1 **可以否決步驟 3**：`GIMR0` 的 bit 15 是 0 的話，兩個 payload 一個都不該送 |
| 步驟 2 在步驟 3 之前 | 步驟 2 要打字、要用 `probe`，而步驟 3 結束時 loader 已經被 `J` 動過兩次。**先做便宜又乾淨的那個** |
| `A2.9` 之後一定要斷電重開 | 跟 `A2.8` 同一條：被 `J` 動過的 loader 不是別的小節寫作時假設的那一台 |
| 不做 `A2.5`、`A2.8` | 兩節的目的都達成、寫入已還原。**`A2.5` 只有在同一場要接著跑 `P9-10` 時才有意義**，這一場沒有 |
| 不做第 5 站 | 仍然卡在開放題 97，`P9-5`/`P9-6`/`P9-7` 維持 `⬜` |

> 🔴 **`A2.8` 步驟 4 的結語一直寫著「見下面的停止條件第 5 條」，而 `A2.8`
> 從頭到尾沒有任何編號的停止條件。** 最接近的一份在 `A2.7`，在**上面**，而且
> 只有四條。`BENCH-LOG.md` 2026-08-22 也照著引用了同一個不存在的編號。
> 內容一直都在（步驟 4 那句「不要在裝置前面設計它」），缺的是它被編號、
> 被放在被指到的位置 —— 而**指向一條不存在的規則的指標，比沒有指標更糟**。
> `A2.8` 現在有五條，`A2.9` 也有五條，而 `tools/check-runsheet.py` 現在會檢查
> 兩件事：標題宣告的條數要對得上實際的條數，以及「第 N 條」這種引用要解析得到。

**這一場的桌面產出**（在插電之前就完成，進站不重算）：

| | |
|---|---|
| `tools/loader-unpack.py --irq` | 中斷接線的解碼器：每一個 `mtc0 $12` 的 bit 0 用逐位元推導算出來（不是比對形狀）、每一個 `GIMR0` 存取、每一個 `request_IRQ` 站點與它的 handler 和裝置名字、命令提示字元的字元來源碰過哪些位址。`reports/bootloader-unit-2018.json` 多一個 `interrupt_wiring` 段 |
| `tools/console-lint.py` | 照 dispatcher 的方式讀 `picocom` log：分辨真提示字元與中斷印出來的假提示字元、跨越假提示字元重建行緩衝區、**把每一個 `Unknown command !` 歸因或明講不知道**。13 個守衛案例 |
| `tools/mkramboot.py --irq-restore` | 兩個 payload，只差五個 word，五個都在第二行 `EW` 上。模擬器擴到 `sw` / `mfc0` / `mtc0` / `jr`，並且檢查 `EW` 那一行不會被 `GetLine` 的 128 字元截掉 |
| 守衛案例 | `test-loader-unpack.sh` 26 → 34、`test-mkramboot.sh` 32 → 42、`test-check-runsheet.sh` 38 → 42，新增 `test-console-lint.sh` 13 |
| 三個新登記項 | `P9-17` `P9-18` `P9-19`，預測與反證都在插電之前凍結（freeze `7ade2454…`） |
