...
| `A1.2` | 從一份 clone 跑到全部報告，外加 153 個守衛案例 | — |
...
| 層 | 你需要 | 做得到哪幾節 |
|---|---|---|
| **T1** | 一份 clone + 網路 | **`A1.1`、`A1.2`、`A4.1`** —— `make ci` 的 **263 個檢查**，一台裝置都不用 |
...
> 該失敗的時候失敗** —— 153 個守衛案例的存在目的就是證明每一個拒絕是活的，
> 加上 110 個 fwrecon 測試 = `make ci` 的 263 個檢查。五分鐘，一份 clone。
>
> **數字要能自己重數，否則它會漂移**：`make ci 2>&1 | grep -c '^  ok'` 減掉
> 最後那一行總結 = 153（2026-08-17 夜），`110 passed` 是 pytest 那一行。
>
...
> `test-console-write.sh`（28），**九支套件共 153 個，`make ci` 全部跑**。
...
> **這一站是整份文件唯一「只有一台筆電也做得完」的部分**，而 `make ci` 是 263 個檢查。先把它跑綠，再去插線 —— 儀器壞掉的時候，你會希望那件事是在有硬體之前就知道的。
...
**這 153 個守衛案例加上 110 個 fwrecon 測試，存在的目的不是證明工具會動，
是證明工具的每一個拒絕是活的。** 例如：
