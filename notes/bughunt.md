...
| 24 | mitigation | `/proc/sys/kernel/randomize_va_space` | — | — | **N as a defence, and the flag claims otherwise** — the sysctl reads **2**, which is full randomisation, while the layout is fully determined by the ELF files across two processes and four boots: `libuClibc` at `0x2aae3000` in `boa` and `0x2aabe000` in `wscd`, differing by exactly `libapmib.so`'s `0x25000` mapped span, measured in `/proc/<pid>/maps`. Linux 2.6.30.9 on MIPS does not act on the flag. **A hardening flag is a claim by a source, and a source is not a measurement** — reading it and stopping would have closed `P5-2` as refuted without one address being looked at. `system` is at `0x2ab08460` | [`mips-ret2libc.md`](mips-ret2libc.md) · [`libbase-unit-2018.json`](../reports/libbase-unit-2018.json) · `P5-2` |
