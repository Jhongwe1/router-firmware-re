...
> ## 🔴 2026-08-17 更新:命令搬走了,這一節現在只講「為什麼」
>
...
> 命令有一個擁有者(`runsheet.md`),推理有一個擁有者(這裡)。兩邊互相連結,
> 沒有任何一段話出現兩次。
>
> **為什麼要這樣改:** §8.12 是**參考書**形狀 —— 它假設讀者知道自己為什麼在這裡。
...
>
> **`make ci` 會驗 runsheet 裡每一個命令仍然解析得到** —— `make` 目標存在、
> 檔案路徑存在、**每一個旗標真的在那支工具的 `--help` 裡**、`§8.x.y` 交叉引用
> 指得到。手寫可以;命令壞掉不能出貨。`tools/test-check-runsheet.sh` 15 個案例
> 證明那支檢查器自己會失敗。
...
| **W05** 收尾(下半) | 1 → 2 → **§8.9.3 四行** → 3 → **10** → 4 → **11** → **9** → 5 → **12** → 7 → 3 → 8 | console 那半場先跑,不需要網段 |
