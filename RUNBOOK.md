...
> 🔴 **`--mib` 從「幫 id 取名字」變成必要的。** 344 筆裡有 13 筆 id 的 bit 15 是 1，
> 那是 table-valued，而其中一筆 `WLAN_ROOT` 是 **22,044 byte，佔 45,226 byte 解壓
> 設定的一半**。到 2026-08-21 之前這些筆只被報成長度與 hex，而
> `notes/compcs-decode.md` 讀起來像是整區都解完了 —— 那五個檢查全部只在檢查
> **串流**（校驗和、環形填充、TLV 數、尾巴 1 byte），沒有一個在問「某一筆的值有沒有
> 被看懂」，因為一筆沒解開的值仍然是格式正確的 TLV。
>
> 現在遞迴解，而幾何是從 `libapmib.so` 的 60-byte 記錄尾巴讀出來的
> （`count = total_size / element_size`），不是從資料猜的。沒給 `--mib` 就沒有那組
> 數字，工具會照實把每一筆 table-valued 標成「沒有下探」而不是假裝解完。
>
...
**2026-08-19 追加：這一節的成本模型是錯的，而修正它比上面任何一句都重要。**

上面寫的是「做完之後必須把映射刪掉」，那假設了 daemon 會活著等你刪。**它不會。**
實測：**任何 `inet_addr()` 不接受的 `NewInternalClient` 都會終止 `/bin/miniigd`**，
而它不會自己回來——`ps` 兩分鐘後看不到那個行程，也沒有東西 respawn 它。所以
這一節真正的單位不是「一發請求」，是**一次斷電重開**。一晚上燒掉三次。

**而「元字元」不是原因，這一點是靠對照組才分得出來的。** 第一次的推論是
「反引號害的」，因為那一發帶反引號。但 22 個 `A`——同樣長度、同樣不是 IP、
**一個元字元都沒有**——殺得一模一樣，而 `NewInternalClient=10.1.1.1` 這一發
daemon 活著而且還能再答一次讀取。**三個點才定得出這條線**：良性 IP 活、
帶元字元的死、不帶元字元的非 IP 也死。少任何一個，結論都會是錯的那一個。

**所以 `P6-1` 的答案不是「有」也不是「沒有」，是登記簿沒有預料的第三種。**
登記簿的反證條件寫「有回應但 SOAP 欄位被過濾 → 這個版本已修」——**欄位完全沒有
被過濾**：那個值原樣進了 `iptables` 的 DNAT 目標，`inet_addr()` 失敗回的
`INADDR_NONE` 被當成位址用，規則長出 `to:255.255.255.255`。**規則裝進去了，
然後行程才死。** 那不是「已修」，也不是 CVE-2014-8361 的命令執行。

**分辨「行程消失」與「行程還在但關掉 listener」，要靠 `ps` 而不是再連一次。**
兩者對外都是 `connection refused`。這個區別不是學術的：`P6-3` 的 `wscd` 正是
後者，而「卡住」與「崩潰」的差別是那整條發現的核心。這一節現在要求走
`A3.23` 的 telnet 路去看 `ps`，而那條路本來是為了取 core dump 開的。

**`(0 references)` 不能單獨拿來判 `P8-7` 的後半。** 同一個 `iptables` 輸出裡，
`MINIUPNPD` 這條 chain 沒有被 `PREROUTING` 引用、`ip_forward` 是 `0` —— 而那一場
WAN 線沒有接。**一台沒有 WAN 的路由器不轉送，不是關於一台有 WAN 的路由器的證據。**
把它讀成「映射不通」會是一個很好聽、而且錯的結論。

**最後一條，而它是關於儀器不是關於裝置：payload 不准經過 shell。**
第一發原本要送 25 bytes，**本機 shell 先把反引號展開了**，實際送出去的是本機
`ping` 的 stdout，431 bytes。那一發不但沒測到東西，還燒掉一次斷電——而且
**看起來像一個結果**：連線關閉、daemon 死。要不是工具自己記下了 `sent`，
那 431 bytes 會被當成 22 bytes 來解釋。`--arg-file` 就是為此存在，
而它在沒有 `--inject` 時拒絕執行，因為一次良性基準和一次注入必須是兩條命令。
**這是 `P9-9` 結果註記裡記過的同一個缺陷，隔一天在另一個工具上重演。**
...
### 8.12.40 第五站為什麼是一個站，以及為什麼它的第一步不碰板子　→ `runsheet.md` `A5.1`

**站號就是裝置狀態，而夾子帶來的是第五種狀態。** 第 1 站不碰裝置，第 2 站停在
`<RealTek>`，第 3 站開機服務中，第 4 站收工。夾在 `U19` 上讀寫的時候，板子**斷電**，
電從程式器經由 `VCC` 那支腳灌進整塊 3.3 V 電源網 —— 那不是前面四種裡的任何一種，
把它塞進第 1 站會讓那一站「一個 byte 都沒動到」這句話變成假的。

**代價的單位也不一樣，而這決定了節與節的順序。** 第 2、3 站的成本單位是一次拔插電源；
第 5 站是一次夾子就座。所以 `A5.1`–`A5.5` 是照「夾上去之後能不能不拆下來」排的：
`A5.2` 的識別、`A5.3` 的第一次就座、`A5.4` 的演練、`A5.5` 的植入，中間只有 `A5.3`
必須刻意拆一次重夾，而那一次拆解本身就是它要的證據。

**文件順序在這裡不等於執行順序，而這是全檔唯一的例外。** 這個檔案的其他部分刻意
維持「從頭讀到尾就是一個能照跑的順序」，而斷電的站接不在收工的站後面。解法不是把
它硬插進第 1 站和第 2 站中間 —— 那會讓站號在文件裡倒著跑 —— 而是承認例外，並且把
真正的順序交給 Part B，那本來就是 Part B 擁有的那一份狀態。

**為什麼第一步是量電表而不是夾上去。** 2026-08-16 量過一次，那張表是這一節要推翻的
東西：`VCC`、`WP#`、`HOLD#` 三支跟著板子的 LDO 走 3.3 V，而 `CS#`、`CLK`、`DI`、`DO`
四支 —— 也就是晶片會被驅動的每一支 —— 全是 5 V。**`VCC` 讀 3.3 V 正是那個陷阱**，
它讓一塊會超壓的板子看起來安全。當時做了 3.3 V 改機、腳位仍然是 5 V，而**原因沒有被
隔離**：三個候選（走線沒真的斷、掀起來的腳還碰著焊墊、`DO` 的上拉跟晶片電源無關所以
理論不完整），而分開它們的那一個量測 —— **CH341A 自己的 pin 28** —— 沒有被量。

所以 `A5.1` 量的是三件事而不是一件：pin 28 是**因**，座上八支腳是**果**，兩個都量到
3.3 V 才叫互相印證；第三件是 `U19` 的本體寬度，因為 SOP-8 有 150 mil 和 208 mil 兩種，
硬夾窄夾子會掀腳，而掀腳的板子開不了機。**「這次改成功了」是一個主張，跟其他主張一樣
需要兩個來源。**

`lsusb` 缺席在這一站是 `FAIL` 而不是 `--`，理由是儀器 bug 45：降級成 skip 的檢查等於
沒有跑，而它降級的環境通常正是工具最少的那一個。`flash-read.sh` 沒有 `lsusb` 只會印
一行 warning 就繼續；`flash-write.sh` 直接拒跑。兩者的差別就是「讀錯」與「寫錯」的差別。

### 8.12.41 三個 byte 為什麼是這個專案最便宜的一個結論　→ `runsheet.md` `A5.2`

**`U19` 的型號從 2026-08-14 到今天只有一個來源，而那個來源是一行印在塑膠上的字。**
`cFeon QH32B-104HIP` 在放大鏡下 `Q` 跟 `O` 幾乎一樣，`notes/hardware-inspection.md`
把這件事當成一個已知的判讀風險寫進去了，而不是靠瞇眼睛決定。

**這一段到 2026-08-21 為止寫的是「`flashrom` 的晶片資料庫是用型號當索引的，所以它不是
第二個來源」——那句話是錯的。** `flashrom` 比對的是 `manufacture_id` 與 `model_id`，
**型號名是查表的輸出，不是索引**。要分辨這兩件事不需要讀原始碼：叫 `flashrom` 去模擬
`W25Q128FV`，它回報的是 `W25Q128.V`。**餵進去的字串跟吐出來的字串不一樣** —— 如果
資料庫是用型號名索引的，這個名字會原樣回來。這一量做在 `dummy` 程式器上，沒有夾子、
沒有裝置、沒有 CH341A。

**所以 `flashrom` 叫出來的名字，是「這三個 byte 是哪一顆」的第二個來源**，而它**不是**
「那三個 byte 是什麼」的第二個來源 —— 那三個 byte 是同一支夾子、同一條匯流排讀來的。
`P9-7` 問的正是型號，而它的單一來源是封裝上那行字；這一行剛好補上第二個。

**而這個來源在 2026-08-21 之前一次都沒有被印出來過。** `tools/flash-read.sh` 解那行字
的樣式是 `Found [^\n]*flash chip`，`[^\n]` 在 POSIX 方括號裡不是「非換行」，是
「不是反斜線、也不是字母 n」——而 flashrom 印的是 `Found Eon flash chip ...`。
少印一行不會失敗，只會安靜。儀器 bug 50。

**同一天還量出第二件，而它比第一件貴。** 這支工具向 flashrom 要的是一個 `-V`，
而 `RDID returned` 這一行 flashrom 1.3.0 **只在 `-VVV` 才印**（`-V`、`-VV` 都是零行，
量在 `dummy` 上）。也就是說：晶片答得好好的，工具會說 `no JEDEC id came back. The chip
is not answering.`，然後叫操作者去檢查電源、pin 1、夾子寬度 —— **三個都是錯的修法**，
在工作台上、夾子在手上、在這一週唯一撐得住整週論證的那一節。儀器 bug 51。

**修法不只是把 `-V` 改成 `-VVV`。** 那樣下一次 flashrom 改格式時，同一個錯誤會用同一種
方式再出現一次。真正的修法是**讓工具分得出兩種失敗**：log 裡如果有一行 flashrom 的
辨識結果，匯流排就是好的 —— 那麼缺的是那一行字，不是那顆晶片，而這時候叫人重新就座
是在破壞一個沒有壞的東西。`rdid_failure_kind` 就是這一句話，`tools/test-flash-tools.sh`
兩個案例分別餵它兩種 log。

**而這兩件事各有兩份，因為 `tools/flash-write.sh` 有自己的一份。** 那一支的
`identify()` 同樣要 `-V`、同樣撈 `RDID returned`，而它的下場不是誤報是**拒絕**：
「不寫一顆不出聲的晶片」。方向是對的 —— 但那代表 `A5.4` 與 `A5.5` 的每一次寫入
都會被擋掉，理由指著一顆答得好好的晶片。**一個失敗安全的工具，仍然可以把一整場燒掉，
只要它指錯地方。**

**修法是 `tools/lib/flashrom-parse.sh`，兩支工具共用 —— 而禁區沒有合併。**
這個區別是這一節第三課：兩條路徑各自拒絕同樣那兩段，是**政策**，而政策重複一次
等於多驗證一次；另一支程式的輸出格式是**事實**，而事實有兩份就是同一個錯有兩個家的
原因。**一個事實一個擁有者，一條政策兩條路徑。**

**這兩件事合起來是這一節的第二課，而它跟第一課同形。** 第一課是「型號只有一個來源」；
第二課是「**你的儀器對另一支程式輸出格式的每一個假設，也只有一個來源，而且它不會報錯，
它只會安靜**」。四個解析樣式裡有兩個是錯的，四天沒有人知道，而套件裡沒有一個案例
碰得到它們 —— 因為那四個假設當時不是函式，是寫在一支需要夾子才跑得起來的流程中間的
四行 `grep`。**現在它們是函式，所以它們可以在沒有硬體的桌面上失敗。**

**真正獨立的來源是晶片被問到時自己回報的三個 byte**，而這一節就是去問。

**而在夾子上去之前，這個預測已經從『大概是這樣』變成『有機制』。** boot loader 自己
帶著一張 SPI 晶片描述表：32 筆記錄、stride `0x20`、每一筆前四個 byte 是三 byte 的
JEDEC id、偏移 `0x18` 是指向型號字串的指標。`tools/loader-unpack.py --chip-table`
把它解出來，**載入基底是回推的而不是假設的** —— 每一個型號指標都必須被同一個
page-aligned 基底解釋，落在 `0x80400000`，而回推的漏斗結尾必須是一，不是「取第一個」。

表裡的 Eon 家族是 `1c3115`、`1c3116`、`1c3015`、`1c3016`。**`1c7016` 一列都沒有。**
這解釋了 boot log 從 2026-08-15 就印著、三週沒有人解釋的 `chipName: UNKNOWN`：
loader 問了，晶片答了，而 loader 的表裡沒有這個答案。

**這個機制讓 `A5.2` 可以失敗，而那才是它的價值。** 若晶片回 `1c3016`，那這顆是
EN25Q32、封裝上的字被誤讀、loader 一直都認得它、`UNKNOWN` 另有原因 —— 上面整段推理
作廢。一個只能被確認的預測不是預測。

**還有一件事是這張表順帶交出來的，而它比型號更值得記。** 表裡三個 JEDEC id 重複：
`c84018` 出現三次，`ef4016` 兩次，而 **`ef3016` 兩次卻指向兩個不同的型號字串**
（`W25X32` 與 `W25X64`）—— W25X64 真正的 id 是 `ef3017`。廠商的查表資料裡有一列
是錯的，而它是靠解表解出來的，不是靠讀字串。另外 `20ba17` 那一列的型號字串是
`MCba17`，看起來像有人拿 id 手打型號時打壞了。

**以及一個沒有答案的問題，它現在有編號了。** 表裡每一筆都帶著最小抹除單位，非
Spansion 的每一列都是 4 KiB。這顆晶片**對不到任何一列**，而 G3.5 在真機上量到的
`FLW` 行為是 4 KiB 的讀-改-抹-寫循環。所以要嘛預設值就是 4 KiB，要嘛查表失敗後還是
選到了 4 KiB —— 哪一個，沒讀過那段程式碼，不主張。

**而 2026-08-21 那一場證明了這一節前面所有的推理都還少一個前提：晶片要有電。**
`A5.2` 從頭到尾假設「夾子坐上去、晶片就會被供電」，那一句話在這塊板子上是錯的。
三種供電組態 —— 程式器自己、主機後面板直出的 USB 埠、外接一顆 ESP8266 的 3.3 V
穩壓 —— 晶片腳上的 `VCC` 都停在 **1.70 V**，而第三種的供電側穩穩給著 3.3 V。
**三種差很多的供電能力給出同一個電壓，那比較像鉗位不像拉垮**，但這一場沒有把
「板子鉗住」與「夾子那一段的串聯電阻」分開，所以它是一條開放題而不是一個結論。

**這一節因此多一個閘門，而它擋的不是失敗，是一種特別難看的成功。** 在 1.7 V 讀出來的
東西如果看起來是對的，它會以「兩支儀器對同一顆晶粒說法不同」的形式進到 `P9-5` 的
比對裡 —— **那正是 `P9-5` 存在要消除的混淆項**。一個測試如果會自己製造出它要消除的
東西，那個測試就不是在量它以為在量的事情。

**還有一件跟第一課同形的：進站前寫下的停止條件表，漏掉了實際發生的那一種失敗。**
2026-08-20 那張表的四條全部假設「工具回了一個不對的答案」——
`no JEDEC id`、`MORE THAN ONE id`、`A5.1` 沒過、`A5.3` 沒過。而那一晚的失敗是
**工具還沒開口就斷電了**：CH341A 自己從 USB 匯流排上消失。**一張只想像過
「答錯」的停止條件表，遇到「沒開口」的時候沒有一條會觸發**，而操作者手上的
下一個動作預設就是重新就座 —— 也就是去修一個沒有壞的東西。

### 8.12.42 一次就座讀兩遍，證明的是傳輸穩定，不是讀得對　→ `runsheet.md` `A5.3`

**這是整個 W08 的那一列。** 到 2026-08-19 為止，這顆快閃記憶體上的每一個 byte 級主張
都只有一個來源：boot loader 自己的 `FLR` 指令，走裝置自己的 UART。兩份完整 dump 逐
byte 相同、2026-08-15 的視窗也對得上 —— 而**三者全部走同一條 `FLR`**，所以一個系統性
的讀取錯誤對它們三個都是隱形的。`dumps/MANIFEST.json` 把這件事寫成
`not_corroborated_by`，README 的板子上掛著一個 ⚠️，而 `docs/lab-inventory.md` 說
只有一支 3.3 V 的程式器能了結它。

**而「讀兩遍」這個驗收條件本身比它聽起來弱，這也是 `flash-read.sh` 把兩個問題拆開的
理由。** 同一次就座裡的兩次讀共用它所有的失敗模式。掀起來的腳、被 SoC 拉住的線、
超壓的驅動 —— 每一個都產生穩定、可重現、格式完整、而且錯的答案，而那正是這個專案
一直在抓的那種失敗。所以工具問的是兩個問題：**可重現**由同一次就座的 N 次讀回答，
**正確**由四個都不是 flashrom 的東西回答，其中兩個在這一節裡（換一次就座、跟 `FLR`
比），一個在 `A5.2`（晶片自己回報的 id），一個在映像的結構本身（`fwrecon flashdump`）。

**預測是「整份 4 MiB 一個 byte 都不差」，而它有理由，也真的可能失敗。** `FLR` 那兩份
是 2026-08-16 讀的，中間這台被寫過好幾次：W06 的 PoC 改了 `H601` 九個 byte 又改回去，
W05/W07 的未認證 POST 改過 `COMPCS` 三個旗標，2026-08-19 按了 reset 鈕。W07 收尾那
一場量到 `flash default-sw` 把 `COMPCS` 與 `COMPDS` **兩區都從硬編碼重寫**，逐 byte
等於 2026-08-16 那一份，`H601` 也逐 byte 相同。所以預測整份相同不是樂觀 —— 而只要
開機途中有任何一條路寫過 flash，它就會不一樣，**那本身是一個結論**，而且是這個 repo
目前沒有的一個：沒有人知道這台開機時會不會寫自己的 flash。

**不一樣的時候，差在哪一段決定它是什麼。** 落在三個設定區，是裝置寫的，要去指出是誰
寫的；落在 kernel/rootfs 那一大段，是兩支儀器對同一顆晶片說法不同 —— 那要當成儀器
問題查，不是當成發現。這個順序很重要：先懷疑儀器，是這個專案抓到三個 tracer bug 的
那條規則。

### 8.12.43 為什麼要在沒有人在乎的 4 KiB 上先走一遍　→ `runsheet.md` `A5.4`

**下一節要改的是這台的管理憑證，所以復原路徑必須在那之前被走過，而不是被相信。**
「量了再修，而且讓復原腳本有能力失敗」是這個 repo 的規則之一，`A2.5` 的 `FLW` 演練
就是同一條規則在 boot loader 那一側的版本。這一節是它在夾子這一側的版本。

**位址是算出來的，不是挑的。** 常駐映像結束在 `0x180000 + 0x1CA041 = 0x34A041`
（`notes/flash-layout.md` 從真機讀出來的），`0x3FF000` 在它後面 690 KiB，而且現在
整段是 `FF`。這一節寫了什麼、抹回什麼，對開機、對設定、對這台任何一個已知的結論都
沒有影響 —— **爆炸半徑為零，而那正是演練需要的性質**。

**這一節證明的是「寫得回來」，不是「寫得進去」。** 一條抹除 → 寫入 → 讀回 → 再抹回
原狀的完整路徑，只有整條走過才算數。`flash-write.sh` 的 `commit` 一定會留下寫入前的
整份讀取並在最後印出路徑，因為那個檔案就是撤銷鍵；`plan` 預設會把它刪掉，因為 `plan`
不改任何東西，留下一份 4 MiB 的副本只會讓工作目錄長出一堆分不清哪份是哪份的映像。

**而這支工具的安全論證是「差異」而不是「位址」，理由值得寫下來。** 檢查位址只證明你
**打進去的**那個位址是被允許的。比對整份映像的差異，證明的是**真的會動的那些 byte**
就是你要動的那些 —— 包含你不知道自己也改到了的那些，而後者才是弄壞晶片的那一種。
成本也是零：flashrom 本來就會先讀再寫，而且只抹除有差異的區塊。

### 8.12.44 五個 byte，兩筆欄位，以及那個 2 是怎麼算出來的　→ `runsheet.md` `A5.5`

**`COMPCS` 是壓縮的，所以「在某個欄位偏移上改一個 byte」這件事並不存在** —— 而這一列
的第一版預測正是那樣寫的，寫完當天在桌面上被自己推翻。記在這裡是因為它是這一節最
重要的一課：**一個做不出來的實驗比一個猜錯的預測更糟**，因為它會把人帶到工作台前才
發現。

**桌面上算出來的三件事，每一件都能單獨失敗。**

一、`admin` 這五個字面 byte 在整個壓縮區裡只出現**一次**，在 flash `0x00C0D1`。
前面緊接著 `b6`，那是 W04 從 `libapmib` 的 MIB 表裡定出來的 `USER_NAME`；後面不遠
是 `b7`，`USER_PASSWORD`。

二、`USER_PASSWORD` 那一筆是**往回指的參照**，不是第二份字面值 —— 而這是算出來的，
不是看出來的。把那五個 byte 改成 `zzzzz`，`fwrecon` 拒絕解碼，理由是 8-bit payload
校驗和變成 178 而不是 0。`zzzzz` 與 `admin` 的位元組和相差 **89**，而 **178 = 2 × 89**。
**那個 2 就是同一份字面值在解壓輸出裡被用了兩次。**

三、所以把它改成 `nimda` —— 同樣五個字元的重排 —— 校驗和一個數都不動。`fwrecon`
對改過的映像回 `checksum_ok`、`ring_fill_agrees`、344 筆，而且**恰好兩筆**值改變，
`USER_NAME` 與 `USER_PASSWORD` 都變成 `nimda`。

**為什麼要選一個不動校驗和的字串。** 因為它把變因減到一個。校驗和不動，這一發問的
就只剩「直寫有沒有到達執行中的系統」。校驗和留壞是**另一發**，問的是另一個問題
（裝置到底驗不驗），而那一發要單獨做、中間要還原 —— 兩件事一起改，兩件事就都答不了。
這是 `P6-1` 那個對照組的同一課：三個點才定得出一條線，任兩個都會支持錯的結論。

**第二發還有一個對象，而它是自己人。** `fwrecon compcs` 拒絕壞校驗和時印的是
「The device itself would reject this blob」。那是一句關於裝置行為的斷言，而到今天
為止沒有任何量測支撐它 —— 它是從格式推出來的。這一發就是讓這個 repo 自己的工具
對那句話負責。

**選 `COMPCS` 不選 `H601`，理由是復原路徑的數量。** `COMPCS` 有兩條互不相干的：把
原圖用同一個 `--allow` 寫回去，以及按 reset 鈕（2026-08-19 已經在這台上量過，
`flash default-sw` 會把它逐 byte 還原成硬編碼那份）。`H601` 一條都沒有 —— 它是這台
出廠時量進去的 MAC 與射頻校準，任何一份廠商映像裡都沒有，reset 也不還原它。所以
`flash-write.sh` 跟 `console-write.py` 一樣，**根本不提供寫它的路徑**，而不是提供
一個要加旗標才打得開的路徑。

**還原之前那一份 `seat-c` 讀取，是這一節裡唯一一件不為 `P9-6` 服務的事。**
它回答開放題 91：這台開機的時候，會不會寫自己的 flash。到今天為止沒有人知道，
而這個 repo 對它的所有推論都建立在「兩趟 `FLR` 的 hash 一樣」上 —— 那兩趟中間沒有
可控的開機事件，所以它們什麼都沒說。

**它為什麼比 `A5.3` 的比對更能回答那個問題，理由只有一句：混淆項少一個。**
`A5.3` 比的是夾子讀出來的影像與 boot loader 的 `FLR` 影像，那是**兩支儀器**。
那裡出現差異有兩個來源 —— 開機途中寫了，或兩支儀器對同一顆晶片說法不同 ——
而 `A5.3` 自己的文字就承認分不開，它只能叫人「先看差在哪一段」。
`seat-c` 比的是同一支夾子在兩個時間點讀出來的兩份影像，中間唯一發生的事件是一次
完整開機。**同一支儀器讀兩次，儀器差異這一項就不存在了。**

**成本是零，而這件事值得說清楚，因為零成本的量測最容易被跳過。** 循環 3 本來就要
再夾一次（還原半），`A5.3` 已經證明同一次就座裡讀兩遍是穩定的，所以這一步花的是
兩分鐘的讀取時間，不是一次夾子就座。**它必須在進站之前就被寫成預測** —— 一個
「順手做一下」的量測，事後看到差異就會變成解釋差異，而解釋不是量測。

**這一件事本來的方案是買一片 RP2040，用 PIO 把 `CLK`/`CS#`/`DI`/`DO` 當同步擷取，
在開機途中被動側錄 SPI 匯流排上的 opcode。** 那個方案更強 —— 它連
**開放題 89**（`FLW` 在查不到型號時用哪一個描述子：送 `0x20` 還是 `0xD8`）都一起答了，
因為擦除指令會直接出現在線上。但它要一片新板子、要在**通電**的板子上夾（本專案對
CH341A 明令禁止的動作，理由是兩個 master 在同一條匯流排上），而 SOIC-8 夾只有一個。
**用桌上已經有的東西回答同一個問題，而且答得比較窄** —— 側錄看得到「寫了什麼指令」，
`seat-c` 只看得到「寫完之後的結果」——是這一週該做的取捨，側錄退回開放題。

**最後：這一節如果成立，它是整篇 writeup 最短的一句話。** 五個 byte，沒有執行任何
一行程式碼，換掉一台路由器的管理帳號與密碼。不是回應、不是封包 —— 是快閃記憶體。

### 8.12.45 一個把協定方向搞反的主張，看起來不像錯的　→ `runsheet.md` `A2.7`

**這一節的工具差一點被寫成相反的東西，而過程比結果值得記。** 2026-08-21 從 loader
的 LZMA 第二階段讀到兩個格式字串 —— `**TFTP Client Upload, File Name: %s` 與
`*TFTP Client Download Success!` —— 把「Client」讀成 loader 自己，於是結論是
「要在工作站上起一個 TFTP 伺服器」。那句話寫進了 `PROGRESS.md` 與 `BENCH-LOG.md`，
而且推上去了。

**「Client」指的是對面那一端。loader 是伺服器。** 而反證它的量測，四天前就在這個
repo 裡：`T-09`，2026-08-17，一個 RRQ 送過去，回來 516 bytes 的 DATA。

**這件事的三個形狀，比修好它重要：**

一、**這是本專案第一條規矩，在專門用來執行那條規矩的那一週被違反。** 一個來源、
讀一次、在上面蓋一個設計。第二來源不缺席、不昂貴、也不在別處——它在同一場、
四小時前才追加過的同一個檔案裡。

二、**`console-dump.py` 早就把答案印在螢幕上了。** 它的 `rescue` 列出「什麼能證明
連線是活的」，第三項是「a TFTP read request comes back with DATA」。那描述的就是
一個伺服器。而 `tools/check-runsheet.py` 讀 `runsheet.md` 與 `RUNBOOK.md`，
**沒有任何東西讀工具自己印的字** —— 這跟儀器 bug 22 是同一個形狀，往外一層。

三、**一個把方向搞反的主張，長得不像錯的。** 它會變成一支被寫出來、被測過、被
commit 的工具，然後在工作台上於 port 69 聽著，而 loader 在等人來問。失敗會以
「救援路徑不通」的形式出現，在插了三次電之後，而且指向裝置而不是指向工具。

**唯一做對的一件事：工具不是先寫的，設計說明先寫。** 把主張寫成文字，是讓它大到
足以被檢查的原因。

**為什麼 `probe` 存在，而不是直接 `get`。** 它重現 `T-09`，而它報出來的東西裡最
重要的一項是 **transfer id**：TFTP 的回覆從一個新的臨時 port 出來，不是從 69。
`T-09` 量到的是 `:2098`。一支照 69 過濾的客戶端會什麼都收不到，然後把服務報成死的，
而一條只針對 69 寫的防火牆規則會造成同一個結果。所以客戶端把對方的**位址**釘死、
每一個 datagram 都檢查，而**埠**從第一個回覆學到之後釘死。

**為什麼 `get` 之前要先做開放題 96。** `T-09` 那 516 byte 對上了 flash `0x060010`
的 `cr6c` 酬載，而**檔名根本不存在**。所以 loader 供應的是什麼？兩個可能：RAM 裡
load address 的內容（那 `get` 就是 `FLR` 輸出的快速通道，4 MiB 從 105 分鐘變成幾秒），
或者它自己有一份固定來源（那 `get` 是另一回事）。**兩次 `FLR` 打不同的範圍、中間各
`get` 一次，一次進站就分得出來。** 在那之前，`get` 的來源是假設的，而工具自己的
輸出就這樣寫。

**而就算答案是「RAM」，`get` 也不是第二支儀器。** 它跟 `FLR`+`DB` 都經過 SoC 自己的
SPI 控制器：它排除掉序列線這一個失敗來源，其他什麼都沒排除。第二支儀器是夾子
（`A5.3`），這一支是**第二條傳輸路徑**。這兩個詞在這個 repo 裡不能互換。

**`put` 為什麼要去讀另一支工具的輸出。** `AUTOBURN` 決定上傳的東西會不會被寫進
flash，而這支工具看不到主控台。它可以「相信操作者跑過 `A2.4`」，那等於沒有檢查；
它也可以自己去開 serial port，那等於兩支工具搶同一個裝置。**第三條路是要求證據**：
`console-dump.py rescue` 會拒絕在沒看到 `AutoBurning=0` 的情況下繼續，並且把它送了
什麼、看到什麼寫成 JSON。`put` 解析那份 JSON，位址要對得上、回應要含 `AutoBurning=0`，
否則一個 byte 都不送。**三種失敗方式各有一個測試。**

**`J` 為什麼不自動化。** 跳轉是對唯一一台機器的狀態改變，它要發生在有人盯著主控台
的地方。而 `P9-12` 的凍結反證條件已經指出下一個坑：**`J` 之後如果主控台沒有輸出，
分不出「跳過去了但沒東西講話」與「根本沒跳」**。最便宜的分辨方式是讓上傳的映像第一
件事就往 UART 寫一個字元；沒有那個字元，這一列只能記 `partial`。

#### 一個正確執行的比對，對應到錯的答案

**同一天、同一節、同一種病，第二次。** 上面 §8.12.45 記的是「Client 指誰」被讀反；
這一條記的是**那一節的量測設計本身**在 2026-08-21 夜裡被拆開之後發現的四個錯，而其中
三個會讓它在動到任何 byte 之前就失敗，第四個會讓它**成功地印出一個錯的結論**。

**前三個是同一句話寫出來的**：`console-dump.py cmd … FLR 300000 81000000 1000`。
參數順序是 `FLW` 的（`FLR` 的第一個參數是目的地，RUNBOOK §8.7.8 與四份實錄都這樣寫）；
`cmd` 沒有任何處理 `(Y)es , (N)o ?` 的程式碼，所以那個 `FLR` 根本不會生效，而排隊中的
提示會吃掉下一個指令；而且沒有 `--at-prompt`，所以工具會對一塊已經停在 `<RealTek>`
的板子送 120 秒的 ESC，然後回報「什麼都沒回來 —— TX/RX 接反、port 錯、或板子沒上電」。
**三個都不是真正的原因**，而那正是 2026-08-21 白天 `A5.2` 那件事的同一個形狀：
一個會失敗的步驟，配上一段指著錯地方的失敗訊息。

**第四個才是真正該記的。** 那一節的判讀表只有兩格：兩份 `get` 不同 → loader 供應的是
RAM 裡 load address 的內容；兩份相同 → 它自己有一份固定來源。而**兩次 `FLR` 打的都是
一個不等於 load address 的暫存位址**，所以兩份必然相同 —— 於是那一節會做出正確的比對、
得到正確的 `0`、然後宣布第二個答案。**一個對的量測，一張錯的表。**

**為什麼會這樣，以及為什麼它跟 §8.12.45 是同一件事。** `FLR` 的參數順序有一個擁有者：
`tools/console-dump.py` 的 `flr()`。作業單把它用手抄了一遍，抄錯了，而
`tools/check-runsheet.py` 的說明自己就寫著它不檢查語意 —— 它會確認每一個旗標存在、
每一個子命令是真的，但一個位置參數放反它看不見，而且它誠實地說了自己看不見。
**修法不是去補那個檢查，是讓作業單不要有第二份。** 改寫後的 `A2.7` 一個手打的 `FLR`
都沒有：要 `FLR` 就跑 `console-dump.py dump`，而順序由那支工具的程式碼決定。
這跟「一份狀態一個擁有者」是同一條規則，只是這一次被複製的不是狀態，是**一個指令的
參數順序**。

**新的量測設計為什麼是四格而不是兩格。** 因為 loader 的程式碼讀出來之後，變數有兩個
不是一個：供應的**位址**取自 `0x8040D3A8`（`LOADADDR` 寫的），供應的**長度**取自
`0x8040DD28`（`FLR` 的第三個參數寫的）。一個把兩個變數綁在一起的實驗，答不出兩個問題。
四格各自只動一個東西：什麼都不動、`FLR` 打到非 load address、`FLR` 打到 load address、
...
**一、`put` 讀的那份 rescue 紀錄沒有年紀。** §8.12.45 說「`put` 去解析另一支工具的
輸出」，那是對的，但第一版只檢查兩件事：位址對不對、回應裡有沒有 `AutoBurning=0`。
**`AUTOBURN` 是 loader 裡的 RAM 變數**（`0x8040D4A0`），斷電就沒 —— 作業單 `A2.4` 的
表頭自己就寫著這句。所以一份四天前的 JSON 可以完整通過每一項檢查，同時對「現在正在
聽的那個 loader」一句話都沒說。**一個在唯一要緊的那個面向上不可能失敗的守衛**，
而猜錯的代價是對唯一一台機器寫 flash。現在它有一個年紀上限，而且工具明講：
上限**不等於**同一次開機，它只是把「任何時候寫的」換成「一小時內寫的」。

**二、`LOADADDR` 之前可以從「只讀」的工具送出去。** `console-dump.py cmd` 的
`FORBIDDEN` 擋掉 `FLW`／`EB`／`EW`／`AUTOBURN`／`J`，理由是那支工具說自己只讀。
`LOADADDR` 不寫 flash，所以它不在名單上 —— 但它寫 `0x8040D3A8`，而那個字決定
**下一次上傳落在哪裡**、**讀取從哪裡供應**、以及**自動執行路徑跳到哪裡**。跟 `AUTOBURN`
同一個等級。現在它進了 `FORBIDDEN`，並且在 `rescue` 裡有一個窄的家：三個拒絕
（對齊、KSEG、不准落在 loader 自己的映像），而且把 loader 回的那一行記進 JSON ——
於是**待會兒手打進 `J` 的那個數字，來源是 loader 自己的回應，不是同一個人的記憶**。

**三、`put` 的檔名之前是裝飾。** loader 的上傳路徑會拿檔名去比對兩個常數
（`0x80401208` / `0x8040122C`），命中就在傳輸結束的那一刻自己跳進去 —— 沒有 `J`，
沒有人在主控台前面。**這不是新發現**：`nfjrom` 是 Realtek SDK 自己的 MP image 名稱，
別的廠商的 GPL 釋出裡有 `nfjrom.script`，而這條 TFTP 救援流程連預設的 `192.168.1.6`
（這個 build 仍然帶著，在 `0x8040EDC0`）都在 OpenWrt 的 wiki 上。**它改變的是工具該怎麼
表現**，不是專案發現了什麼：`J` 由人來按是刻意的設計，而一個預設檔名不該是那個決定
被弄丟的地方。

#### `P9-12` 不能用「上傳任何一個檔案」關掉

**這一列的凍結反證條件 2026-08-18 就寫死了，而它指名了自己的盲點**：`J` 之後主控台
沒有輸出，就分不出「跳過去了但沒東西講話」與「根本沒跳」，那時只能記 `partial`；
而最便宜的分辨方式是**讓上傳的映像第一件事就往 UART 寫一個字元**。

**所以能關掉這一列的映像不存在於任何廠商的檔案裡，必須做一個出來** —— 而在
2026-08-21 之前，作業單指名的 `w08-ramboot.bin` 在整個 repo 裡只被提到一次，
就是那一行，**沒有任何工具、target 或腳本會產生它**。

`tools/mkramboot.py` 做的是四十行左右的大端序 MIPS：印一個帶 nonce 的橫幅，等一下，
再印一次，永遠。三件事值得記在這裡而不是記在工具的說明裡：

**一、UART 位址不是從記憶裡拿的，是從 loader 自己的 putchar 讀出來的**
（`0x80406B6C`：`ori a1,v0,0x2014` 是 LSR，`ori v1,v0,0x2000` 是 THR，
`andi v0,v0,0x60` 是 THRE|TEMT）。連**有上限的自旋**都照抄 —— loader 自己數到 6540
就不等了，直接寫。抄它的理由不是省事：一個無上限的自旋會把「UART 從來沒說它空了」
變成主控台上的沉默，而沉默正是 `P9-12` 唯一分不出來的那個觀察。

**二、那支工具的第一版是錯的，而抓到它的是模擬器不是眼睛。** 分支位移算成相對
`PC+8`（很多文件是這樣描述的），於是每一個分支都落早一個字。它組譯得出來、
反組譯看起來正常、`objdump` 也不會抱怨 —— **它只是會把橫幅的第一個 byte 印四十一次**。
把位移的定義釘死的是 loader 自己的兩個 `beqz`，不是任何一本參考書。
**這就是那支工具的每一次建置都要跑一遍模擬器的理由**：一個沒有人執行過的 payload
是一個假設，而它要被上傳進唯一一台機器的 RAM 然後被跳進去。

**三、模擬器第一版跟編碼器共用常數，所以它不是模型。** 守衛套件把 `UART_THR` 改掉，
payload 跟模型一起搬家，於是一個寫到錯位址的 `sb` 被模型高高興興收下。現在模擬器帶
**裝置的**位址，編碼器帶自己的，兩邊不一致就是這個檔案存在的理由。
七個「把 bug 放回去、建置必須變紅」的案例裡，有三個是這樣長出來的。

**而 `J` 自己會先印一行**（`---Jump to address=%X`，`0x8040B35C`）—— 那是 loader 印的，
不是 payload 印的。它把「沒有輸出」拆成兩種：有那行沒橫幅 = 跳了但目標沒講話；
連那行都沒有 = 根本沒跳。**這不會讓 payload 變得不必要**（那行只證明 loader 打算跳，
不證明目標執行了），但它把 `P9-12` 的三種結果分乾淨了，而三種結果各自記什麼，
寫在作業單那一節的表裡。


#### 進站之後：四格全中，而最有價值的是唯一一條被推翻的預測

**2026-08-21 夜，`A2.7` 全節跑完，四格的 sha256 全部命中桌面上算好的那四個。**
`0x80401ED4` 的讀法成立：位址取 `[0x8040D3A8]`（`LOADADDR` 寫），長度取
`[0x8040DD28]`（`FLR` 的第三個參數寫）。開放題 96 用量測關掉。

**而這一節設計裡最值錢的一件事，是它讓自己有可能被推翻。** 舊設計只有兩個結果，
兩個都會被解釋成「有結論」；新設計有四格加上「落不上任何一格」這第五種可能，
而第五種的意思是「今天的靜態讀法錯了」。**一個不能反證自己的實驗，跑不跑結果一樣。**

三個計畫外的東西，全部是免費的：

* **`DB 80500000 64` 在任何 `FLR` 之前就等於 flash `0x060010`** —— loader 在讓出
  ESC 視窗**之前**就把 `cr6c` 酬載搬進 RAM 了。這順帶解釋了 2026-08-17 的 `T-09`：
  它拿到的 516 個 byte 不是「loader 去讀了 flash」，是 RAM 裡那份副本。
* **`IPCONFIG` 不會重設長度全域** —— 第 4 格沒有送 `FLR`，長度仍然是 `0x1000`。
* **`:2098` → `:2099`**，在兩次不同的開機上各驗一次。

#### 一個模擬器認證過、而矽片否決掉的 payload —— 這一場真正的產出

**`P9-12` 的第一次跳轉成功了，而 payload 是壞的。** 主控台印
`---Jump to address=80500000`，然後每一輪吐出剛好 **16 個 byte**，重複了 272 次。
16 是 16550 發送 FIFO 的深度。

成因：`andi t2,t2,0x60` 坐在 `lbu t2,0(t1)` 的 **load delay slot** 裡。這顆核心的
load delay slot 是**架構層可見的**（MIPS-I，無互鎖），所以那個 `andi` 遮的是**上一次**
的讀值 —— 而它在第一個字元之後永遠非零。等待迴圈從來沒有等過。

**`tools/mkramboot.py` 每一次建置都會模擬，而它說那個 payload 會印出完整的橫幅。**
它模擬的是一顆有互鎖的核心。**一個比裝置寬容的模型不是模型** —— 它認證的正好是
裝置會拒絕的那些 bug。

**這是同一場裡第三次，模型錯在寬容的方向**：第一次是模擬器跟編碼器共用 MMIO 常數
（改編碼器，模型跟著搬家，於是寫到錯位址被收下）；第二次是分支位移算成相對 `PC+8`；
第三次是這個。三次都產生一個有自信的綠燈。

**而反證的證據是免費的，在進站之前就拿得到**：把 `stage2.bin` 的每一個 load 掃一遍，
數「後面那個指令有沒有讀剛載入的暫存器」。答案是 **1,474 個 load，0 個**，
其中 646 個後面直接接 `nop`。**那是編譯器在告訴你這顆核心的 hazard 規則。**

**更難看的一點：答案本來就在我抄的那段程式裡。** loader 自己的 putchar 在
`0x80406B88` 有一個 `nop`。我把 THR 與 LSR 的位址抄走了、把 6540 的自旋上限抄走了、
把那個 `nop` 當成填充丟掉了。**抄一段常式的時候，看起來像填充的那個指令，
正是要去問清楚的那一個。**

修法有兩層，而只有第二層是修法：payload 補兩個 `nop`（一層），
**模擬器改成遇到「在 load delay slot 裡讀暫存器」就拒絕**（第二層）。
第一層讓今天過關，第二層讓明天的 payload 不會再這樣過關。
複驗是一次單變數實驗：只加兩個 `nop`，其餘不動，橫幅完整。

#### 一張三格的判讀表，漏掉了實際發生的那一格

`A2.7` 進站前寫的 `J` 判讀表有三格：banner 重複 = `confirmed`；只有跳轉那行 =
跳了但沒講話；連跳轉那行都沒有 = 沒跳。

**實際發生的是第四格：banner 出現，但每一輪都在同一個字元被切掉。** 而那一格的
判定跟三格裡任何一格都不同 —— `P9-12` 仍然成立（裝置確實執行了一份沒看過的映像），
但要修的是映像不是結論。表已經補上第四格。

**這跟 2026-08-21 白天那張停止條件表漏掉「工具還沒開口就斷電」是同一種毛病**：
想像失敗的時候，只想像了「答案不對」，沒有想像「答案對了一半」。

### 8.12.46 一張手抄的表，錯在沒有人查得到的地方　→ `runsheet.md` `A2.8`

**開放題 98 問的是「`FLW` 的第四個參數做什麼」，而真正的答案是「這個問題是一張
手抄的表生出來的」。** 指令表 `0x8040DBC0` 在 2026-08-21 白天被人眼從十六進位
讀進 `notes/loader-tftp-and-commands.md`，寫成 `{name, help, argc, handler}`。
它是 `{name, argc, handler, help}`。**表裡每一格的值都是對的**，錯的是描述那個
記錄的那一句話 —— 而那正是最難被發現的一種錯：一個對的表配一句錯的話，
讀起來跟兩個都對完全一樣。

同一份手抄還漏掉 `FLW` 說明字串的後六個字（`: Write offset-data to SPI from
RAM` —— 恰好是指名那個欄位的六個字），並且把 `0xB8003000` 寫成「停計時器」，
而它是全域中斷遮罩，跟下一行的 `mtc0` 是同一個 `cli()` 的兩半。

#### 三個讀者，而不是一個

這一節的三個量測都不是新的實驗設計，它們是**把一個只有一個來源的結論補到三個**：

* `mips-linux-gnu-objdump`（原本唯一的那個）；
* `tools/loader-unpack.py --commands`，它有自己的 MIPS 解碼器，不呼叫 objdump，
  而且會**推導**而不是被告知：記錄從哪裡開始、哪一欄是計數、哪一欄是 handler
  （靠「指到的那個 word 是不是 `addiu sp,sp,-N`」分辨 handler 與說明字串），
  任何一項narrow 不到唯一解就拒絕出報告；
* 廠商自己的 C，在一份公開的 GPL 釋出裡（`rtl819x/bootcode/boot/monitor/`）。

**第三個來源解釋了前兩個看到的每一個常數**：`cnt2=0` 那一行的
`//strtoul(argv[3], …)` 是被註解掉的，`printf(… cnt2+1 …)` 就是主控台上那個
`flash#1` 與寫入呼叫拿到的 `0` 差一的原因，而 dispatcher 裡那段
`if (n_arg != argc-1)` 被包在 `#if 0` 裡 —— 所以指令表的計數欄位不是「有人忘了
維護」，是**有人關掉了唯一會讀它的那段程式**，欄位就從規格退化成註解。

**但它是一份較晚的 SDK、另一顆 SoC。** 它證明「為什麼」，不證明「這一台」。
所以三個來源的分工要說清楚：這一台的二進位是權威，工具是第二個獨立解碼器，
廠商原始碼是解釋。三者不一致時，前者贏，而不一致的那兩次都留在註記裡。

#### 為什麼「沒有指令讀 offset 4」是一個需要被證明會失敗的主張

`declared_argc_is_read_by_the_dispatcher: false` 是整個開放題 98 的答案，
而它離「一個寫死的 `False`」只有一行之遙。所以守衛套件裡有一個**反向對照**：
同一個合成映像，讀取那一段多加一個 `lw v1,4(s0)`，那個欄位就必須回 `true`。
沒有那一格，一支永遠回 `false` 的工具會通過所有拒絕案例。

同樣的道理用在每一個 handler 上：合成映像裡種了七種不同形狀的 handler
（什麼都不碰、檢查 argc、不檢查、計算索引、把載入放在延遲槽、跨呼叫存活、
兩條路徑其中一條覆寫 `$a1`），解碼器必須把七種讀成七種。**這七種不是憑空想的，
每一種都對應真表裡至少一列**，而其中兩種是版本一的線性掃描讀錯的那兩列：
`IPCONFIG` 被讀成完全不碰 `argv`（因為 `argc==0` 那一條路覆寫了 `$a1`），
`EB` 被讀成只讀一個槽（因為它的索引是算出來的）。

七個突變體逐一被套件抓到，而第一輪只抓到四個 —— 另外三個活下來，
不是因為工具對，是因為**合成映像裡沒有任何一個 handler 走那條路**。
一個沒有被證明會失敗的守衛案例，跟沒有那個案例是同一件事。

#### `J` 三件事一起做，所以「跳完之後網路死了」什麼都沒歸因

`0x804092AC` 遮掉全部中斷、`0x804092B0` 清掉 CP0 的 `IE`、
`0x804092F4`–`0x80409354` 把 `PCRP0`–`PCRP4` 的 `EnablePHYIf` 清掉，
然後才 `jalr`。**上一場的計畫寫的是「跳完之後跑一次 `get` 或 `ip neigh`」**，
而那個設計最好的結果也只是「網路死了」——四個充分原因並存，指不出哪一個。

改成不跳轉的開關實驗，是這一節唯一真正的設計改動：中斷全程不動，
只清那五個 bit，`get` 死掉、寫回去、`get` 活過來。**單一變數，而且可逆。**
上一場那一項沒有被刪掉 —— 它變成步驟 4，跑在 `J` 之後、而且是在
**loader 還活著**的狀態下跑（因為 `J` 是 `jalr`），所以它問的問題從
「網路死了嗎」升級成「中斷那一半有沒有份」。

#### 一句被推翻的話，而它是自己人寫的

`runsheet.md` Part B 的 `B-W08 進站實錄` 寫著「`J` 之後沒有軟體的路回去」。
那句話是從一次真實觀察來的 —— `P9-12` 的 payload 無窮迴圈，唯一回得去的路
是電源開關。**但那是 payload 的性質，不是 `J` 的性質。** `0x80409360` 是
`jalr` 不是 `jr`。一次觀察被寫成一條規則，而規則的範圍比觀察大。

**這件事不用「更正」處理，用一次量測處理。** 讀出指令不等於從那裡回來過，
所以它是 `P9-16`，八個 byte、一次 `J`、看提示字元有沒有回來。
`P9-10` 是一連串 RAM payload，每一次都省不省得掉一次電源循環，
是那一週能不能在一場之內做完的差別。

#### 那個沒有被測的當機

`FLW` 送少於三個參數會在 `strtoul` 解參考一個 NULL。它寫在 `A2.5` 的方框裡
當成打字警告，**而它沒有被排成一個測試項目，是刻意的**：一個已知會當機、
