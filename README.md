...
- [ ] **W07 — systematic bug hunt** (no gate) — 8 categories, not driven by known CVEs. **55 of 58 register rows closed**, and the bench visit happened on 2026-08-18 — 21 rows closed on the silicon plus 4 upgraded off emulated evidence, across five power cycles and two cable moves. The heaviest result was not on the list: an unauthenticated POST from W05 had written `DHCP_MTU_SIZE=0` to flash, and this unit has been unable to obtain a WAN address ever since — through every reboot, and through four bench sessions that had no reason to look. The deliverable is [`notes/bughunt.md`](notes/bughunt.md), twenty verdicts each pointing at a file under `reports/`, and a *relatively safe* section the same size as the verdict table. Three of the twenty are this project's own findings **withdrawn**, and one more turned out to have a CVE against it — the most recent retraction came from building an instrument that could tell the emulator's behaviour from the firmware's ([`tools/alignfix/`](tools/alignfix/)), not from arguing about a caveat
